...
## 📦 Estoque com histórico de compras

O cadastro de produtos mudou de modelo: agora separamos os **dados do produto** (nome, marca,
localização, quantidade mínima, custo unitário, markup, foto) do **histórico de compras**
(data, fornecedor, quantidade, custo unitário, custo total, número da nota fiscal).

- **Cadastrar produto novo**: preenche só os dados básicos. O saldo em estoque começa em zero.
- **Lançar entrada de estoque**: abra o produto (clique em "Abrir" na lista) → botão **"Adicionar
  Compra"** → preencha data, fornecedor, quantidade, custo unitário e NF → confirme. O saldo do
  produto aumenta automaticamente, e essa compra fica registrada para sempre no histórico (não
  pode ser editada depois, para manter a conferência confiável).
- O **custo unitário** do produto (usado para calcular o preço de venda) é um campo manual,
  independente do histórico de compras — você decide quando atualizá-lo.

**Quantidade de material por serviço:** cada material adicionado numa tarefa agora tem um campo
de quantidade (×N). O custo total daquele item é `custo unitário × quantidade`, então não é mais
necessário adicionar o mesmo material várias vezes quando ele é usado em quantidade maior que 1.



Cada veículo, na aba Clientes/OS, tem um botão **PDF** (âmbar) que gera um orçamento
profissional para download, pronto para enviar ao cliente. O PDF inclui:

- Dados da empresa (nome, endereço, telefone, CNPJ) — configuráveis no ícone ⚙️ no topo
- Dados do cliente e do veículo (modelo, placa, mecânico responsável)
- Tabela detalhada com cada serviço e seus materiais/peças associados
- **Total de Peças/Materiais** e **Total de Mão de Obra** somados separadamente
- Total geral em destaque

O PDF é gerado inteiramente no navegador (não depende de nenhum servidor extra) usando a
biblioteca jsPDF, carregada automaticamente na primeira vez que alguém clica em "PDF".

**Antes de usar, configure os dados da empresa:** ícone ⚙️ no topo → preencha nome, endereço,
telefone e CNPJ → Salvar. Esses dados ficam salvos no banco e aparecem em todos os PDFs gerados
depois.
