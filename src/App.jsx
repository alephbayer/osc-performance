...
const THEMES = {
  ...
  light: {
    black:"#f0f2f5", gray900:"#ffffff", gray800:"#f8f9fa", gray700:"#e9ecef",
    gray600:"#ced4da", gray500:"#868e96", gray400:"#495057", gray200:"#212529",
    white:"#0a0a0a",
  },
};
...
function getTheme() {
  try {
    const stored = localStorage.getItem("osc_theme") || "auto";
...
function getThemePref() {
  try { return localStorage.getItem("osc_theme") || "auto"; } catch { return "auto"; }
}

function setThemeStorage(t) {
  try { localStorage.setItem("osc_theme", t); } catch {}
}

const B = {
  get black()  { return THEMES[getTheme()].black;   },
  get gray900(){ return THEMES[getTheme()].gray900;  },
  get gray800(){ return THEMES[getTheme()].gray800;  },
  get gray700(){ return THEMES[getTheme()].gray700;  },
  get gray600(){ return THEMES[getTheme()].gray600;  },
  get gray500(){ return THEMES[getTheme()].gray500;  },
  get gray400(){ return THEMES[getTheme()].gray400;  },
  get gray200(){ return THEMES[getTheme()].gray200;  },
  get white()  { return THEMES[getTheme()].white;    },
  ...
  return (<div style={{minHeight:"100vh",background:B.black,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter','Segoe UI',sans-serif",padding:20}}>
    <div style={{maxWidth:380,width:"100%"}}>
      <div style={{textAlign:"center",marginBottom:28}}>
        <div style={{width:56,height:56,borderRadius:14,background:B.blue,display:"flex",alignItems:"center",justifyContent:"center",margin:"0 auto 14px"}}><ICar s={28} c={B.white}/></div>
...
// ─── Collapsible Vehicle Group (for client portal tabs) ──────────────────────
function CollapsibleVehicleGroup({vehicle,count,children}) {
  const [open,setOpen]=useState(false);
  return (<div style={{marginBottom:12}}>
    <button onClick={()=>setOpen(o=>!o)} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"11px 14px",background:B.gray800,border:`1px solid ${B.gray700}`,borderRadius:open?"12px 12px 0 0":12,cursor:"pointer",textAlign:"left"}}>
      {vehicle.photo
        ?<img src={vehicle.photo} alt="" style={{width:36,height:36,borderRadius:8,objectFit:"cover",flexShrink:0,border:`1px solid ${B.gray600}`}}/>
        :<div style={{width:36,height:36,borderRadius:8,background:B.gray700,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><ICar s={18} c={B.blue}/></div>}
      <div style={{flex:1,minWidth:0}}>
        <div style={{fontWeight:800,fontSize:14,color:B.white,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis"}}>{vehicle.model}</div>
        <div style={{fontSize:11,color:B.gray400}}>{vehicle.plate}{vehicle.color?` · ${vehicle.color}`:""}</div>
      </div>
      {count&&<span style={{fontSize:10,color:B.gray500,flexShrink:0}}>{count}</span>}
      <span style={{color:B.gray400,fontSize:12,flexShrink:0,marginLeft:4}}>{open?"▲":"▼"}</span>
    </button>
    {open&&<div style={{border:`1px solid ${B.gray700}`,borderTop:"none",borderRadius:"0 0 12px 12px",overflow:"hidden"}}>
      {children}
    </div>}
  </div>);
}
...
// ─── Client Notes Tab ─────────────────────────────────────────────────────────
function ClientNotesTab({client, vehicles}) {
  ...
  useEffect(()=>{
    ...
    setLoading(true);
    db.getClientVehicleNotes(selectedVehicle)
  ...
  const addNote=async()=>{
    ...
    const row=await db.addClientVehicleNote(selectedVehicle,client.id,newNote.trim());
    setNotes(p=>[row,...p]);
    setNewNote("");
  };

  const deleteNote=async(id)=>{
    await db.deleteClientVehicleNote(id);
  ...
  return (<>
    ...
    {/* Vehicle selector */}
    {vehicles.length>1&&<div style={{marginBottom:14,display:"flex",gap:6,flexWrap:"wrap"}}>
      {vehicles.map(v=>(
        <button key={v.id} onClick={()=>setSelectedVehicle(v.id)}
          style={{padding:"7px 14px",borderRadius:8,border:`1px solid ${selectedVehicle===v.id?blue:B.gray600}`,background:selectedVehicle===v.id?`${blue}18`:"none",color:selectedVehicle===v.id?blue:B.gray400,fontWeight:700,fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",gap:6}}>
          {v.photo&&<img src={v.photo} alt="" style={{width:18,height:18,borderRadius:4,objectFit:"cover"}}/>}
          {v.model}
        </button>
      ))}
    </div>}
    ...
    {/* Add note */}
    <div style={{background:B.gray800,borderRadius:12,padding:14,border:`1px solid ${B.gray700}`,marginBottom:16}}>
      <textarea value={newNote} onChange={e=>setNewNote(e.target.value)}
        placeholder="Ex: Barulho estranho ao frear, Ar condicionado não gela, Luz do painel acesa..."
        rows={3} style={{width:"100%",padding:"10px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none",resize:"none",boxSizing:"border-box",fontFamily:"inherit",lineHeight:1.5}}/>
      <button onClick={addNote} disabled={!newNote.trim()}
        style={{marginTop:8,width:"100%",padding:"10px 0",borderRadius:9,background:newNote.trim()?blue:B.gray700,border:"none",color:B.white,fontWeight:800,fontSize:13,cursor:newNote.trim()?"pointer":"not-allowed"}}>
        Adicionar anotação
      </button>
    </div>
    ...
    :notes.map(n=>(
      <div key={n.id} style={{background:B.gray800,borderRadius:10,padding:"12px 14px",marginBottom:8,border:`1px solid ${n.resolved?B.green+"44":B.gray700}`,display:"flex",alignItems:"flex-start",gap:10}}>
        <div style={{flex:1}}>
        ...
        {!n.resolved&&<button onClick={()=>deleteNote(n.id)}
          style={{background:"none",border:"none",cursor:"pointer",color:B.gray500,padding:4,flexShrink:0}}
          onMouseEnter={e=>e.currentTarget.style.color=B.red}
          onMouseLeave={e=>e.currentTarget.style.color=B.gray500}>
          <ITrash s={14}/>
        </button>}
      </div>
    ))}
...
function ClientPortal({client,vehicles,tasks,employees,payments,osHistory,defaultRate,onLogout}) {
  ...
  const requestPush=async()=>{
    ...
    await db.saveClientPushSubscription(client.id,sub);
    setPushStatus("granted");
  }; // active | history | account
  const blue=B.blue;
  ...
  const tabBtn=(key,label,icon)=>(
    <button onClick={()=>setTab(key)} style={{flex:1,padding:"8px 4px 6px",borderRadius:9,border:"none",cursor:"pointer",fontWeight:700,
      background:tab===key?blue:"transparent",color:tab===key?B.white:B.gray400,
      display:"flex",flexDirection:"column",alignItems:"center",gap:3,minWidth:0}}>
      <span style={{display:"flex",alignItems:"center",justifyContent:"center",opacity:tab===key?1:.7}}>{icon}</span>
      <span style={{fontSize:10,lineHeight:1.2,textAlign:"center",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:"100%"}}>{label}</span>
    </button>
  );
  ...
  return (<div style={{height:"100%",display:"flex",flexDirection:"column",background:B.black,fontFamily:"'Inter','Segoe UI',sans-serif",color:B.white}}>
    ...
    <div style={{maxWidth:680,margin:"0 auto",padding:"20px 14px",paddingBottom:"calc(40px + env(safe-area-inset-bottom))",minHeight:"100vh"}}>
      {pushStatus==="asking"&&VAPID_PUBLIC_KEY&&<div style={{marginBottom:14,padding:"10px 14px",background:`${B.blue}12`,border:`1px solid ${B.blue}33`,borderRadius:10,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
        ...
        <button onClick={requestPush} style={{padding:"6px 14px",borderRadius:7,background:B.blue,border:"none",color:B.white,fontWeight:700,fontSize:11,cursor:"pointer",flexShrink:0}}>Ativar</button>
        <button onClick={()=>setPushStatus("denied")} style={{padding:"6px 10px",borderRadius:7,background:"none",border:`1px solid ${B.gray600}`,color:B.gray400,fontSize:11,cursor:"pointer",flexShrink:0}}>Agora não</button>
      </div>}
      ...
      {tab==="active"&&<>
        {activeVehicles.length===0
          ...
          :activeVehicles.map(v=>{
            ...
            const perfTs=vts.filter(t=>(t.division||"performance")==="performance");
            const finTs=vts.filter(t=>t.division==="finishing");
            const hasPerf=!!v.enteredAt||perfTs.length>0;
            const hasFin=!!v.enteredAtFinishing||finTs.length>0;
            const done=vts.filter(t=>t.done).length;
            const countLabel=`${done}/${vts.length} concluído${vts.length!==1?"s":""}`;
            return (<CollapsibleVehicleGroup key={v.id} vehicle={v} count={countLabel}>
              {/* OS number badges */}
              <div style={{padding:"10px 14px",borderBottom:`1px solid ${B.gray700}`,display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
                {hasPerf&&v.osNumber&&<span style={{fontSize:10,fontWeight:800,color:B.orange,background:`${B.orange}18`,border:`1px solid ${B.orange}33`,borderRadius:6,padding:"2px 8px"}}>{fmtOS(v.osNumber)}</span>}
                {hasFin&&v.osNumberFinishing&&<span style={{fontSize:10,fontWeight:800,color:FD.primary,background:FD.bg,border:`1px solid ${FD.border}`,borderRadius:6,padding:"2px 8px"}}>{fmtOSFD(v.osNumberFinishing)}</span>}
                <a href={getPublicLink(v.id)} target="_blank" rel="noreferrer"
                  style={{marginLeft:"auto",display:"flex",alignItems:"center",gap:5,padding:"5px 12px",borderRadius:7,background:B.blue,color:B.white,fontWeight:700,fontSize:11,textDecoration:"none"}}>
              ...
              {hasPerf&&perfTs.length>0&&<div style={{padding:"12px 16px",borderBottom:hasFin?`1px solid ${B.gray700}`:"none"}}>
                ...
                <div style={{marginTop:8,fontSize:12,color:B.gray400}}>{perfTs.filter(t=>t.done).length}/{perfTs.length} concluídos</div>
              </div>}
              ...
              {hasFin&&finTs.length>0&&<div style={{padding:"12px 16px"}}>
                ...
                <div style={{marginTop:8,fontSize:12,color:B.gray400}}>{finTs.filter(t=>t.done).length}/{finTs.length} concluídos</div>
              </div>}

              {/* No tasks yet */}
              {vts.length===0&&<div style={{padding:"16px",fontSize:13,color:B.gray500,textAlign:"center"}}>Serviços ainda não lançados</div>}
            </CollapsibleVehicleGroup>);
          })}
      </>}
      ...
      {tab==="history"&&<>
        {cliHistory.length===0
          ...
          :cliVehicles.map(v=>{
            const vHist=cliHistory.filter(h=>(h.vehicle_id||h.vehicleId)===v.id);
            if(vHist.length===0) return null;
            return (<CollapsibleVehicleGroup key={v.id} vehicle={v} count={`${vHist.length} OS${vHist.length!==1?"s":""}`}>
              {vHist.map(h=>{
                ...
                const snap=h.tasks_snapshot||h.tasksSnapshot||[];
                const hPayments=payments.filter(p=>p.osHistoryId===h.id);
                const histLink=`${window.location.href.split("?")[0]}?vh=${v.id}`;
                return (<div key={h.id} style={{background:B.gray800,borderRadius:14,marginBottom:10,border:`1px solid ${B.gray700}`,overflow:"hidden"}}>
                  <div style={{padding:"12px 16px",borderBottom:`1px solid ${B.gray700}`,display:"flex",alignItems:"center",gap:10}}>
                    <div style={{flex:1}}>
                      <div style={{fontWeight:700,fontSize:13,color:B.white,display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                        {osLabel}
                        <span style={{fontSize:10,fontWeight:700,color:divColor,background:`${divColor}18`,border:`1px solid ${divColor}33`,borderRadius:5,padding:"1px 6px"}}>{divLabel}</span>
                      </div>
                      <div style={{fontSize:11,color:B.gray500,marginTop:2}}>
                        Entregue em {deliveredAt?new Date(deliveredAt).toLocaleDateString("pt-BR"):"—"}
                      </div>
                    </div>
                    <div style={{textAlign:"right"}}>
                      <div style={{fontSize:13,fontWeight:800,color:B.white}}>{fmtBRL(totalValue)}</div>
                      <div style={{fontSize:10,color:B.gray500}}>{snap.length} serviço{snap.length!==1?"s":""}</div>
                    </div>
                  </div>
      ...
      {tab==="account"&&<>
        ...
        {cliVehicles.map(v=>{
          ...
          const hasActivity=v.enteredAt||v.enteredAtFinishing||vPayments.length>0||vHistory.length>0;
          ...
          const perfTotal=vehicleTotal(v.id,perfTasks,defaultRate,v);
          const finLabor=finTasks.reduce((s,t)=>s+taskCost(t,defaultRate).labor,0);
          const finMat=finTasks.reduce((s,t)=>s+taskCost(t,defaultRate).mat,0);
          const finDisc=finLabor*Number(v.osDiscountPctFinishing||0)/100;
          const finTotal=finLabor+finMat-finDisc;
          const totalDue=(v.enteredAt?perfTotal:0)+(v.enteredAtFinishing?finTotal:0);
          const totalPaid=vPayments.reduce((s,p)=>s+Number(p.amount),0);
          const balance=totalDue-totalPaid;
          const hasActiveOS=!!(v.enteredAt||v.enteredAtFinishing);
          const totalPaidAll=[...vPayments,...payments.filter(p=>p.vehicleId===v.id&&p.osHistoryId)].reduce((s,p)=>s+Number(p.amount),0);
          const countLabel=hasActiveOS?`Em serviço · ${fmtBRL(totalPaidAll)} pago`:`${vHistory.length} OS${vHistory.length!==1?"s":""}`;
          return (<CollapsibleVehicleGroup key={v.id} vehicle={v} count={countLabel}>
            <div style={{background:B.gray800}}>
            {hasActiveOS&&<div style={{display:"flex",gap:1,borderBottom:`1px solid ${B.gray700}`}}>
              <div style={{flex:1,padding:"12px 16px",textAlign:"center"}}>
                <div style={{fontSize:10,color:B.gray400,marginBottom:2}}>Total</div>
                <div style={{fontSize:15,fontWeight:800,color:B.white}}>{fmtBRL(totalDue)}</div>
              </div>
              <div style={{flex:1,padding:"12px 16px",textAlign:"center",borderLeft:`1px solid ${B.gray700}`}}>
                <div style={{fontSize:10,color:B.green,marginBottom:2}}>Pago</div>
                <div style={{fontSize:15,fontWeight:800,color:B.green}}>{fmtBRL(totalPaid)}</div>
              </div>
              <div style={{flex:1,padding:"12px 16px",textAlign:"center",borderLeft:`1px solid ${B.gray700}`}}>
                <div style={{fontSize:10,color:balance>0?B.red:B.green,marginBottom:2}}>{balance>0?"Saldo":"Quitado"}</div>
                <div style={{fontSize:15,fontWeight:800,color:balance>0?B.red:B.green}}>{balance>0?fmtBRL(balance):"✓"}</div>
              </div>
            </div>}
            {hasActiveOS&&vPayments.length>0&&<div style={{padding:"10px 16px",borderBottom:vHistory.length>0?`1px solid ${B.gray700}`:"none"}}>
              <div style={{fontSize:10,color:B.gray400,fontWeight:700,marginBottom:6,textTransform:"uppercase",letterSpacing:.5}}>Pagamentos — OS atual</div>
              {vPayments.map((p,i)=>(
            ...
            {vHistory.length>0&&<div style={{padding:"10px 16px"}}>
              <div style={{fontSize:10,color:B.gray400,fontWeight:700,marginBottom:8,textTransform:"uppercase",letterSpacing:.5}}>OSs anteriores</div>
              {vHistory.map(h=>{
                const div=h.division||"performance";
                const osNum=h.os_number||h.osNumber;
                const osLabel=div==="finishing"?fmtOSFD(osNum):fmtOS(osNum);
...
function getClientPortalLink() {
  return `${window.location.href.split("?")[0]}?portal=cliente`;
}
...
const CATEGORIES=[
  ...
  {id:"Alimentação",    color:"#f97316"},
  {id:"Ar Condicionado",color:"#06b6d4"},
  {id:"Arrefecimento",  color:"#0ea5e9"},
  {id:"Chassi",         color:"#78716c"},
  {id:"Direção",        color:"#10b981"},
  {id:"Elétrica",       color:"#eab308"},
  {id:"Eletrônica",     color:"#6366f1"},
  {id:"Escapamento",    color:"#a3a3a3"},
  {id:"Exterior",       color:"#38bdf8"},
...
// Finishing Division categories
const CATEGORIES_FINISHING = [
  {id:"Acabamento / Finalização", color:"#14b8a6"},
  {id:"Funilaria / Lanternagem",  color:"#f97316"},
  {id:"Pintura",                  color:"#a855f7"},
];
const CAT_MAP_FINISHING = Object.fromEntries(CATEGORIES_FINISHING.map(c=>[c.id,c.color]));

// Finishing Division brand colors
const FD = {
  primary:  "#a855f7",  // purple
  bg:       "#a855f715",
  border:   "#a855f744",
  label:    "OSC Finishing Division",
  short:    "Finishing",
  prefix:   "FD",
};
...
const CHECKLIST_ITEMS = [
  // Basic
  {id:"limpeza",    label:"Condição de limpeza do veículo",          taskLabel:"Higienização do veículo",                   category:"Exterior",      level:"basic"},
  {id:"oleo",       label:"Nível/Qualidade de óleo do motor",        taskLabel:"Substituição do óleo do motor",             category:"Motor",         level:"basic"},
  {id:"oleo_dif",   label:"Óleo do diferencial",                     taskLabel:"Substituição do óleo do diferencial",       category:"Powertrain",    level:"basic"},
  {id:"oleo_cambio",label:"Óleo da caixa de marcha",                 taskLabel:"Substituição do óleo da caixa de marcha",   category:"Powertrain",    level:"basic"},
  {id:"fluido_fre", label:"Nível do fluido de freio",                taskLabel:"Substituição do fluido de freio",           category:"Freio",         level:"basic"},
  {id:"fluido_dir", label:"Nível do fluido de direção hidráulica",   taskLabel:"Substituição do fluido de direção",         category:"Direção",       level:"basic"},
  {id:"arrefecim",  label:"Nível do fluido de arrefecimento",        taskLabel:"Substituição do fluido de arrefecimento",   category:"Arrefecimento", level:"basic"},
  {id:"past_diant", label:"Pastilhas de freio dianteiras",           taskLabel:"Substituição das pastilhas dianteiras",     category:"Freio",         level:"basic"},
  {id:"past_tras",  label:"Pastilhas / lonas de freio traseiras",    taskLabel:"Substituição das lonas/pastilhas traseiras",category:"Freio",         level:"basic"},
  {id:"discos",     label:"Discos / tambores de freio",              taskLabel:"Substituição dos discos/tambores de freio", category:"Freio",         level:"basic"},
  {id:"pneus",      label:"Desgaste dos pneus",                      taskLabel:"Substituição dos pneus",                   category:"Chassi",        level:"basic"},
  {id:"injecao",    label:"Luz de injeção / check engine",           taskLabel:"Diagnóstico eletrônico — check engine",     category:"Eletrônica",    level:"basic"},
  {id:"luzes",      label:"Luzes (faróis, lanternas, setas)",        taskLabel:"Reparo do sistema de iluminação",          category:"Elétrica",      level:"basic"},
  {id:"bateria",    label:"Estado da bateria",                       taskLabel:"Substituição da bateria",                  category:"Elétrica",      level:"basic"},
  {id:"filtro_ar",  label:"Filtro de ar",                            taskLabel:"Substituição do filtro de ar",             category:"Motor",         level:"basic"},
  // Advanced — structural
  {id:"bucha_diant",  label:"Buchas de bandeja dianteiras",            taskLabel:"Substituição das buchas dianteiras",           category:"Suspensão",     level:"advanced"},
  {id:"bucha_tras",   label:"Buchas de bandeja traseiras",             taskLabel:"Substituição das buchas traseiras",             category:"Suspensão",     level:"advanced"},
  {id:"batente_diant",label:"Batentes de suspensão dianteiros",        taskLabel:"Substituição dos batentes dianteiros",          category:"Suspensão",     level:"advanced"},
  {id:"batente_tras", label:"Batentes de suspensão traseiros",         taskLabel:"Substituição dos batentes traseiros",           category:"Suspensão",     level:"advanced"},
  {id:"coifa_homo",   label:"Coifas de homocinética",                  taskLabel:"Substituição das coifas de homocinética",       category:"Suspensão",     level:"advanced"},
  {id:"coxim_susp",   label:"Coxins de suspensão (strut mount)",       taskLabel:"Substituição dos coxins de suspensão",          category:"Suspensão",     level:"advanced"},
  {id:"terminal",     label:"Terminal de direção",                     taskLabel:"Substituição do terminal de direção",           category:"Direção",       level:"advanced"},
  {id:"coifa_dir",    label:"Coifa de direção (cremalheira)",          taskLabel:"Substituição da coifa de direção",              category:"Direção",       level:"advanced"},
  {id:"barra_est",    label:"Barra estabilizadora e buchas",           taskLabel:"Substituição das buchas da barra estabilizadora",category:"Suspensão",   level:"advanced"},
  {id:"amor_diant",   label:"Amortecedores dianteiros",                taskLabel:"Substituição dos amortecedores dianteiros",     category:"Suspensão",     level:"advanced"},
  {id:"amor_tras",    label:"Amortecedores traseiros",                 taskLabel:"Substituição dos amortecedores traseiros",      category:"Suspensão",     level:"advanced"},
  {id:"rolamento",    label:"Rolamento de roda",                       taskLabel:"Substituição do rolamento de roda",             category:"Suspensão",     level:"advanced"},
  {id:"correia",      label:"Correia dentada / correia acessórios",    taskLabel:"Substituição da correia dentada",               category:"Motor",         level:"advanced"},
  {id:"escapam",      label:"Escapamento (vazamentos e ruídos)",       taskLabel:"Reparo do sistema de escapamento",              category:"Escapamento",   level:"advanced"},
  {id:"coxim",        label:"Coxim do motor",                          taskLabel:"Substituição do coxim do motor",                category:"Motor",         level:"advanced"},
  {id:"transmissao",  label:"Transmissão / homocinética",              taskLabel:"Reparo da transmissão / homocinética",          category:"Powertrain",    level:"advanced"},
  // Advanced — leaks
  {id:"vaz_motor",  label:"Vazamento de óleo — motor",              taskLabel:"Reparo de vazamento de óleo do motor",     category:"Motor",         level:"advanced"},
  {id:"vaz_cambio", label:"Vazamento de óleo — câmbio",             taskLabel:"Reparo de vazamento de óleo do câmbio",    category:"Powertrain",    level:"advanced"},
  {id:"vaz_dif",    label:"Vazamento de óleo — diferencial",        taskLabel:"Reparo de vazamento do diferencial",       category:"Powertrain",    level:"advanced"},
  {id:"vaz_fre",    label:"Vazamento de fluido de freio",           taskLabel:"Reparo de vazamento do fluido de freio",   category:"Freio",         level:"advanced"},
  {id:"vaz_dir",    label:"Vazamento de fluido de direção",         taskLabel:"Reparo de vazamento do fluido de direção", category:"Direção",       level:"advanced"},
  {id:"vaz_arref",  label:"Vazamento de fluido de arrefecimento",   taskLabel:"Reparo de vazamento do fluido de arrefecimento",category:"Arrefecimento",level:"advanced"},
];
...
async function generateQuotePDF(vehicle, tasks, client, employee, company, defaultRate, tasksOverride, payments=[]) {
  ...
  if (vehicle.notes && vehicle.notes.trim()) {
    ...
    const noteLines = doc.splitTextToSize(vehicle.notes.trim(), contentW - 6);
    noteLines.forEach(noteLine => { if (y + 6 > 282) { doc.addPage(); y = 16; } doc.text(noteLine, marginX + 3, y); y += 5.5; });
    y += 10;
  }

  // ── Checklist ────────────────────────────────────────────────────────────────
  const cl = Array.isArray(vehicle.checklist) ? vehicle.checklist : [];
  const clAnswered = cl.filter(c => c.status);
  if (clAnswered.length > 0) {
    if (y + 16 > 282) { doc.addPage(); y = 16; }
    doc.setFillColor(235, 240, 255);
    doc.setDrawColor(80, 100, 200); doc.setLineWidth(0.3);
    doc.rect(marginX, y, contentW, 7, "F");
    doc.setFont("helvetica","bold"); doc.setFontSize(8); doc.setTextColor(50, 70, 180);
    ...
    clAnswered.forEach((state, idx) => {
      ...
      const statusLabel = isOk ? "Bom" : "Ruim";
      ...
      const labelLines = doc.splitTextToSize(item.label, colW - 18);
      doc.text(labelLines[0], x + 2, y);

      // Status on the right in color
      doc.setFont("helvetica","bold"); doc.setFontSize(7.5); doc.setTextColor(...statusColor);
      doc.text(statusLabel, x + colW - 2, y, { align: "right" });
    });
    y += 14;
  }
  ...
  catOrder.forEach(cat=>{
    ...
    const matTextLines = mats.map(m => {
      ...
      const brandTag = m.brand ? ` · ${m.brand}` : "";
  ...
  // Legacy single fuelCost + new fuels array
  const allFuels = [...(fuelCostVal>0?[{type:"Combustivel",date:"",value:fuelCostVal}]:[]), ...fuels.filter(f=>f.value>0)];
  allFuels.forEach(fuel=>{
    ...
    doc.setFont("helvetica","normal"); doc.setFontSize(8); doc.setTextColor(...gray);
    const fuelLabel = [fuel.type||"Combustivel", fuel.date||""].filter(Boolean).join(" · ");
    doc.text(fuelLabel, marginX + 3, y + 5.5);
    doc.text("—", cDisc, y + 5.5, { align: "right" });
    doc.setFont("helvetica","bold"); doc.setTextColor(...black);
    doc.text(fmtBRL(Number(fuel.value||0)), cTotal, y + 5.5, { align: "right" });
    y += 9;
  });
...
function PhotoGallery({photos=[],onAdd,onAddMany,onRemove,onUpdate,readOnly=false,maxH=140,tasks=[]}) {
  ...
  return (<>
    {confirmRemove!==null&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.75)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:200,padding:20}} onClick={()=>setConfirmRemove(null)}>
      <div style={{background:B.gray800,borderRadius:14,padding:24,maxWidth:320,width:"100%",border:`1px solid ${B.red}44`}} onClick={e=>e.stopPropagation()}>
        <div style={{fontSize:15,fontWeight:800,color:B.white,marginBottom:8}}>Excluir foto?</div>
        <div style={{fontSize:13,color:B.gray400,marginBottom:20}}>Esta ação não pode ser desfeita.</div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={()=>setConfirmRemove(null)} style={{flex:1,padding:"10px 0",borderRadius:9,background:B.gray700,border:`1px solid ${B.gray600}`,color:B.gray200,fontWeight:700,fontSize:13,cursor:"pointer"}}>Cancelar</button>
          <button onClick={()=>{onRemove(confirmRemove);setConfirmRemove(null);}} style={{flex:1,padding:"10px 0",borderRadius:9,background:B.red,border:"none",color:B.white,fontWeight:800,fontSize:13,cursor:"pointer"}}>Excluir</button>
        </div>
      </div>
    </div>}
      ...
      {normalized.map((p,i)=>{
        ...
        return (<div key={i} style={{width:maxH,flexShrink:0}}>
          <div style={{position:"relative",width:maxH,height:maxH,borderRadius:8,overflow:"hidden",border:`1px solid ${editIdx===i?B.purple:B.gray600}`,cursor:"pointer"}} onClick={()=>{setLB(p);setEditIdx(null);}}>
            ...
            {!readOnly&&<button onClick={e=>{e.stopPropagation();setConfirmRemove(i);}} style={{position:"absolute",top:4,right:4,background:"rgba(0,0,0,.7)",border:"none",borderRadius:99,width:22,height:22,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer"}}>
              <IX s={12} c={B.white}/>
            </button>}
            {/* Badges */}
...
// ─── Category components ──────────────────────────────────────────────────────
function CategoryPill({category,size=10,isFD=false}) {
  if(!category) return null;
  const color=isFD?(CAT_MAP_FINISHING[category]||CAT_MAP[category]||B.gray500):(CAT_MAP[category]||CAT_MAP_FINISHING[category]||B.gray500);
...
function CategorySelect({value,onChange,categories=CATEGORIES}) {
  ...
  const allCatMap={...CAT_MAP,...CAT_MAP_FINISHING};
  const color=value?allCatMap[value]||B.gray500:B.gray500;
...
function OpenOSModal({vehicle,employees,onConfirm,onClose,title="Abrir nova OS",color,optional=false}) {
  const accentColor = color || B.green;
  const [selectedEmp,setSelectedEmp]=useState(null);
  const [entryInput,setEntryInput]=useState(new Date().toISOString().slice(0,16));
  const canConfirm = optional || !!selectedEmp;
  return (<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:90,padding:16}} onClick={onClose}>
    <div style={{background:B.gray800,borderRadius:16,maxWidth:420,width:"100%",overflow:"hidden",border:`1px solid ${accentColor}55`,maxHeight:"80vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
      <div style={{padding:"14px 18px",background:B.gray900,borderBottom:`2px solid ${accentColor}`,display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        <div style={{width:34,height:34,borderRadius:8,background:`${accentColor}18`,display:"flex",alignItems:"center",justifyContent:"center"}}><IFileText s={16} c={accentColor}/></div>
        <div>
          <div style={{fontWeight:700,fontSize:14,color:B.white}}>{title}</div>
      ...
      <div style={{padding:16,overflowY:"auto",flex:1}}>
        ...
        <FieldLabel>{optional?"Mecânico responsável (opcional)":"Selecione o mecânico responsável"}</FieldLabel>
        <div style={{display:"flex",flexDirection:"column",gap:6,marginTop:6}}>
          {employees.map(emp=>(
            <button key={emp.id} onClick={()=>setSelectedEmp(p=>p===emp.id?null:emp.id)}
              style={{textAlign:"left",padding:"10px 12px",borderRadius:9,background:selectedEmp===emp.id?`${accentColor}22`:B.gray700,border:`1px solid ${selectedEmp===emp.id?accentColor:B.gray600}`,color:B.white,cursor:"pointer",display:"flex",alignItems:"center",gap:9}}>
              <div style={{width:30,height:30,borderRadius:7,background:`${accentColor}22`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}><IWrench s={14} c={accentColor}/></div>
              <div>
                <div style={{fontWeight:700,fontSize:13}}>{emp.name}</div>
                {emp.phone&&<div style={{fontSize:11,color:B.gray400}}>{emp.phone}</div>}
              </div>
              {selectedEmp===emp.id&&<span style={{marginLeft:"auto",color:accentColor,fontWeight:700}}>✓</span>}
            </button>
          ))}
          {employees.length===0&&<div style={{fontSize:12,color:B.gray500,padding:"8px 0"}}>Nenhum mecânico da Finishing cadastrado.</div>}
        </div>
      </div>
      <div style={{padding:12,borderTop:`1px solid ${B.gray700}`,flexShrink:0}}>
        <button onClick={()=>canConfirm&&onConfirm(selectedEmp,entryInput)} disabled={!canConfirm}
          style={{width:"100%",padding:"10px 0",borderRadius:9,background:canConfirm?accentColor:B.gray700,border:"none",color:B.white,cursor:canConfirm?"pointer":"not-allowed",fontWeight:800,fontSize:14}}>
          Abrir OS
        </button>
      </div>
    </div>
...
function MaterialChip({mat,idx,onUpdate,onRemove,showCost=false,readOnlyName=false,editableQty=false}) {
  ...
  return (<div style={{background:bg,border,borderRadius:8,padding:"7px 10px",width:"100%",boxSizing:"border-box"}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        {readOnlyName||mat.fromStock
          ...
          :<div style={{flex:1,minWidth:0}}>
              ...
              {mat.brand
                ?<span style={{fontSize:10,color:B.gray500,marginLeft:4}}>{mat.brand} <button onClick={()=>onUpdate(idx,{...mat,brand:""})} style={{background:"none",border:"none",cursor:"pointer",color:B.gray600,padding:0,fontSize:10}}>✕</button></span>
    ...
    {/* Toggle buttons — second row below name */}
    ...
    <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
      ...
      <div style={{flex:1,minWidth:0}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap",marginBottom:3}}>
          <CategorySelect value={task.category} onChange={cat=>onUpdate(task.id,{category:cat})} categories={task.division==="finishing"?CATEGORIES_FINISHING:CATEGORIES}/>
    ...
    <div style={{marginLeft:30,marginTop:6}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        ...
        <div style={{display:"flex",flexDirection:"column",gap:4,flex:"1 1 160px"}}>
          ...
          {newMat.trim()&&<input value={newBrand} onChange={e=>setNewBrand(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addMat()} placeholder="Marca (opcional)"
            style={{background:B.gray700,border:`1px solid ${B.gray600}`,borderRadius:6,padding:"3px 9px",outline:"none",color:B.gray300,fontSize:11,width:"100%",boxSizing:"border-box"}}/>}
        </div>
        {/* hours */}
...
function TaskItemManager({task,defaultRate,stock,onToggle,onDelete,onUpdate,onConsumeStock,onReturnStock,employees=[],isFD=false,purchaseOrders=[],allTasks=[],reservedParts=[],currentEmployee=null}) {
  ...
  // Received purchase orders not yet fully allocated and NOT reserved
  const receivedOrders=purchaseOrders.filter(o=>{
    if(o.vehicleId!==task.vehicleId||o.status!=="received") return false;
    // If already reserved in partsList, don't show here
    if(reservedParts.some(p=>p.purchaseOrderId===o.id)) return false;
    const allocated=allTasks.filter(t=>t.vehicleId===task.vehicleId).flatMap(t=>t.materials||[]).filter(m=>m.purchaseOrderId===o.id).reduce((s,m)=>s+(m.qty||1),0);
  ...
  const addFromPurchaseOrder=(o)=>{
    ...
    const newMats=[...mats,{name:o.partName,brand:o.notesAdmin||"",cost:o.costPrice||0,value:salePrice,qty:o.quantity,fromStock:false,stockItemId:null,purchaseOrderId:o.id}];
    ...
    {showPOP&&hasPickable&&<div style={{margin:"4px 0 8px 30px",background:`${B.amber}10`,border:`1px solid ${B.amber}33`,borderRadius:9,padding:"8px 12px"}}>
      ...
      {receivedOrders.map(o=>{
        ...
        return (<div key={o.id} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:`1px solid ${B.amber}22`}}>
          {o.photoUrl&&<img src={o.photoUrl} alt="" style={{width:32,height:32,borderRadius:6,objectFit:"cover",flexShrink:0}}/>}
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:12,fontWeight:700,color:B.white}}>{o.partName}{o.notesAdmin?<span style={{color:B.gray400,fontWeight:400}}> — {o.notesAdmin}</span>:""}</div>
            <div style={{fontSize:10,color:B.gray400}}>Pedido · Qtd: {o.quantity}{salePrice?` · ${fmtBRL(salePrice)}`:""}</div>
          </div>
          <button onClick={()=>addFromPurchaseOrder(o)} style={{padding:"5px 12px",borderRadius:7,background:B.amber,border:"none",color:B.black,fontWeight:800,fontSize:11,cursor:"pointer",flexShrink:0}}>+ Usar</button>
        </div>);
      })}
      {availableReserved.map((p,i)=>(
        <div key={i} style={{display:"flex",alignItems:"center",gap:8,padding:"6px 0",borderBottom:`1px solid ${B.purple}22`}}>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:12,fontWeight:700,color:B.white}}>{p.name}{p.brand?<span style={{color:B.gray400,fontWeight:400}}> — {p.brand}</span>:""}</div>
            <div style={{fontSize:10,color:B.purple}}>📦 Reservado{p.location?` · ${p.location}`:""} · Qtd: {p.qty||1}{p.value?` · ${fmtBRL(p.value)}`:""}</div>
          </div>
          <button onClick={()=>addFromReserved(p)} style={{padding:"5px 12px",borderRadius:7,background:B.purple,border:"none",color:B.white,fontWeight:800,fontSize:11,cursor:"pointer",flexShrink:0}}>+ Usar</button>
        </div>
      ))}
    </div>}
...
function VehicleCard({vehicle,tasks,employees,clients,stock,defaultRate,managerMode,onAddTask,onToggleTask,onDeleteTask,onUpdateTask,onDeleteVehicle,onTransferMechanic,onTransferOwner,onUpdateVehicle,onConsumeStock,onReturnStock,hideManagerButtons=false,payments=[],onAddPayment,onDeletePayment,onUpdatePayment,company,onAddMechanic,onRemoveMechanic,onSetStatus,onDeliver,onDeliverFinishing,isOwner=false,division="performance",onAddPurchaseOrder,purchaseOrders=[],onOpenOS=null,currentMechanic=null}) {
  ...
  useEffect(()=>{
    ...
    db.getClientVehicleNotes(vehicle.id).then(n=>{
      const pending=n.filter(x=>!x.resolved);
      setClientNotes(pending);
      if(pending.length>0) setShowClientNotes(true); // auto-open if there are notes
    }).catch(()=>{});
  },[vehicle.id]);
  const [open,setOpen] = useState(false);
  ...
  const [showChecklist,setSCL]=useState(false);
  ...
  const vts   = tasks.filter(t=>t.vehicleId===vehicle.id&&(isFD?(t.division==="finishing"):(t.division||"performance")==="performance"));
  const done  = vts.filter(t=>t.done).length;
  ...
  return (<>
    <div style={{background:B.gray800,borderRadius:10,border:`1px solid ${vehicle.urgent?B.red+"66":B.gray700}`,overflow:"hidden",marginBottom:10,boxShadow:vehicle.urgent?`0 0 0 1px ${B.red}44,0 2px 12px ${B.red}22`:"none"}}>
      ...
      {showChecklist&&<div style={{padding:"12px 16px",background:B.black,borderBottom:`1px solid ${B.gray700}`}}>
        <VehicleChecklist vehicle={vehicle} tasks={vts} onUpdateVehicle={onUpdateVehicle} managerMode={managerMode} isFD={isFD} onAddTask={async(label,category)=>{
          await onAddTask(vehicle.id, label, category, isFD?"finishing":"performance");
        }}/>
      </div>}
      {/* Parts list inline section */}
      {showParts&&<div style={{padding:"12px 16px",background:B.black,borderBottom:`1px solid ${B.gray700}`}}>
        <VehiclePartsList vehicle={vehicle} onUpdateVehicle={onUpdateVehicle} managerMode={managerMode} isFD={isFD} tasks={tasks}/>
      </div>}
      ...
      {showPhotos&&<div style={{padding:"12px 14px",background:B.black,borderBottom:`1px solid ${B.gray700}`}}>
        ...
        <PhotoGallery
          ...
          onRemove={i=>{
            const norm=photos.map(p=>typeof p==="string"?{url:p,taskId:null,caption:""}:p);
            onUpdateVehicle(vehicle.id,{[isFD?"photosFinishing":"photos"]:norm.filter((_,j)=>j!==i)});
          }}
      ...
      {open&&<div style={{padding:"12px 14px"}}>
        ...
        <div style={{marginBottom:10}}>
          ...
          <textarea
            ...
            onBlur={e=>{const field=isFD?"notesFinishing":"notes";if(e.target.value!==(isFD?(vehicle.notesFinishing||""):(vehicle.notes||"")))onUpdateVehicle(vehicle.id,{[field]:e.target.value});}}
            placeholder={isFD?"Observações da Finishing...":"Observações sobre o veículo ou a OS... (visível para mecânicos e gestores)"}
            rows={3}
        ...
        {(()=>{
          ...
          return catOrder.map(cat=>{
            ...
            const catColor=cat?(isFD?CAT_MAP_FINISHING[cat]||CAT_MAP[cat]:CAT_MAP[cat]||CAT_MAP_FINISHING[cat])||B.gray500:null;
          ...
          <div>
            <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
              ...
              <button onClick={()=>{const fuels=[...(vehicle.fuels||[]),{date:"",type:"",value:0}];onUpdateVehicle(vehicle.id,{fuels});}}
                ...
                <IPlus s={11} c={B.amber}/>Adicionar combustível
              </button>
            </div>
            {(vehicle.fuels||[]).map((fuel,fi)=>(
              <div key={fi} style={{display:"flex",flexDirection:"column",gap:5,background:B.gray800,border:`1px solid ${B.gray700}`,borderRadius:7,padding:"8px 10px",marginBottom:5}}>
                <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
                  <span style={{fontSize:10,color:B.amber,fontWeight:700,flexShrink:0}}>#{fi+1}</span>
                  <div style={{display:"flex",alignItems:"center",gap:4,flex:"1 1 120px"}}>
                    <span style={{fontSize:10,color:B.gray500,flexShrink:0}}>Combustível:</span>
                    <InlineEdit value={fuel.type||""} onSave={v=>{const fuels=[...(vehicle.fuels||[])];fuels[fi]={...fuels[fi],type:v};onUpdateVehicle(vehicle.id,{fuels});}} placeholder="Ex: Gasolina, Etanol…"/>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:4,flex:"1 1 100px"}}>
                    <span style={{fontSize:10,color:B.gray500,flexShrink:0}}>Data:</span>
                    <InlineEdit value={fuel.date||""} onSave={v=>{const fuels=[...(vehicle.fuels||[])];fuels[fi]={...fuels[fi],date:v};onUpdateVehicle(vehicle.id,{fuels});}} placeholder="DD/MM/AAAA"/>
                  </div>
                  <div style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}}>
                    <span style={{fontSize:10,color:B.gray500}}>R$</span>
                    <InlineEdit value={fuel.value?fmtR2(fuel.value):""} onSave={v=>{const fuels=[...(vehicle.fuels||[])];fuels[fi]={...fuels[fi],value:parseFloat(v.replace(",","."))||0};onUpdateVehicle(vehicle.id,{fuels});}} placeholder="0" type="number"/>
                  </div>
                  {fuel.value>0&&<span style={{fontSize:11,color:B.amber,fontWeight:700}}>{fmtBRL(fuel.value)}</span>}
                  <button onClick={()=>{const fuels=(vehicle.fuels||[]).filter((_,i)=>i!==fi);onUpdateVehicle(vehicle.id,{fuels});}}
...
function EmployeeCard({employee,vehicles,tasks,employees,clients,stock,defaultRate,onAddVehicle,onDeleteVehicle,onTransferMechanic,onTransferOwner,onAddTask,onToggleTask,onDeleteTask,onUpdateTask,onUpdateVehicle,onConsumeStock,onReturnStock,onDelete,onSendWA,onUpdatePhone,onUpdateName,onUpdateDivision,onUpdateSpecialty,onAddMechanic,onRemoveMechanic,onSetStatus,onDeliver,isOwner=false,purchaseOrders=[],onAddPurchaseOrder}) {
  ...
  const saveEdit=()=>{
    if(editName.trim()&&editName.trim()!==employee.name) onUpdateName(employee.id,editName.trim());
  ...
  return (<><div style={{background:B.gray800,borderRadius:14,border:`1px solid ${B.gray700}`,marginBottom:20,overflow:"hidden",boxShadow:"0 4px 20px rgba(0,0,0,.35)"}}>
    <div onClick={()=>setOpen(o=>!o)} style={{padding:"12px 16px",background:B.gray900,borderBottom:`2px solid ${B.orange}`,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap",cursor:"pointer",userSelect:"none"}}>
        ...
        {editing
          ?<div style={{display:"flex",flexDirection:"column",gap:6}} onClick={e=>e.stopPropagation()}>
            <input value={editName} onChange={e=>setEditName(e.target.value)} placeholder="Nome"
              style={{padding:"6px 10px",borderRadius:7,border:`1px solid ${B.orange}`,background:B.gray900,color:B.white,fontSize:13,outline:"none"}}/>
            <input value={editPhone} onChange={e=>setEditPhone(e.target.value)} placeholder="Telefone (ex: 5511999998888)"
              style={{padding:"6px 10px",borderRadius:7,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none"}}/>
            <div style={{display:"flex",gap:6}}>
              <button onClick={saveEdit} style={{padding:"5px 14px",borderRadius:7,background:B.orange,border:"none",color:B.white,fontWeight:700,fontSize:12,cursor:"pointer"}}>Salvar</button>
              <button onClick={()=>{setEditing(false);setEditName(employee.name);setEditPhone(employee.phone||"");}} style={{padding:"5px 10px",borderRadius:7,background:B.gray700,border:"none",color:B.gray200,fontSize:12,cursor:"pointer"}}>Cancelar</button>
            </div>
          </div>
          :<>
      ...
      <div style={{display:"flex",gap:5,flexShrink:0}} onClick={e=>e.stopPropagation()}>
        {!editing&&<button onClick={()=>{setEditing(true);setEditName(employee.name);setEditPhone(employee.phone||"");setOpen(true);}}
          title="Editar mecânico"
          style={{padding:"7px 9px",borderRadius:8,border:`1px solid ${B.gray600}`,cursor:"pointer",background:B.gray700,color:B.gray300,display:"flex",alignItems:"center"}}>
          <IEdit s={13}/>
        </button>}
...
function AccountModal({vehicle,tasks,payments,defaultRate,onAddPayment,onDeletePayment,onUpdatePayment,onClose}) {
  ...
  const hasFinishing=finTasks.length>0||!!vehicle.enteredAtFinishing;
  const hasPerformance=perfTasks.length>0||!!vehicle.enteredAt;
  ...
  const finPaid=vPayments.filter(p=>p.division==="finishing").reduce((s,p)=>s+Number(p.amount),0);
  const paid=perfPaid+finPaid;

  const perfTotal=vehicleTotal(vehicle.id,perfTasks,defaultRate,{...vehicle,fuels:vehicle.fuels,tows:vehicle.tows,osDiscountPct:vehicle.osDiscountPct});
  const finLabor=finTasks.reduce((s,t)=>s+taskCost(t,defaultRate).labor,0);
  const finMat=finTasks.reduce((s,t)=>s+taskCost(t,defaultRate).mat,0);
  const finDiscount=finLabor*Number(vehicle.osDiscountPctFinishing||0)/100;
  const finTotal=finLabor+finMat-finDiscount;
  const total=perfTotal+finTotal;
...
function VehicleHistoryCard({vehicle,tasks,employees,clients,defaultRate,onUpdateVehicle,now,osHistory=[],onOpenOS,onOpenOSFinishing,company,payments=[],onAddPayment,onDeletePayment,onUpdatePayment,isOwner=false,onDeleteOsHistory=null,onDeleteVehicle=null}) {
  ...
  useEffect(()=>{
    db.getClientVehicleNotes(vehicle.id).then(n=>{
      const pending=n.filter(x=>!x.resolved);
      setClientNotes(pending);
      if(pending.length>0) setShowClientNotes(true);
    }).catch(()=>{});
  },[vehicle.id]);
  ...
  const hasActiveFinishing = !!vehicle.enteredAtFinishing;
  ...
  return (<div style={{background:B.gray800,borderRadius:12,border:`1px solid ${B.gray700}`,overflow:"hidden"}}>
    ...
    {open&&<div style={{padding:"14px 16px"}}>
      ...
      {/* ── Active OS ── */}
      {(hasActiveOS||hasActiveFinishing)&&<>
      ...
      {/* No active OS — open buttons */}
      {!hasActiveOS&&!vehicle.enteredAtFinishing&&<div style={{textAlign:"center",padding:"16px 0",color:B.gray400}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:4}}>
        {onOpenOS&&!hasActiveOS&&<button onClick={()=>setShowOpenOS(true)} style={{padding:"7px 16px",borderRadius:9,background:B.greenBg,border:`1px solid ${B.green}44`,color:B.green,cursor:"pointer",fontWeight:700,fontSize:12,display:"inline-flex",alignItems:"center",gap:5}}>
          <IPlus s={13} c={B.green}/>⚙️ OS Performance
        </button>}
        {onOpenOSFinishing&&!vehicle.enteredAtFinishing&&<button onClick={()=>setShowOpenOSFD(true)} style={{padding:"7px 16px",borderRadius:9,background:FD.bg,border:`1px solid ${FD.border}`,color:FD.primary,cursor:"pointer",fontWeight:700,fontSize:12,display:"inline-flex",alignItems:"center",gap:5}}>
          <IPlus s={13} c={FD.primary}/>🎨 OS Finishing
        </button>}
      </div>
        ...
        {clientNotes.map(n=>(
          <div key={n.id} style={{display:"flex",alignItems:"flex-start",gap:10,padding:"8px 0",borderBottom:`1px solid ${B.amber}22`}}>
            ...
            <button onClick={async()=>{await db.resolveClientVehicleNote(n.id);setClientNotes(p=>p.filter(x=>x.id!==n.id));}}
              style={{padding:"4px 10px",borderRadius:6,background:B.greenBg,border:`1px solid ${B.green}44`,color:B.green,cursor:"pointer",fontSize:11,fontWeight:700,flexShrink:0}}>
              ✓ Recebido
            </button>
          </div>
        ))}
  ...
  return (<div style={{background:B.gray900,borderRadius:11,padding:14,border:`1px solid ${isMat?B.amber+"44":(form.category||cat)==="pesado"?B.red+"44":B.blue+"44"}`,marginBottom:14}}>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:8}}>
      ...
      <input value={form.link} onChange={e=>setForm(p=>({...p,link:e.target.value}))} placeholder="Link (opcional)"
        style={{padding:"9px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none"}}/>
      <div style={{display:"flex",gap:8}}>
        <input value={form.value} onChange={e=>setForm(p=>({...p,value:e.target.value}))} placeholder="Valor (R$)" type="number" min="0"
          style={{flex:1,padding:"9px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none"}}/>
        <input value={form.quantity} onChange={e=>setForm(p=>({...p,quantity:e.target.value}))} placeholder="Qtd" type="number" min="1"
          style={{width:60,padding:"9px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none"}}/>
      </div>
      <textarea value={form.objective} onChange={e=>setForm(p=>({...p,objective:e.target.value}))} placeholder="Objetivo / justificativa da compra" rows={2}
        style={{padding:"9px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:12,outline:"none",resize:"none",fontFamily:"inherit"}}/>
      <div style={{display:"flex",gap:8}}>
      ...
      <div style={{display:"flex",gap:8}}>
        <button onClick={onSubmit} disabled={!form.name.trim()} style={{flex:1,padding:"9px 0",borderRadius:8,background:form.name.trim()?(cat==="pesado"?B.red:B.blue):B.gray700,border:"none",color:B.white,fontWeight:800,fontSize:13,cursor:form.name.trim()?"pointer":"not-allowed"}}>
          {editId?"Salvar alterações":"Adicionar"}
        </button>
        <button onClick={onCancel} style={{padding:"9px 14px",borderRadius:8,background:B.gray700,border:`1px solid ${B.gray600}`,color:B.gray300,fontSize:12,cursor:"pointer"}}>
          Cancelar
        </button>
      </div>
    </div>
...
function InvCard({inv,adminRole,onUpdate,onStartEdit}) {
  const [confirmDel,setConfirmDel]=useState(false);
  const st=INV_STATUS[inv.status]||INV_STATUS.pending;
  const prColor=PRIORITY_COLORS[inv.priority]||B.gray500;
  const divLabel=inv.division==="finishing"?"🎨":inv.division==="ambos"?"⚙️🎨":"⚙️";
  ...
  const changePriority=(delta)=>{
    const next=Math.min(5,Math.max(1,(inv.priority||3)+delta));
    if(next!==inv.priority) onUpdate(inv.id,{priority:next});
  };

  if(confirmDel) return (
    <div style={{background:`${B.red}12`,borderRadius:12,marginBottom:8,border:`1px solid ${B.red}44`,padding:"14px 16px"}}>
      <div style={{fontWeight:700,fontSize:13,color:B.white,marginBottom:4}}>Excluir "{inv.name}"?</div>
      <div style={{fontSize:12,color:B.gray400,marginBottom:12}}>Esta ação não pode ser desfeita.</div>
      <div style={{display:"flex",gap:8}}>
        <button onClick={()=>setConfirmDel(false)} style={{flex:1,padding:"10px 0",borderRadius:9,background:B.gray700,border:`1px solid ${B.gray600}`,color:B.gray200,fontWeight:700,fontSize:13,cursor:"pointer"}}>Cancelar</button>
        <button onClick={()=>onUpdate(inv.id,null)} style={{flex:1,padding:"10px 0",borderRadius:9,background:B.red,border:"none",color:B.white,fontWeight:800,fontSize:13,cursor:"pointer"}}>Excluir</button>
      </div>
    </div>
  );

  return (<div style={{background:B.gray800,borderRadius:12,marginBottom:8,border:`1px solid ${prColor}44`,overflow:"hidden"}}>
    <div style={{padding:"12px 14px",display:"flex",gap:10,alignItems:"flex-start"}}>
      {/* Priority bar + controls — only for pending items */}
      {inv.status==="pending"&&<div style={{display:"flex",flexDirection:"column",alignItems:"center",gap:3,flexShrink:0}}>
        <button onClick={()=>changePriority(-1)} disabled={inv.priority<=1}
          style={{width:28,height:28,borderRadius:7,border:`1px solid ${B.gray600}`,background:B.gray700,color:inv.priority<=1?B.gray600:B.white,cursor:inv.priority<=1?"not-allowed":"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>▲</button>
        <div style={{width:6,borderRadius:99,background:prColor,height:28,flexShrink:0}}/>
        <button onClick={()=>changePriority(1)} disabled={inv.priority>=5}
          style={{width:28,height:28,borderRadius:7,border:`1px solid ${B.gray600}`,background:B.gray700,color:inv.priority>=5?B.gray600:B.white,cursor:inv.priority>=5?"not-allowed":"pointer",fontSize:14,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>▼</button>
      </div>}
      {inv.status!=="pending"&&<div style={{width:6,borderRadius:99,background:st.color,alignSelf:"stretch",flexShrink:0,minHeight:40}}/>}
    ...
    {/* Action bar */}
    <div style={{borderTop:`1px solid ${B.gray700}`,padding:"8px 12px",display:"flex",gap:6,flexWrap:"wrap"}}>
      {canApprove&&inv.status==="pending"&&<button onClick={()=>onUpdate(inv.id,{status:"approved"})}
        style={{flex:1,padding:"9px 0",borderRadius:9,background:`${B.blue}22`,border:`1px solid ${B.blue}44`,color:B.blue,fontSize:12,fontWeight:800,cursor:"pointer",minWidth:80}}>
        ✅ Aprovar
      </button>}
      ...
      {canDelete&&<button onClick={()=>setConfirmDel(true)}
        style={{padding:"9px 16px",borderRadius:9,background:`${B.red}12`,border:`1px solid ${B.red}33`,color:B.red,fontSize:12,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:5}}>
        <ITrash s={13}/>Excluir
      </button>}
    </div>
  </div>);
...
function InvestmentsTab({investments,adminRole,onAdd,onUpdate,onDelete,onAddToStock,defaultCategory=null}) {
  ...
  const filtered=investments.filter(inv=>{
    ...
    if(divFilter!=="all"&&inv.division!=="ambos"&&inv.division!==divFilter) return false;
    if(search.trim()){const q=search.toLowerCase();if(!(inv.name.toLowerCase().includes(q)||(inv.objective||"").toLowerCase().includes(q))) return false;}
    return true;
  });
  ...
  const submit=async()=>{
    if(!form.name.trim()) return;
    const inv={name:form.name.trim(),link:form.link.trim(),value:parseFloat(form.value)||0,quantity:parseInt(form.quantity)||1,objective:form.objective.trim(),division:form.division,category:form.category||adding||"pesado",priority:parseInt(form.priority)||3};
    if(editId){await onUpdate(editId,inv);setEditId(null);}else{await onAdd(inv);}
    setAdding(null);resetForm();
  };
  const startEdit=(inv)=>{
    setForm({name:inv.name,link:inv.link||"",value:String(inv.value||""),quantity:String(inv.quantity||1),objective:inv.objective||"",division:inv.division,priority:String(inv.priority||3),category:inv.category||"pesado"});
    setEditId(inv.id);setAdding(inv.category);
  };
  ...
  const renderCat=(items,color,open,isMateriais=false)=>(<>
    {statusGroups.map(sg=>{
      ...
      return (<div key={sg.key} style={{marginBottom:8}}>
  ...
  return (<div>
    ...
    <div style={{position:"relative",marginBottom:12}}>
      ...
      <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar por nome ou objetivo..."
        style={{width:"100%",padding:"9px 12px 9px 32px",borderRadius:9,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none",boxSizing:"border-box"}}/>
      {search&&<button onClick={()=>setSearch("")} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:B.gray500,fontSize:16,lineHeight:1}}>×</button>}
    </div>
    ...
    {/* Leves */}
    {(catFilter==="all"||catFilter==="leve")&&<div style={{marginBottom:8}}>
      {sectionHeader(leves,B.blue,"⚡","Investimentos Leves",openLeve,setOpenLeve)}
...
// ─── Purchase Order Panel (inside VehicleCard for mechanics) ─────────────────
function PurchaseOrderPanel({vehicleId,tasks,onAdd,orders=[]}) {
  const [form,setForm]=useState({partName:"",description:"",taskId:"",quantity:"1",photoUrl:""});
  ...
  const submit=async()=>{
    ...
    setForm({partName:"",description:"",taskId:"",quantity:"1",photoUrl:""});
    setAdding(false);
  };
  ...
  const handlePhoto=async(e)=>{
    ...
    setUploading(true);
    try{ const url=await resizeAndUpload(file,"vehicles"); setForm(p=>({...p,photoUrl:url})); }catch{}
    setUploading(false);
  };
  ...
  return (<div style={{padding:"12px 14px",background:`${B.amber}08`,borderBottom:`1px solid ${B.amber}33`}}>
    ...
    {/* Existing orders for this vehicle */}
    {orders.map(o=>{
      const st=PO_STATUS[o.status];
      return (<div key={o.id} style={{background:B.gray800,borderRadius:9,padding:"8px 12px",marginBottom:8,border:`1px solid ${st.color}44`}}>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          {o.photoUrl&&<img src={o.photoUrl} alt="" style={{width:36,height:36,borderRadius:6,objectFit:"cover",flexShrink:0}}/>}
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:13,fontWeight:700,color:B.white}}>{o.partName}</div>
    ...
    {adding&&<div style={{background:B.gray800,borderRadius:10,padding:12,border:`1px solid ${B.amber}44`}}>
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        <input value={form.partName} onChange={e=>setForm(p=>({...p,partName:e.target.value}))} placeholder="Nome da peça *"
          style={{padding:"8px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none"}}/>
        <textarea value={form.description} onChange={e=>setForm(p=>({...p,description:e.target.value}))} placeholder="Descrição / especificações (opcional)" rows={2}
          style={{padding:"8px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:12,outline:"none",resize:"none",fontFamily:"inherit"}}/>
        <div style={{display:"flex",gap:8}}>
          <select value={form.taskId} onChange={e=>setForm(p=>({...p,taskId:e.target.value}))}
            style={{flex:1,padding:"8px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:form.taskId?B.white:B.gray500,fontSize:12,outline:"none"}}>
          ...
          <input value={form.quantity} onChange={e=>setForm(p=>({...p,quantity:e.target.value}))} type="number" min="1" placeholder="Qtd"
            style={{width:60,padding:"8px 10px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:12,outline:"none"}}/>
        </div>
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          {form.photoUrl
        ...
        <div style={{display:"flex",gap:8}}>
          <button onClick={submit} style={{flex:1,padding:"9px 0",borderRadius:8,background:B.amber,border:"none",color:B.black,fontWeight:800,fontSize:13,cursor:"pointer"}}>Enviar pedido</button>
          <button onClick={()=>{setAdding(false);setForm({partName:"",description:"",taskId:"",quantity:"1",photoUrl:""}); }} style={{padding:"9px 14px",borderRadius:8,background:B.gray700,border:`1px solid ${B.gray600}`,color:B.gray300,fontSize:12,cursor:"pointer"}}>Cancelar</button>
        </div>
      </div>
    </div>}
  </div>);
...
function PurchaseOrdersTab({orders,vehicles,employees,tasks,adminRole,onUpdate,onDelete,onReserve}) {
  ...
  const filtered=search.trim()
    ?byStatus.filter(o=>{
        ...
        return o.partName.toLowerCase().includes(q)
          ||(o.description||"").toLowerCase().includes(q)
          ||(o.notesAdmin||"").toLowerCase().includes(q)
  ...
  const startEdit=(o)=>{
    setEditId(o.id);
    setEditForm({link:o.link||"",costPrice:o.costPrice!=null?String(o.costPrice):"",brand:o.notesAdmin||"",quantity:String(o.quantity||1)});
  };
  ...
  const confirmReceive=async(reserve)=>{
    const o=receiveModal;
    await onUpdate(o.id,{status:"received"});
    if(reserve) onReserve&&onReserve(o,location.trim());
    setReceiveModal(null);
    setLocation("");
  };

  const confirmReturn=async(o)=>{
    // Reset back to pending, clear admin fields, add a note about return
    await onUpdate(o.id,{status:"pending",link:"",costPrice:null,notesAdmin:"⚠️ Item devolvido — localizar novamente",quantity:o.quantity});
    setReturnModal(null);
  };
  ...
  return (<div style={{padding:"20px 0"}}>
    {/* Receive decision modal */}
    {receiveModal&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999,padding:16}} onClick={()=>setReceiveModal(null)}>
      <div style={{background:B.gray800,borderRadius:18,maxWidth:380,width:"100%",border:`1px solid ${B.green}55`,padding:24}} onClick={e=>e.stopPropagation()}>
        <div style={{fontSize:22,textAlign:"center",marginBottom:8}}>📦</div>
        <div style={{fontWeight:800,fontSize:16,color:B.white,textAlign:"center",marginBottom:4}}>Peça recebida!</div>
        <div style={{fontSize:13,color:B.gray300,textAlign:"center",marginBottom:4}}><b style={{color:B.white}}>{receiveModal.partName}</b></div>
        <div style={{fontSize:12,color:B.gray400,textAlign:"center",marginBottom:20}}>A peça está correta? O que fazer com ela?</div>
        <input value={location} onChange={e=>setLocation(e.target.value)} placeholder="Localização (ex: Prateleira A3) — para reserva"
          style={{width:"100%",padding:"9px 12px",borderRadius:9,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none",boxSizing:"border-box",marginBottom:12}}/>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          <button onClick={()=>confirmReceive(false)} style={{width:"100%",padding:"12px 0",borderRadius:10,background:B.green,border:"none",color:B.white,fontWeight:800,fontSize:14,cursor:"pointer"}}>
            ✅ Instalar agora — adicionar à tarefa
          </button>
          <button onClick={()=>confirmReceive(true)} style={{width:"100%",padding:"12px 0",borderRadius:10,background:`${B.purple}22`,border:`1px solid ${B.purple}55`,color:B.purple,fontWeight:800,fontSize:14,cursor:"pointer"}}>
            📦 Reservar — guardar no veículo
          </button>
          <button onClick={()=>{setReceiveModal(null);setReturnModal(receiveModal);}} style={{width:"100%",padding:"12px 0",borderRadius:10,background:`${B.red}18`,border:`1px solid ${B.red}44`,color:B.red,fontWeight:800,fontSize:14,cursor:"pointer"}}>
            ↩ Devolver — item incorreto
          </button>
          <button onClick={()=>setReceiveModal(null)} style={{width:"100%",padding:"9px 0",borderRadius:10,background:"none",border:`1px solid ${B.gray600}`,color:B.gray400,fontWeight:600,fontSize:13,cursor:"pointer"}}>
            Cancelar
          </button>
        </div>
      </div>
    ...
    {returnModal&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.8)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999,padding:16}} onClick={()=>setReturnModal(null)}>
      <div style={{background:B.gray800,borderRadius:18,maxWidth:380,width:"100%",border:`1px solid ${B.red}55`,padding:24}} onClick={e=>e.stopPropagation()}>
        <div style={{fontSize:22,textAlign:"center",marginBottom:8}}>↩</div>
        <div style={{fontWeight:800,fontSize:16,color:B.white,textAlign:"center",marginBottom:4}}>Confirmar devolução?</div>
        <div style={{fontSize:13,color:B.gray300,textAlign:"center",marginBottom:4}}><b style={{color:B.white}}>{returnModal.partName}</b></div>
        <div style={{fontSize:12,color:B.gray400,textAlign:"center",marginBottom:20}}>O pedido voltará para o início. O administrativo precisará localizar um novo fornecedor.</div>
        <div style={{display:"flex",flexDirection:"column",gap:8}}>
          <button onClick={()=>confirmReturn(returnModal)} style={{width:"100%",padding:"12px 0",borderRadius:10,background:B.red,border:"none",color:B.white,fontWeight:800,fontSize:14,cursor:"pointer"}}>
            ↩ Confirmar devolução
          </button>
          <button onClick={()=>setReturnModal(null)} style={{width:"100%",padding:"9px 0",borderRadius:10,background:"none",border:`1px solid ${B.gray600}`,color:B.gray400,fontWeight:600,fontSize:13,cursor:"pointer"}}>
            Cancelar
          </button>
        </div>
      </div>
    ...
    <div style={{position:"relative",marginBottom:16}}>
      <svg style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",pointerEvents:"none"}} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke={B.gray500} strokeWidth="2"><circle cx="11" cy="11" r="8"/><line x1="21" y1="21" x2="16.65" y2="16.65"/></svg>
      <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar por peça, veículo, mecânico..."
        style={{width:"100%",padding:"9px 12px 9px 32px",borderRadius:9,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none",boxSizing:"border-box"}}/>
      {search&&<button onClick={()=>setSearch("")} style={{position:"absolute",right:8,top:"50%",transform:"translateY(-50%)",background:"none",border:"none",cursor:"pointer",color:B.gray500,fontSize:16,lineHeight:1}}>×</button>}
    </div>
    ...
    {filtered.filter(o=>o.status!=="received").map(o=>{
      ...
      return (<div key={o.id} style={{background:B.gray800,borderRadius:14,marginBottom:12,border:`1px solid ${st.color}44`,overflow:"hidden"}}>
        {/* Header */}
        <div style={{padding:"12px 14px",borderBottom:`1px solid ${B.gray700}`,display:"flex",gap:10,alignItems:"flex-start"}}>
          {o.photoUrl&&<img src={o.photoUrl} alt="" style={{width:52,height:52,borderRadius:8,objectFit:"cover",flexShrink:0,border:`1px solid ${B.gray600}`}}/>}
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontWeight:800,fontSize:14,color:B.white}}>{o.partName}</div>
            {o.description&&<div style={{fontSize:12,color:B.gray400,marginTop:2}}>{o.description}</div>}
            <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:6}}>
              {v&&<span style={{fontSize:10,color:B.blue,background:`${B.blue}18`,borderRadius:5,padding:"1px 7px",fontWeight:700}}>🚗 {v.model}</span>}
              {emp&&<span style={{fontSize:10,color:B.orange,background:`${B.orange}18`,borderRadius:5,padding:"1px 7px",fontWeight:700}}>👤 {emp.name}</span>}
              {t&&<span style={{fontSize:10,color:B.gray400,background:B.gray700,borderRadius:5,padding:"1px 7px"}}>🔧 {t.label}</span>}
              <span style={{fontSize:10,color:B.gray500}}>Qtd: {o.quantity}</span>
            </div>
          </div>
          <div style={{textAlign:"right",flexShrink:0}}>
        ...
        {/* Admin form / info */}
        {isEditing?(
          <div style={{padding:"12px 14px",borderBottom:`1px solid ${B.gray700}`,display:"flex",flexDirection:"column",gap:8}}>
            <div style={{fontSize:11,fontWeight:700,color:B.blue,textTransform:"uppercase",letterSpacing:.5,marginBottom:2}}>Dados administrativos</div>
            <input value={editForm.link} onChange={e=>setEditForm(p=>({...p,link:e.target.value}))} placeholder="Link da peça (URL)"
              style={{padding:"8px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:12,outline:"none"}}/>
            <div style={{display:"flex",gap:8}}>
              <input value={editForm.costPrice} onChange={e=>setEditForm(p=>({...p,costPrice:e.target.value}))} placeholder="Preço de custo (R$)" type="number"
                style={{flex:1,padding:"8px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:12,outline:"none"}}/>
              <input value={editForm.quantity} onChange={e=>setEditForm(p=>({...p,quantity:e.target.value}))} placeholder="Qtd" type="number" min="1"
                style={{width:60,padding:"8px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:12,outline:"none"}}/>
            </div>
            <input value={editForm.brand} onChange={e=>setEditForm(p=>({...p,brand:e.target.value}))} placeholder="Marca (ex: Bosch, Monroe...)"
              style={{padding:"8px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:12,outline:"none"}}/>
            <div style={{display:"flex",gap:8}}>
              <button onClick={saveEdit} style={{flex:1,padding:"8px 0",borderRadius:8,background:B.blue,border:"none",color:B.white,fontWeight:800,fontSize:12,cursor:"pointer"}}>Salvar</button>
              <button onClick={()=>setEditId(null)} style={{padding:"8px 14px",borderRadius:8,background:B.gray700,border:`1px solid ${B.gray600}`,color:B.gray300,fontSize:12,cursor:"pointer"}}>Cancelar</button>
            </div>
          </div>
        ):(o.link||o.costPrice!=null||o.notesAdmin)&&(
          <div style={{padding:"10px 14px",borderBottom:`1px solid ${B.gray700}`,background:`${B.blue}08`}}>
            {o.link&&<div style={{display:"flex",alignItems:"center",gap:6,marginBottom:6}}>
              <span style={{fontSize:10,color:B.gray500,flexShrink:0}}>Link:</span>
              <a href={o.link} target="_blank" rel="noreferrer" style={{fontSize:12,color:B.blue,textDecoration:"none",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",flex:1}}>{o.link}</a>
            </div>}
        ...
        <div style={{padding:"10px 14px",display:"flex",gap:8,flexWrap:"wrap",alignItems:"center"}}>
          {canEdit&&o.status!=="received"&&<button onClick={()=>isEditing?setEditId(null):startEdit(o)}
            style={{padding:"6px 12px",borderRadius:8,background:B.gray700,border:`1px solid ${B.gray600}`,color:B.gray300,fontSize:11,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:5}}>
            <IEdit s={12}/>Editar dados
          </button>}
          ...
          {(adminRole==="owner"||adminRole==="admin")&&(o.status==="bought"||o.status==="ready_to_buy")&&<button onClick={()=>setReturnModal(o)}
            style={{padding:"6px 12px",borderRadius:8,background:`${B.red}12`,border:`1px solid ${B.red}33`,color:B.red,fontSize:11,fontWeight:700,cursor:"pointer",display:"flex",alignItems:"center",gap:4}}>
            ↩ Cancelar pedido
          </button>}
...
// ─── Expenses Panel ───────────────────────────────────────────────────────────
const EXPENSE_CATS = [
  {id:"salario",  label:"Salário",   color:"#3b82f6", icon:"👤"},
  {id:"aluguel",  label:"Aluguel",   color:"#f59e0b", icon:"🏢"},
  {id:"servicos", label:"Serviços",  color:"#8b5cf6", icon:"🔌"},
  {id:"outros",   label:"Outros",    color:"#6b7280", icon:"📦"},
];

function ExpensesPanel({expenses=[],onAdd,onUpdate,onDelete}) {
  const [open,setOpen]=useState(false);
  const [adding,setAdding]=useState(false);
  const [editId,setEditId]=useState(null);
  const [form,setForm]=useState({name:"",amount:"",category:"salario",division:"ambos",recurrent:true});

  const totalExpenses=expenses.reduce((s,e)=>s+e.amount,0);
  const byCat=EXPENSE_CATS.map(c=>({...c,items:expenses.filter(e=>e.category===c.id),total:expenses.filter(e=>e.category===c.id).reduce((s,e)=>s+e.amount,0)})).filter(c=>c.items.length>0);

  const resetForm=()=>setForm({name:"",amount:"",category:"salario",division:"ambos",recurrent:true});

  const submit=async()=>{
    const amt=parseFloat(String(form.amount).replace(",","."));
    if(!form.name.trim()||!amt||amt<=0) return;
    if(editId){
      await onUpdate(editId,{...form,amount:amt});
      setEditId(null);
    } else {
  ...
  const startEdit=(e)=>{
    setForm({name:e.name,amount:String(e.amount),category:e.category,division:e.division,recurrent:e.recurrent});
    setEditId(e.id);
    setAdding(false);
  };

  const FormRow=()=>(
    <div style={{background:B.gray900,borderRadius:10,padding:14,border:`1px solid ${B.gray600}`,marginTop:10}}>
      <div style={{display:"flex",flexDirection:"column",gap:8}}>
        <input value={form.name} onChange={e=>setForm(p=>({...p,name:e.target.value}))} placeholder="Nome da despesa (ex: Salário João)"
          style={{padding:"9px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none"}}/>
        <div style={{display:"flex",gap:8}}>
          <input value={form.amount} onChange={e=>setForm(p=>({...p,amount:e.target.value}))} placeholder="Valor (R$)" type="number" min="0"
            style={{flex:1,padding:"9px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none"}}/>
        ...
        <div style={{display:"flex",gap:8}}>
          <select value={form.division} onChange={e=>setForm(p=>({...p,division:e.target.value}))}
            style={{flex:1,padding:"9px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none"}}>
            <option value="ambos">⚙️🎨 Ambas</option>
            <option value="performance">⚙️ Performance</option>
            <option value="finishing">🎨 Finishing</option>
          </select>
          <label style={{display:"flex",alignItems:"center",gap:6,fontSize:12,color:B.gray300,cursor:"pointer",padding:"0 4px"}}>
            <input type="checkbox" checked={form.recurrent} onChange={e=>setForm(p=>({...p,recurrent:e.target.checked}))} style={{accentColor:B.orange}}/>
            Mensal
          </label>
        </div>
        <div style={{display:"flex",gap:8}}>
          <button onClick={submit} style={{flex:1,padding:"9px 0",borderRadius:8,background:B.orange,border:"none",color:B.white,fontWeight:800,fontSize:13,cursor:"pointer"}}>
            {editId?"Salvar alteração":"Adicionar"}
          </button>
          <button onClick={()=>{setAdding(false);setEditId(null);resetForm();}} style={{padding:"9px 14px",borderRadius:8,background:B.gray700,border:`1px solid ${B.gray600}`,color:B.gray300,fontWeight:600,fontSize:13,cursor:"pointer"}}>
            Cancelar
          </button>
        </div>
      </div>
  ...
  return (<div style={{marginTop:24,background:B.gray800,borderRadius:14,border:`1px solid ${B.gray700}`,overflow:"hidden"}}>
    {/* Header */}
    <button onClick={()=>setOpen(o=>!o)} style={{width:"100%",display:"flex",alignItems:"center",gap:10,padding:"14px 16px",background:"none",border:"none",cursor:"pointer",textAlign:"left"}}>
      <div style={{width:34,height:34,borderRadius:8,background:`${B.red}18`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:16,flexShrink:0}}>💸</div>
      <div style={{flex:1}}>
        <div style={{fontWeight:800,fontSize:14,color:B.white}}>Despesas Fixas</div>
        <div style={{fontSize:11,color:B.gray400}}>Salários, aluguel e custos operacionais</div>
      </div>
      <div style={{textAlign:"right",marginRight:8}}>
        <div style={{fontSize:15,fontWeight:800,color:B.red}}>{fmtBRL(totalExpenses)}</div>
        <div style={{fontSize:10,color:B.gray500}}>{expenses.length} item{expenses.length!==1?"s":""}/mês</div>
      </div>
      <span style={{color:B.gray400}}>{open?"▲":"▼"}</span>
    </button>

    {open&&<div style={{padding:"0 16px 16px"}}>
      {/* Category summary */}
      {byCat.length>0&&<div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:14}}>
        {byCat.map(c=>(
          <div key={c.id} style={{background:`${c.color}18`,border:`1px solid ${c.color}33`,borderRadius:8,padding:"6px 12px",display:"flex",alignItems:"center",gap:6}}>
            <span>{c.icon}</span>
            <div>
              <div style={{fontSize:10,color:c.color,fontWeight:700}}>{c.label}</div>
              <div style={{fontSize:12,fontWeight:800,color:B.white}}>{fmtBRL(c.total)}</div>
            </div>
          </div>
        ))}
      </div>}
      ...
      {/* List */}
      {expenses.length===0&&!adding&&<div style={{textAlign:"center",padding:"20px 0",color:B.gray500,fontSize:13}}>Nenhuma despesa cadastrada</div>}
      {expenses.map(e=>{
        const cat=EXPENSE_CATS.find(c=>c.id===e.category)||EXPENSE_CATS[3];
        if(editId===e.id) return <div key={e.id}><FormRow/></div>;
        return (<div key={e.id} style={{display:"flex",alignItems:"center",gap:10,padding:"10px 0",borderBottom:`1px solid ${B.gray700}`}}>
          <span style={{fontSize:18,flexShrink:0}}>{cat.icon}</span>
          <div style={{flex:1,minWidth:0}}>
            <div style={{fontSize:13,fontWeight:700,color:B.white}}>{e.name}</div>
            <div style={{fontSize:10,color:B.gray500}}>
              <span style={{color:cat.color}}>{cat.label}</span>
              {" · "}{e.division==="ambos"?"⚙️🎨":e.division==="finishing"?"🎨":"⚙️"}
              {e.recurrent&&" · Mensal"}
            </div>
          </div>
          <div style={{fontSize:13,fontWeight:800,color:B.red,flexShrink:0}}>{fmtBRL(e.amount)}</div>
          <button onClick={()=>startEdit(e)} style={{background:"none",border:"none",cursor:"pointer",color:B.gray500,padding:4,fontSize:12}} title="Editar">✏️</button>
          <button onClick={()=>onDelete(e.id)} style={{background:"none",border:"none",cursor:"pointer",color:B.gray500,padding:4}} title="Remover">
            <ITrash s={14}/>
          </button>
        </div>);
      })}
      ...
      {!adding&&!editId&&<button onClick={()=>setAdding(true)} style={{marginTop:10,width:"100%",padding:"9px 0",borderRadius:9,background:`${B.orange}18`,border:`1px dashed ${B.orange}66`,color:B.orange,fontWeight:700,fontSize:13,cursor:"pointer"}}>
        + Adicionar despesa
      </button>}

      {/* Total */}
      {expenses.length>0&&<div style={{marginTop:12,padding:"10px 14px",background:`${B.red}12`,border:`1px solid ${B.red}33`,borderRadius:9,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <span style={{fontSize:12,color:B.gray400,fontWeight:700}}>Total mensal de despesas</span>
        <span style={{fontSize:16,fontWeight:900,color:B.red}}>{fmtBRL(totalExpenses)}</span>
      </div>}
    </div>}
  </div>);
}
...
function FinanceTab({tasks,vehicles,clients,employees,payments,defaultRate,expenses=[],onAddExpense,onUpdateExpense,onDeleteExpense,osHistory=[],internalTransfers=[],onAddTransfer,onDeleteTransfer,adminRole=""}) {
  ...
  return (<div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:10,marginBottom:24}}>
      {[
        {label:"Receita (pagamentos)",value:totalRevenue,color:B.blue,bg:B.blueBg,icon:<IChart s={16} c={B.blue}/>},
        {label:"Custo (material)",value:totalCost,color:B.red,bg:`${B.red}18`,icon:<IBox s={16} c={B.red}/>},
        {label:"Lucro",value:totalProfit,color:B.green,bg:B.greenBg,icon:<ITrendUp s={16} c={B.green}/>},
        {label:"A receber (saldo aberto)",value:totalReceivable,color:B.amber,bg:B.amberBg,icon:<IBank s={16} c={B.amber}/>},
    ...
    {vehicleRows.length===0?<div style={{textAlign:"center",padding:"40px 0",color:B.gray400,fontSize:13}}>Nenhum pagamento registrado ainda.</div>
      :<div style={{display:"flex",flexDirection:"column",gap:8}}>
        {vehicleRows.map(r=>(<div key={r.v.id} style={{background:B.gray800,borderRadius:11,border:`1px solid ${B.gray700}`,padding:"12px 16px",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
          ...
          <div style={{display:"flex",gap:16,flexWrap:"wrap"}}>
            <div style={{textAlign:"right"}}><div style={{fontSize:10,color:B.gray400}}>Pago</div><div style={{fontSize:13,fontWeight:700,color:B.blue}}>{fmtBRL(r.revenue)}</div></div>
  ...
  return (<div style={{minHeight:"100vh",background:B.black,fontFamily:"'Inter','Segoe UI',sans-serif",color:B.white}}>
    ...
    <div style={{background:B.gray900,borderBottom:`2px solid ${hasFin?FD.primary:B.orange}`}}>
      ...
      <div style={{padding:"12px 16px",paddingTop:"calc(12px + env(safe-area-inset-top))",display:"flex",alignItems:"center",gap:10}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontWeight:900,fontSize:14,color:B.white,lineHeight:1.2}}>
            OSC <span style={{color:B.orange}}>Performance</span>
            {hasFin&&<><span style={{color:B.gray600,margin:"0 5px"}}>+</span><span style={{color:FD.primary}}>Finishing</span></>}
          </div>
          <div style={{fontSize:10,color:B.gray500,textTransform:"uppercase",letterSpacing:.5,marginTop:2}}>
            {hasPerf&&v.osNumber&&<span style={{color:B.orange,marginRight:6}}>{fmtOS(v.osNumber)}</span>}
            {hasFin&&v.osNumberFinishing&&<span style={{color:FD.primary,marginRight:6}}>{fmtOSFD(v.osNumberFinishing)}</span>}
    ...
    <div style={{maxWidth:620,margin:"0 auto",padding:"20px 16px"}}>
        ...
        <div style={S.pad}>
          ...
          <div style={{display:"flex",gap:8,flexWrap:"wrap",marginTop:10}}>
            ...
            {v.osNumber&&<div style={{...S.chip,background:`${B.orange}18`,border:`1px solid ${B.orange}33`}}>
              <div style={S.label}>⚙️ OS Performance</div>
              <div style={{fontSize:15,fontWeight:800,color:B.orange}}>{fmtOS(v.osNumber)}</div>
            </div>}
            {v.osNumberFinishing&&<div style={{...S.chip,background:FD.bg,border:`1px solid ${FD.border}`}}>
              <div style={S.label}>🎨 OS Finishing</div>
              <div style={{fontSize:15,fontWeight:800,color:FD.primary}}>{fmtOSFD(v.osNumberFinishing)}</div>
            </div>}
      ...
      {(()=>{
        ...
        <div style={S.pad}>
          ...
          {/* Checklist */}
          {(()=>{
            const cl=Array.isArray(v.checklist)?v.checklist:[];
            const answered=cl.filter(c=>c.status);
            if(!answered.length) return null;
            const badItems=cl.filter(c=>c.status==="bad");
            const okItems=cl.filter(c=>c.status==="ok");
            return (<div style={{marginBottom:12,background:B.gray900,borderRadius:10,overflow:"hidden",border:`1px solid ${B.gray700}`}}>
              <div style={{padding:"10px 14px",borderBottom:`1px solid ${B.gray700}`,display:"flex",alignItems:"center",gap:8}}>
                <span style={{fontSize:13,fontWeight:800,color:B.blue}}>📋 Checklist de entrada</span>
                <span style={{fontSize:11,color:B.green,marginLeft:4}}>✓ {okItems.length} ok</span>
                {badItems.length>0&&<span style={{fontSize:11,color:"#f87171"}}>✕ {badItems.length} com problema</span>}
              </div>
              <div style={{padding:"10px 14px",display:"flex",flexDirection:"column",gap:4}}>
                {CHECKLIST_ITEMS.filter(item=>cl.find(c=>c.id===item.id)?.status).map(item=>{
                  const state=cl.find(c=>c.id===item.id);
                  const isBad=state?.status==="bad";
                  const catColor=CAT_MAP[item.category]||B.gray500;
                  return (<div key={item.id} style={{display:"flex",alignItems:"center",gap:8,padding:"5px 0",borderBottom:`1px solid ${B.gray800}`}}>
                    <span style={{fontSize:14,flexShrink:0}}>{isBad?"⚠️":"✅"}</span>
                    <span style={{fontSize:12,color:isBad?"#fca5a5":B.gray300,flex:1}}>{item.label}</span>
                    <span style={{fontSize:9,fontWeight:700,color:catColor,background:catColor+"18",borderRadius:4,padding:"1px 5px",flexShrink:0}}>{item.category}</span>
                  </div>);
                })}
              </div>
              {/* Entry photos */}
          ...
          {((v.fuels||[]).filter(f=>Number(f.value||0)>0).length>0||(v.tows||[]).filter(t=>Number(t.value||0)>0).length>0)&&<div style={{marginBottom:12,display:"flex",flexDirection:"column",gap:6}}>
            {(v.fuels||[]).filter(f=>Number(f.value||0)>0).map((f,i)=>(
              <div key={i} style={{display:"flex",alignItems:"center",gap:8,background:B.gray900,border:`1px solid ${B.gray700}`,borderRadius:8,padding:"8px 12px"}}>
                ...
                <div style={{flex:1,minWidth:0}}>
                  <span style={{fontSize:13,color:B.gray300}}>{f.type||"Combustível"}</span>
                  {f.date&&<span style={{fontSize:11,color:B.gray500,marginLeft:6}}>{f.date}</span>}
                </div>
                <span style={{fontSize:13,fontWeight:700,color:B.amber,flexShrink:0}}>{fmtBRL(f.value)}</span>
              </div>
            ))}
          ...
          <div style={{marginTop:8}}>
            {catOrder.map(cat=>{
              const groupTasks=regularTs.filter(t=>(t.category||null)===cat).sort((a,b)=>(a.done?1:0)-(b.done?1:0));
      ...
      {hasFin&&finTasks.length>0&&<div style={{...S.card,border:`1px solid ${FD.border}`}}>
        <div style={{background:FD.bg,borderBottom:`2px solid ${FD.primary}`,padding:"10px 16px",display:"flex",alignItems:"center",gap:8}}>
          <span style={{fontSize:16}}>🎨</span>
          <span style={{fontWeight:800,fontSize:13,color:FD.primary}}>OSC Finishing Division</span>
          {v.osNumberFinishing&&<span style={{fontSize:11,color:FD.primary,background:`${FD.primary}18`,borderRadius:5,padding:"1px 7px",fontWeight:800}}>{fmtOSFD(v.osNumberFinishing)}</span>}
          <span style={{fontSize:11,color:B.gray400,marginLeft:"auto"}}>{finTasks.filter(t=>t.done).length}/{finTasks.length} concluídos</span>
        </div>
        <div style={S.pad}>
          {buildCatOrder(finTasks).map(cat=>{
            const catTs=finTasks.filter(t=>(t.category||null)===cat).sort((a,b)=>(a.done?1:0)-(b.done?1:0));
            const catColor=cat?CAT_MAP_FINISHING[cat]||CAT_MAP[cat]||B.gray500:B.gray500;
            return (<div key={cat||"__none__"} style={{marginBottom:12}}>
              <div style={{display:"flex",alignItems:"center",gap:6,padding:"5px 10px",background:catColor+"18",borderLeft:`3px solid ${catColor}`,borderRadius:"0 6px 6px 0",marginBottom:6}}>
                ...
                <span style={{fontSize:10,color:catColor+"88",marginLeft:"auto"}}>{catTs.filter(t=>t.done).length}/{catTs.length}</span>
              </div>
              {catTs.map(t=>{
                const tPhotos=photosF.filter(p=>p.taskId===t.id);
                const signer=t.completedByEmployeeId?employees.find(e=>e.id===t.completedByEmployeeId):null;
                    ...
                    <div style={{flex:1,minWidth:0}}>
                      ...
                      {(t.materials||[]).filter(m=>m.name).map((m,i)=>(
                        <div key={i} style={{display:"flex",alignItems:"center",gap:5,flexWrap:"wrap",marginTop:3}}>
                          ...
                          <span style={{fontSize:11,color:B.gray500}}>{m.name}</span>
                          {m.brand&&<span style={{fontSize:10,color:B.gray600}}>· {m.brand}</span>}
                          {m.qty>1&&<span style={{fontSize:10,color:B.gray600}}>×{m.qty}</span>}
                        </div>
                      ))}
                      {signer&&<div style={{marginTop:4}}><span style={{fontSize:10,color:B.green,background:B.greenBg,border:`1px solid ${B.green}33`,borderRadius:5,padding:"1px 6px"}}>✓ {signer.name}</span></div>}
                      {tPhotos.length>0&&<div style={{display:"flex",gap:6,flexWrap:"wrap",marginTop:8}}>
                        {tPhotos.map((p,pi)=><div key={pi} style={{width:72,height:72,borderRadius:8,overflow:"hidden",cursor:"pointer",border:`1px solid ${FD.border}`,flexShrink:0}} onClick={()=>setLB(p)}>
      ...
      {warrantyAllTasks.length>0&&<div style={{background:B.gray900,borderRadius:14,overflow:"hidden",margin:"0 0 16px",border:`2px solid ${B.red}44`}}>
        <div style={{display:"flex",alignItems:"center",gap:8,padding:"12px 16px",background:`${B.red}18`,borderBottom:`1px solid ${B.red}33`}}>
          ...
          <div>
            <div style={{fontWeight:800,fontSize:14,color:B.red}}>Serviços em Garantia</div>
      ...
      {/* ── Account / Financeiro ── */}
      {(()=>{
        const perfPaid=payments.filter(p=>p.vehicleId===v.id&&!p.osHistoryId&&(p.division||"performance")==="performance").reduce((s,p)=>s+Number(p.amount),0);
        const finPaid=payments.filter(p=>p.vehicleId===v.id&&!p.osHistoryId&&p.division==="finishing").reduce((s,p)=>s+Number(p.amount),0);
        const activePaid=perfPaid+finPaid;
        const pastOSs=osHistory.filter(h=>h.vehicle_id===v.id);
        const totalOwedPast=pastOSs.reduce((s,h)=>{const paid=payments.filter(p=>p.osHistoryId===h.id).reduce((a,p)=>a+Number(p.amount),0);return s+Math.max(0,Number(h.total_value||0)-paid);},0);
        const perfLaborTotal=perfTasks.reduce((s,t)=>s+taskCost(t,defaultRate).labor,0);
        const perfPartsTotal=perfTasks.reduce((s,t)=>s+taskCost(t,defaultRate).mat,0);
        const perfFreightTotal=perfTasks.reduce((s,t)=>s+taskCost(t,defaultRate).freight,0);
        const fuelTotal=(v.fuels||[]).reduce((s,f)=>s+Number(f.value||0),0)+Number(v.fuelCost||0);
        const towTotal=(v.tows||[]).reduce((s,t)=>s+Number(t.value||0),0);
        const perfDiscount=perfLaborTotal*Number(v.osDiscountPct||0)/100;
        const perfTotal=perfLaborTotal+perfPartsTotal+perfFreightTotal+fuelTotal+towTotal-perfDiscount;
        const finLaborTotal=finTasks.reduce((s,t)=>s+taskCost(t,defaultRate).labor,0);
        const finPartsTotal=finTasks.reduce((s,t)=>s+taskCost(t,defaultRate).mat,0);
        const finFreightTotal=finTasks.reduce((s,t)=>s+taskCost(t,defaultRate).freight,0);
        const finDiscount=finLaborTotal*Number(v.osDiscountPctFinishing||0)/100;
        const finTotal=finLaborTotal+finPartsTotal+finFreightTotal-finDiscount;
        const activeTotal=perfTotal+finTotal;
        ...
        const renderBreakdown=(taskList,catMapExtra={})=>buildCatOrder(taskList).map(cat=>{
          const catTs=taskList.filter(t=>(t.category||null)===cat);
          const cL=catTs.reduce((s,t)=>s+taskCost(t,defaultRate).labor,0);
          const cP=catTs.reduce((s,t)=>s+taskCost(t,defaultRate).mat,0);
          const cF=catTs.reduce((s,t)=>s+taskCost(t,defaultRate).freight,0);
          const cT=cL+cP+cF; if(cT===0) return null;
          const catColor=cat?(catMapExtra[cat]||CAT_MAP[cat]||B.gray500):B.gray500;
          return (<div key={cat||"__none__"} style={{paddingBottom:8,marginBottom:8,borderBottom:`1px solid ${B.gray700}`}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
              ...
              <span style={{fontSize:12,fontWeight:800,color:B.white}}>{fmtBRL(cT)}</span>
            </div>
            {catTs.map((t,ti)=>{const tc=taskCost(t,defaultRate);if(tc.total===0)return null;
              return(<div key={ti} style={{marginBottom:4,paddingLeft:8,borderLeft:`2px solid ${catColor}33`}}>
        ...
        return (<div style={S.card}>
          <div style={S.pad}>
            ...
            {hasActive&&<div style={{marginBottom:totalOwedPast>0?16:0}}>
              {/* Performance */}
              {perfTotal>0&&<div style={{marginBottom:12}}>
                <div style={{fontSize:11,color:B.orange,fontWeight:800,textTransform:"uppercase",letterSpacing:.5,marginBottom:8}}>⚙️ OSC Performance</div>
                <div style={{background:B.gray900,border:`1px solid ${B.gray700}`,borderRadius:10,padding:"12px 14px",marginBottom:6}}>
                  <div style={{display:"flex",flexDirection:"column",gap:6}}>
                    ...
                    <div style={{borderTop:`1px solid ${B.gray600}`,paddingTop:8,display:"flex",justifyContent:"space-between"}}>
                      ...
                      <span style={{fontSize:14,fontWeight:900,color:B.orange}}>{fmtBRL(perfTotal)}</span>
                    </div>
                  </div>
                </div>
              </div>}
              {/* Finishing */}
              {finTotal>0&&<div style={{marginBottom:12}}>
                <div style={{fontSize:11,color:FD.primary,fontWeight:800,textTransform:"uppercase",letterSpacing:.5,marginBottom:8}}>🎨 OSC Finishing Division</div>
                <div style={{background:B.gray900,border:`1px solid ${FD.border}`,borderRadius:10,padding:"12px 14px",marginBottom:6}}>
                  <div style={{display:"flex",flexDirection:"column",gap:6}}>
                    {renderBreakdown(finTasks,CAT_MAP_FINISHING)}
                    {finDiscount>0&&<div style={{display:"flex",justifyContent:"space-between",fontSize:13}}><span style={{color:B.red}}>Desconto</span><span style={{color:B.red,fontWeight:600}}>-{fmtBRL(finDiscount)}</span></div>}
                    <div style={{borderTop:`1px solid ${FD.border}`,paddingTop:8,display:"flex",justifyContent:"space-between"}}>
                      <span style={{fontSize:13,fontWeight:800,color:B.white}}>Subtotal</span>
                      <span style={{fontSize:14,fontWeight:900,color:FD.primary}}>{fmtBRL(finTotal)}</span>
                    </div>
                  </div>
                </div>
              </div>}
              {/* Grand total */}
              {activeTotal>0&&<div style={{background:B.gray900,border:`1px solid ${B.gray600}`,borderRadius:10,padding:"12px 14px",marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between"}}>
                  <span style={{fontSize:14,fontWeight:800,color:B.white}}>Total geral</span>
                  <span style={{fontSize:18,fontWeight:900,color:B.amber}}>{fmtBRL(activeTotal)}</span>
                </div>
              </div>}
              {activePaid>0&&<div style={{background:B.greenBg,border:`1px solid ${B.green}33`,borderRadius:10,padding:"10px 14px",marginBottom:8}}>
                ...
                <div style={{display:"flex",flexDirection:"column"}}>
                  {payments.filter(p=>p.vehicleId===v.id&&!p.osHistoryId).map((p,pi,arr)=>{
                    const isFDPay=p.division==="finishing";
                    return (<div key={p.id}>
                      <div style={{display:"flex",alignItems:"center",gap:3,flexWrap:"wrap",padding:"10px 0"}}>
                        <span style={{fontSize:12,color:B.green,fontWeight:700}}>💰 {fmtBRL(p.amount)}</span>
                        <span style={{fontSize:11,color:B.gray400,marginLeft:2}}>·</span>
                        <span style={{fontSize:11,color:B.gray300,marginRight:"auto"}}>{p.method}</span>
                        <span style={{fontSize:9,fontWeight:800,borderRadius:4,padding:"2px 7px",
                      ...
                      {pi<arr.length-1&&<div style={{height:1,background:`${B.green}22`}}/>}
                    </div>);
                  })}
                </div>
              </div>}
            ...
            {totalOwedPast>0&&<div>
              ...
              {pastOSs.filter(h=>Number(h.total_value||0)>0).map(h=>{
                ...
                const divColor=(h.division||"performance")==="finishing"?FD.primary:B.orange;
                return (<div key={h.id} style={{background:settled?B.greenBg:B.gray900,border:`1px solid ${settled?B.green+"33":B.gray700}`,borderRadius:10,padding:"10px 14px",marginBottom:8}}>
                  <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",flexWrap:"wrap",gap:8}}>
                    <div>
                      <div style={{fontSize:11,color:B.gray400,marginBottom:2,display:"flex",alignItems:"center",gap:5}}>
                        <span style={{fontSize:9,fontWeight:800,color:divColor}}>{(h.division||"performance")==="finishing"?"🎨":"⚙️"}</span>
                        {h.os_number?`OS ${String(h.os_number).padStart(4,"0")}`:""}{h.delivered_at?` · ${new Date(h.delivered_at).toLocaleDateString("pt-BR")}`:""}
                      </div>
                  ...
                  {payments.filter(p=>p.osHistoryId===h.id).length>0&&<div style={{marginTop:10,paddingTop:10,borderTop:`1px solid ${settled?B.green+"22":B.gray700}`,display:"flex",flexDirection:"column"}}>
                    {payments.filter(p=>p.osHistoryId===h.id).map((p,pi,arr)=>(
                      <div key={p.id}>
                        <div style={{display:"flex",alignItems:"center",gap:3,flexWrap:"wrap",padding:"6px 0"}}>
                          <span style={{fontSize:12,color:B.green,fontWeight:700,flexShrink:0}}>💰 {fmtBRL(p.amount)}</span>
                          <span style={{fontSize:11,color:B.gray400,marginLeft:2,flexShrink:0}}>·</span>
                          <span style={{fontSize:11,color:B.gray300,flexShrink:0,marginRight:"auto"}}>{p.method}</span>
                          <span style={{fontSize:11,color:B.gray500,flexShrink:0}}>{new Date(p.paidAt).toLocaleDateString("pt-BR",{day:"2-digit",month:"2-digit",year:"numeric"})}</span>
                          {p.note&&<span style={{fontSize:11,color:B.gray500,width:"100%"}}>{p.note}</span>}
                        </div>
                        {pi<arr.length-1&&<div style={{height:1,background:settled?`${B.green}22`:B.gray700}}/>}
                      </div>
                    ))}
                  </div>}
                </div>);
...
function MechanicPortal({employee,vehicles,tasks,employees,clients,stock,onAddTask,onToggleTask,onDeleteTask,onUpdateTask,onUpdateVehicle,onLogout,purchaseOrders=[],onAddPurchaseOrder}) {
  const isFD = employee.division === "finishing";
  const accentColor = isFD ? FD.primary : B.orange;
  const [pushStatus,setPushStatus]=useState(null); // null | "asking" | "granted" | "denied" | "unavailable"

  useEffect(()=>{
    // Pre-register SW immediately so it's ready when user taps "Ativar"
    ...
    const check=async()=>{
      if(!("PushManager" in window)||!VAPID_PUBLIC_KEY){ setPushStatus("unavailable"); return; }
      if("Notification" in window && Notification.permission==="denied"){ setPushStatus("denied"); return; }
      const sub=await getPushSubscription();
      if(sub){ setPushStatus("granted"); return; }
  ...
  return (<div style={{height:"100%",display:"flex",flexDirection:"column",background:B.black,fontFamily:"'Inter','Segoe UI',sans-serif",color:B.white}}>
    <div style={{background:B.gray900,borderBottom:`1px solid ${B.gray700}`,padding:"0 18px",paddingTop:"env(safe-area-inset-top)",position:"sticky",top:0,zIndex:20,flexShrink:0}}>
      <div style={{maxWidth:680,margin:"0 auto",display:"flex",alignItems:"center",height:58,gap:12}}>
        <div style={{width:36,height:36,borderRadius:9,background:accentColor,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{isFD?<span style={{fontSize:18}}>🎨</span>:<IWrench s={18} c={B.white}/>}</div>
        <div><div style={{fontWeight:800,fontSize:14,color:B.white}}>{employee.name}</div><div style={{fontSize:10,color:B.gray400,textTransform:"uppercase",letterSpacing:.5}}>Minhas Tarefas — {isFD?"OSC Finishing Division":"OSC Performance"}</div></div>
        <button onClick={onLogout} style={{marginLeft:"auto",padding:"7px 12px",borderRadius:8,background:B.gray800,border:`1px solid ${B.gray700}`,color:B.gray300,cursor:"pointer",fontWeight:600,fontSize:12,display:"flex",alignItems:"center",gap:5}}>
          <ILogout s={13} c={B.gray300}/>Sair
        </button>
      </div>
    ...
    <div style={{maxWidth:680,margin:"0 auto",padding:"20px 14px",paddingBottom:"calc(40px + env(safe-area-inset-bottom))",minHeight:"100vh"}}>
      ...
      {pushStatus==="asking"&&<div style={{background:`${accentColor}18`,border:`1px solid ${accentColor}44`,borderRadius:12,padding:"12px 16px",marginBottom:16,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
        <span style={{fontSize:20}}>🔔</span>
        <div style={{flex:1,minWidth:0}}>
          <div style={{fontSize:13,fontWeight:700,color:B.white}}>Ativar notificações?</div>
          <div style={{fontSize:11,color:B.gray400}}>Receba alertas quando houver atualizações nos seus veículos.</div>
        </div>
        <button onClick={requestPush} style={{padding:"7px 14px",borderRadius:8,background:accentColor,border:"none",color:B.white,fontWeight:700,fontSize:12,cursor:"pointer",flexShrink:0}}>Ativar</button>
        <button onClick={()=>setPushStatus("denied")} style={{padding:"7px 10px",borderRadius:8,background:"none",border:`1px solid ${B.gray600}`,color:B.gray400,fontWeight:600,fontSize:12,cursor:"pointer",flexShrink:0}}>Agora não</button>
      </div>}
...
async function registerPushSubscription(employeeId) {
  try {
    if (!("serviceWorker" in navigator) || !("PushManager" in window)) return false;
    if (!VAPID_PUBLIC_KEY) return false;

    // iOS requires permission request to be as close to user gesture as possible
    const perm = await Notification.requestPermission();
    if (perm !== "granted") return false;

    // Use existing registration — must be registered before the button click
    let reg = await navigator.serviceWorker.getRegistration("/sw.js");
    if (!reg) reg = await navigator.serviceWorker.register("/sw.js", { scope: "/" });

    // Wait for SW to be ready (with timeout for iOS)
    await Promise.race([
    ...
    await db.savePushSubscription(employeeId, sub);
    return true;
  } catch(e) { console.warn("Push register:", e); return false; }
}
...
// ─── Version & Changelog ─────────────────────────────────────────────────────
const APP_VERSION = "2026.08.18.13";

function ChangelogModal({onClose}) {
  const [entries,setEntries]=useState([]);
  useEffect(()=>{
    fetch("/changelog.json?v="+APP_VERSION)
      .then(r=>r.json()).then(setEntries).catch(()=>{});
  },[]);
  return (<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:9999,padding:16}} onClick={onClose}>
    <div style={{background:B.gray800,borderRadius:18,maxWidth:420,width:"100%",border:`1px solid ${B.orange}55`,overflow:"hidden",maxHeight:"80vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
      <div style={{padding:"16px 20px",background:B.gray900,borderBottom:`2px solid ${B.orange}`,display:"flex",alignItems:"center",gap:10}}>
        ...
        <div>
          <div style={{fontWeight:800,fontSize:15,color:B.white}}>Nova versão disponível</div>
          <div style={{fontSize:11,color:B.gray400}}>v{APP_VERSION}</div>
        </div>
        <button onClick={onClose} style={{marginLeft:"auto",background:"none",border:"none",cursor:"pointer",color:B.gray400,fontSize:20,lineHeight:1}}>×</button>
      </div>
      <div style={{overflowY:"auto",padding:"16px 20px",flex:1}}>
        {entries.map((e,i)=>(
          <div key={i} style={{marginBottom:i<entries.length-1?20:0}}>
      ...
      <div style={{padding:"12px 20px",borderTop:`1px solid ${B.gray700}`}}>
        <button onClick={onClose} style={{width:"100%",padding:"11px 0",borderRadius:10,background:B.orange,border:"none",color:B.white,fontWeight:800,fontSize:14,cursor:"pointer"}}>
          Entendido ✓
        </button>
      </div>
    </div>
...
// ─── WebAuthn / Biometria ─────────────────────────────────────────────────────
const WEBAUTHN_KEY = "osc_webauthn_credential";
const WEBAUTHN_ROLE_KEY = "osc_webauthn_role";

function isWebAuthnAvailable() {
  return !!(window.PublicKeyCredential && navigator.credentials);
}

async function webAuthnRegister(role) {
  const challenge = crypto.getRandomValues(new Uint8Array(32));
  const userId = crypto.getRandomValues(new Uint8Array(16));
  try {
    const cred = await navigator.credentials.create({ publicKey: {
      challenge,
      rp: { name: "OSC Performance", id: window.location.hostname },
      user: { id: userId, name: "osc-admin", displayName: "OSC Admin" },
      pubKeyCredParams: [{ type: "public-key", alg: -7 }, { type: "public-key", alg: -257 }],
      authenticatorSelection: { authenticatorAttachment: "platform", userVerification: "required" },
      timeout: 60000,
    }});
    if (!cred) return false;
    localStorage.setItem(WEBAUTHN_KEY, JSON.stringify({
      id: Array.from(new Uint8Array(cred.rawId)),
      type: cred.type,
    }));
    localStorage.setItem(WEBAUTHN_ROLE_KEY, role);
    return true;
...
async function webAuthnAuthenticate() {
  ...
  try {
    ...
    if (!cred) return null;
    return localStorage.getItem(WEBAUTHN_ROLE_KEY);
  } catch(e) { console.warn("WebAuthn auth:", e); return null; }
}

function webAuthnClear() {
  localStorage.removeItem(WEBAUTHN_KEY);
  localStorage.removeItem(WEBAUTHN_ROLE_KEY);
}
...
function AdminLoginScreen({onLogin}) {
  ...
  const [err,setErr]=useState("");
  const [bioAvailable]=useState(isWebAuthnAvailable);
  const [hasBio,setHasBio]=useState(hasWebAuthnCredential);
  ...
  const handleBioLogin=async()=>{
    setBioLoading(true);
    const role=await webAuthnAuthenticate();
    setBioLoading(false);
    if(role) { onLogin(role); }
    else { setErr("Biometria falhou. Use a senha."); setHasBio(false); webAuthnClear(); }
  };

  const acceptBio=async()=>{
    const ok=await webAuthnRegister(pendingRole);
    if(ok) setHasBio(true);
    setOfferBio(false);
    onLogin(pendingRole);
  };

  const skipBio=()=>{ setOfferBio(false); onLogin(pendingRole); };

  // Offer to register biometrics after password login
  if(offerBio) return (
    <div style={{minHeight:"100vh",background:B.black,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter','Segoe UI',sans-serif",padding:20}}>
      <div style={{maxWidth:380,width:"100%",background:B.gray800,borderRadius:20,padding:32,border:`1px solid ${B.gray700}`,textAlign:"center"}}>
        <div style={{fontSize:52,marginBottom:12}}>🔐</div>
        <div style={{fontWeight:800,fontSize:17,color:B.white,marginBottom:8}}>Ativar acesso biométrico?</div>
        <div style={{fontSize:13,color:B.gray400,marginBottom:24,lineHeight:1.6}}>
          Na próxima vez, você pode entrar com<br/>Face ID, Touch ID ou biometria do dispositivo<br/>sem precisar digitar a senha.
        </div>
        <button onClick={acceptBio} style={{width:"100%",padding:"13px 0",borderRadius:11,background:B.orange,border:"none",color:B.white,fontWeight:800,fontSize:14,cursor:"pointer",marginBottom:10}}>
          ✓ Ativar biometria
        </button>
        <button onClick={skipBio} style={{width:"100%",padding:"11px 0",borderRadius:11,background:"none",border:`1px solid ${B.gray600}`,color:B.gray400,fontWeight:600,fontSize:13,cursor:"pointer"}}>
          Agora não
        </button>
      </div>
    </div>
      ...
      <div style={{background:B.gray800,borderRadius:16,padding:24,border:`1px solid ${B.gray700}`}}>
        ...
        {bioAvailable&&hasBio&&<>
          <button onClick={handleBioLogin} disabled={bioLoading}
            style={{width:"100%",padding:"14px 0",borderRadius:11,background:bioLoading?B.gray700:`${B.orange}18`,border:`1px solid ${B.orange}44`,color:bioLoading?B.gray400:B.orange,fontWeight:800,fontSize:15,cursor:bioLoading?"not-allowed":"pointer",marginBottom:16,display:"flex",alignItems:"center",justifyContent:"center",gap:10}}>
            <span style={{fontSize:22}}>{bioLoading?"…":"🔐"}</span>
            {bioLoading?"Verificando biometria…":"Entrar com biometria"}
          </button>
          <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
            <div style={{flex:1,height:1,background:B.gray700}}/>
            <span style={{fontSize:11,color:B.gray500}}>ou use a senha</span>
            <div style={{flex:1,height:1,background:B.gray700}}/>
          </div>
        </>}
        ...
        {bioAvailable&&hasBio&&<button onClick={()=>{webAuthnClear();setHasBio(false);}} style={{width:"100%",marginTop:10,padding:"8px 0",borderRadius:8,background:"none",border:"none",color:B.gray600,fontSize:11,cursor:"pointer"}}>
          Remover biometria salva
        </button>}
...
function PublicVehicleHistoryView({vehicleId,vehicles,tasks,employees,osHistory=[]}) {
  ...
  const renderTaskList=(ts,photos=[])=>{
    ...
    return catOrder.map(cat=>{
      ...
      const catColor=cat?(CAT_MAP_FINISHING[cat]&&!CAT_MAP[cat]?CAT_MAP_FINISHING[cat]:CAT_MAP[cat]||CAT_MAP_FINISHING[cat])||B.gray500:null;
          ...
          {activeVehicles.length===0
            ...
            :<>
              {activeVehicles.length>3&&<div style={{position:"relative",marginBottom:8}}>
                <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar por modelo, placa ou cor…"
        ...
        <div>
          ...
          <div style={{display:"flex",gap:8}}>
            <input ref={inputRef} value={label} onChange={e=>setLabel(e.target.value)}
              ...
              placeholder="Ex: Verificar freio traseiro…"
...
// ─── Finishing Division Checklist ────────────────────────────────────────────
const CHECKLIST_FINISHING = [
  {id:"f_painel",      label:"Painel dianteiro",              level:"basic"},
  {id:"f_capo",        label:"Capô",                          level:"basic"},
  {id:"f_teto",        label:"Teto",                          level:"basic"},
  {id:"f_para_diant",  label:"Para-choque dianteiro",         level:"basic"},
  {id:"f_para_tras",   label:"Para-choque traseiro",          level:"basic"},
  {id:"f_porta_dle",   label:"Porta dianteira esquerda",      level:"basic"},
  {id:"f_porta_dle2",  label:"Porta traseira esquerda",       level:"basic"},
  {id:"f_porta_ddi",   label:"Porta dianteira direita",       level:"basic"},
  {id:"f_porta_ddi2",  label:"Porta traseira direita",        level:"basic"},
  {id:"f_lateral_e",   label:"Lateral esquerda",              level:"basic"},
  {id:"f_lateral_d",   label:"Lateral direita",               level:"basic"},
  {id:"f_porta_malas", label:"Tampa do porta-malas",          level:"basic"},
  {id:"f_roda_dle",    label:"Roda dianteira esquerda",       level:"basic"},
  {id:"f_roda_ddi",    label:"Roda dianteira direita",        level:"basic"},
  {id:"f_roda_te",     label:"Roda traseira esquerda",        level:"basic"},
  {id:"f_roda_td",     label:"Roda traseira direita",         level:"basic"},
  {id:"f_vidros",      label:"Vidros (estado geral)",         level:"advanced"},
  {id:"f_retrovisores",label:"Retrovisores",                  level:"advanced"},
  {id:"f_soleira",     label:"Soleiras",                      level:"advanced"},
  {id:"f_colunas",     label:"Colunas (A, B, C)",             level:"advanced"},
];

// ─── Vehicle Parts List (inline) ─────────────────────────────────────────────
function VehiclePartsList({vehicle, onUpdateVehicle, managerMode=true, isFD=false, tasks=[]}) {
  const partsField = isFD ? "partsListFinishing" : "partsList";
  const parts = Array.isArray(isFD ? vehicle.partsListFinishing : vehicle.partsList)
    ? (isFD ? vehicle.partsListFinishing : vehicle.partsList) : [];
  ...
  const save = (patch) => onUpdateVehicle(vehicle.id, {[partsField]: patch});

  const addPart = () => {
    if(!newName.trim()) return;
    save([...parts, {id: Date.now().toString(), name:newName.trim(), qty:parseInt(newQty)||1, location:newLoc.trim(), note:newNote.trim(), photos:[]}]);
    setNewName(""); setNewQty("1"); setNewLoc(""); setNewNote(""); setAdding(false);
  };

  const removePart = (id) => save(parts.filter(p=>p.id!==id));

  const updatePart = (id, patch) => save(parts.map(p=>p.id===id?{...p,...patch}:p));

  return (<div>
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
      <span style={{fontSize:13,fontWeight:800,color:B.purple}}>🔩 Peças reservadas</span>
      <span style={{fontSize:11,color:B.gray400}}>{parts.length} item{parts.length!==1?"s":""}</span>
      {managerMode&&<button onClick={()=>setAdding(p=>!p)} style={{marginLeft:"auto",padding:"3px 10px",borderRadius:6,background:adding?`${B.purple}22`:`${B.purple}12`,border:`1px solid ${B.purple}44`,color:B.purple,cursor:"pointer",fontSize:11,fontWeight:700}}>
        {adding?"✕ Cancelar":"+ Adicionar"}
      </button>}
    </div>

    {/* Add form */}
    {adding&&managerMode&&<div style={{marginBottom:10,padding:"10px 12px",background:B.gray900,border:`1px solid ${B.purple}33`,borderRadius:8,display:"flex",flexDirection:"column",gap:7}}>
      <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
        <input value={newName} onChange={e=>setNewName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addPart()} placeholder="Nome da peça *"
          style={{flex:"1 1 160px",padding:"7px 10px",borderRadius:7,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none"}}/>
        <input value={newQty} onChange={e=>setNewQty(e.target.value)} type="number" min="1" placeholder="Qtd"
          style={{width:60,padding:"7px 10px",borderRadius:7,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none"}}/>
        <input value={newLoc} onChange={e=>setNewLoc(e.target.value)} placeholder="Localização (prateleira, caixa…)"
          style={{flex:"1 1 160px",padding:"7px 10px",borderRadius:7,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none"}}/>
      </div>
      <input value={newNote} onChange={e=>setNewNote(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addPart()} placeholder="Observação (opcional)"
        style={{width:"100%",padding:"7px 10px",borderRadius:7,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:12,outline:"none",boxSizing:"border-box"}}/>
      <button onClick={addPart} disabled={!newName.trim()} style={{padding:"7px 14px",borderRadius:7,background:newName.trim()?B.purple:B.gray700,border:"none",color:B.white,cursor:newName.trim()?"pointer":"not-allowed",fontWeight:700,fontSize:13,alignSelf:"flex-start"}}>
        Salvar peça
      </button>
    </div>}

    {/* Parts list */}
    {parts.length===0&&!adding&&<div style={{fontSize:12,color:B.gray500,padding:"12px 0",textAlign:"center"}}>Nenhuma peça reservada ainda.</div>}
    <div style={{display:"flex",flexDirection:"column",gap:5}}>
      {parts.map((part,idx)=>{
        const installed=isInstalled(part,idx);
        return (<div key={part.id||idx} style={{padding:"8px 10px",background:installed?`${B.green}08`:B.gray900,border:`1px solid ${installed?B.green+"44":B.gray700}`,borderRadius:8,display:"flex",alignItems:"flex-start",gap:8}}>
          <span style={{fontSize:16,flexShrink:0}}>🔩</span>
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
              {managerMode
                ?<InlineEdit value={part.name} onSave={v=>updatePart(part.id,{name:v})} placeholder="Nome da peça"/>
                :<span style={{fontSize:13,color:B.white,fontWeight:600}}>{part.name}</span>}
              <span style={{fontSize:11,color:B.purple,background:`${B.purple}18`,borderRadius:5,padding:"1px 7px",fontWeight:700,flexShrink:0}}>
                {managerMode
                  ?<span style={{cursor:"pointer"}} onClick={()=>{const q=prompt("Quantidade:",part.qty);if(q)updatePart(part.id,{qty:parseInt(q)||1});}}>{part.qty}×</span>
                  :<span>{part.qty}×</span>}
              </span>
              {installed&&<span style={{fontSize:10,fontWeight:800,color:B.green,background:`${B.green}18`,border:`1px solid ${B.green}33`,borderRadius:5,padding:"1px 7px",flexShrink:0}}>✓ Instalada</span>}
            </div>
            {part.location&&<div style={{fontSize:11,color:B.gray400,marginTop:2}}>📍 {managerMode
              ?<InlineEdit value={part.location} onSave={v=>updatePart(part.id,{location:v})} placeholder="Localização"/>
              :<span>{part.location}</span>}
            </div>}
            {part.note&&<div style={{fontSize:11,color:B.gray500,fontStyle:"italic",marginTop:2}}>{part.note}</div>}
            {/* Photos */}
            <div style={{marginTop:6,display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}}>
              {(part.photos||[]).map((url,pi)=>(
                <div key={pi} style={{position:"relative",width:56,height:56,borderRadius:6,overflow:"hidden",flexShrink:0}}>
                  <img src={url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
                  {managerMode&&<button onClick={()=>updatePart(part.id,{photos:(part.photos||[]).filter((_,i)=>i!==pi)})}
                    style={{position:"absolute",top:2,right:2,background:"rgba(0,0,0,.7)",border:"none",borderRadius:99,width:16,height:16,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",padding:0}}>
                    <IX s={9} c={B.white}/>
                  </button>}
                </div>
              ))}
              {managerMode&&<UploadBtn onFile={url=>{
                updatePart(part.id,{photos:[...(part.photos||[]),url]});
              }} folder={`parts/${vehicle.id}`} label="📷" style={{width:56,height:56,borderRadius:6,fontSize:20,justifyContent:"center",flexDirection:"column"}}/>}
            </div>
          </div>
          {managerMode&&<button onClick={()=>removePart(part.id)} style={{background:"none",border:"none",cursor:"pointer",color:B.gray600,padding:2,flexShrink:0}}
            onMouseEnter={e=>e.currentTarget.style.color=B.red} onMouseLeave={e=>e.currentTarget.style.color=B.gray600}><IX s={13}/></button>}
        </div>);
      })}
    </div>
  </div>);
...
// ─── Vehicle Checklist (inline) ───────────────────────────────────────────────
function VehicleChecklist({vehicle, tasks, onUpdateVehicle, onAddTask, managerMode=true, isFD=false}) {
  const [showAdvanced, setShowAdvanced] = useState(false);
  const [creating, setCreating] = useState(null);
  const [lbPhoto, setLbPhoto] = useState(null);

  const checklistField = isFD ? "checklistFinishing" : "checklist";
  const checklistSource = isFD ? vehicle.checklistFinishing : vehicle.checklist;
  const itemsList = isFD ? CHECKLIST_FINISHING : CHECKLIST_ITEMS;
  const accentColor = isFD ? FD.primary : B.blue;

  const checklist = Array.isArray(checklistSource) ? checklistSource : [];
  const getItem = (id) => checklist.find(c=>c.id===id) || {id, status:null, taskCreated:false};
  const clPhotos = checklist.find(c=>c.id==="__photos")?.urls || [];

  const setStatus = (id, status) => {
    const existing = checklist.filter(c=>c.id!==id);
    const item = getItem(id);
    onUpdateVehicle(vehicle.id, {[checklistField]:[...existing, {...item, id, status}]});
  };

  const savePhotos = (urls) => {
    const rest = checklist.filter(c=>c.id!=="__photos");
  ...
  const createTask = async (item) => {
    setCreating(item.id);
    await onAddTask(item.taskLabel, item.category);
    const existing = checklist.filter(c=>c.id!==item.id);
    const cur = getItem(item.id);
    onUpdateVehicle(vehicle.id, {[checklistField]:[...existing, {...cur, id:item.id, taskCreated:true}]});
    setCreating(null);
  };

  const visibleItems = (isFD || showAdvanced)
    ? itemsList
    : itemsList.filter(i=>i.level==="basic");

  const badCount = checklist.filter(c=>c.status==="bad").length;
  const okCount  = checklist.filter(c=>c.status==="ok").length;
  const advancedBadCount = !isFD ? checklist.filter(c=>c.status==="bad"&&CHECKLIST_ITEMS.find(i=>i.id===c.id)?.level==="advanced").length : 0;

  return (<div>
    {/* Header */}
    <div style={{display:"flex",alignItems:"center",gap:8,marginBottom:12}}>
      <span style={{fontSize:13,fontWeight:800,color:accentColor}}>📋 Checklist {isFD?"estético":"de entrada"}</span>
      <span style={{fontSize:11,color:B.gray400}}>{okCount} ok · {badCount} com problema</span>
      {!isFD&&<button onClick={()=>setShowAdvanced(p=>!p)} style={{marginLeft:"auto",padding:"3px 10px",borderRadius:6,background:showAdvanced?`${B.red}18`:`${B.amber}15`,border:`1px solid ${showAdvanced?B.red+"44":B.amber+"33"}`,color:showAdvanced?B.red:B.amber,cursor:"pointer",fontSize:11,fontWeight:700}}>
        {showAdvanced?"− Recolher avançado":"🔧 + Diagnóstico avançado"}
      </button>}
    </div>

    {/* Items */}
    <div style={{display:"flex",flexDirection:"column",gap:5}}>
      {!showAdvanced&&<div style={{fontSize:9,color:B.gray600,textTransform:"uppercase",letterSpacing:.5,marginBottom:2}}>Inspeção básica</div>}
      {showAdvanced&&<div style={{fontSize:9,color:B.gray600,textTransform:"uppercase",letterSpacing:.5,marginBottom:2}}>Inspeção básica</div>}
      {visibleItems.map((item, idx) => {
        ...
        const isBad = state.status === "bad";
        const isOk  = state.status === "ok";
        const showDivider = showAdvanced && idx === CHECKLIST_ITEMS.filter(i=>i.level==="basic").length;

        return (<div key={item.id}>
          {showDivider&&<div style={{fontSize:9,color:B.red,textTransform:"uppercase",letterSpacing:.5,margin:"8px 0 4px",fontWeight:700}}>🔧 Diagnóstico avançado</div>}
          <div style={{padding:"8px 10px",borderRadius:8,border:`1px solid ${isBad?"#f87171"+"55":isOk?B.green+"33":B.gray700}`,background:isBad?"#f8717108":isOk?`${B.green}06`:B.gray900,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"}}>
            {/* Category dot */}
            <span style={{width:7,height:7,borderRadius:99,background:catColor,flexShrink:0,display:"inline-block"}}/>
            {/* Label */}
            <span style={{fontSize:12,color:B.gray200,flex:"1 1 140px"}}>{item.label}</span>
            {/* Status buttons */}
            <div style={{display:"flex",gap:5,flexShrink:0}}>
              <button onClick={()=>setStatus(item.id, isOk?null:"ok")}
                style={{padding:"3px 10px",borderRadius:6,border:`1px solid ${isOk?B.green+"66":B.gray600}`,background:isOk?`${B.green}22`:"none",color:isOk?B.green:B.gray500,cursor:"pointer",fontSize:11,fontWeight:700}}>
                ✓ Bom
              </button>
              <button onClick={()=>setStatus(item.id, isBad?null:"bad")}
                style={{padding:"3px 10px",borderRadius:6,border:`1px solid ${isBad?"#f87171"+"66":B.gray600}`,background:isBad?"#f8717122":"none",color:isBad?"#f87171":B.gray500,cursor:"pointer",fontSize:11,fontWeight:700}}>
                ✕ Ruim
              </button>
            </div>
            {/* Create task button */}
            {isBad&&managerMode&&<button onClick={()=>createTask(item)} disabled={state.taskCreated||creating===item.id}
              style={{padding:"3px 10px",borderRadius:6,background:state.taskCreated?`${B.green}15`:`${catColor}22`,border:`1px solid ${state.taskCreated?B.green+"44":catColor+"44"}`,color:state.taskCreated?B.green:catColor,cursor:state.taskCreated?"default":"pointer",fontSize:10,fontWeight:700,flexShrink:0,whiteSpace:"nowrap"}}>
              {creating===item.id?"…":state.taskCreated?"✓ Tarefa criada":"+ Criar tarefa"}
            </button>}
          </div>
        </div>);
      })}
      ...
      <div style={{display:"flex",flexWrap:"wrap",gap:8}}>
        {clPhotos.map((url,i)=>(
          <div key={i} style={{position:"relative",width:90,height:90,flexShrink:0}}>
            <div style={{width:90,height:90,borderRadius:8,overflow:"hidden",cursor:"pointer",border:`1px solid ${B.gray600}`}} onClick={()=>setLbPhoto(url)}>
              <img src={url} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
            </div>
            <button onClick={()=>savePhotos(clPhotos.filter((_,j)=>j!==i))}
              style={{position:"absolute",top:3,right:3,width:18,height:18,borderRadius:99,background:"rgba(0,0,0,.75)",border:"none",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",color:B.white,fontSize:10}}>✕</button>
          </div>
        ))}
    ...
    {lbPhoto&&<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.95)",zIndex:200,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setLbPhoto(null)}>
      <img src={lbPhoto} alt="" style={{maxWidth:"95vw",maxHeight:"92vh",objectFit:"contain",borderRadius:8}}/>
      <button onClick={()=>setLbPhoto(null)} style={{position:"fixed",top:16,right:16,background:"rgba(255,255,255,.1)",border:"none",borderRadius:99,padding:10,cursor:"pointer"}}><IX s={18} c={B.white}/></button>
    </div>}
  </div>);
}

// ─── Quick Material Modal ─────────────────────────────────────────────────────
function QuickMatModal({vehicles,tasks,stock=[],onClose,onAddMaterial}) {
  const activeVehicles=vehicles.filter(v=>v.enteredAt||tasks.some(t=>t.vehicleId===v.id));
  const [selectedVid,setSelectedVid]=useState(activeVehicles.length===1?activeVehicles[0].id:"");
  ...
  const [matMode,setMatMode]=useState("manual"); // "manual" | "stock"
  const [stockSearch,setStockSearch]=useState("");
  const [saving,setSaving]=useState(false);
  const nameRef=useRef(null);

  const filteredVehicles=activeVehicles.filter(v=>
    !vSearch||v.model.toLowerCase().includes(vSearch.toLowerCase())||v.plate.toLowerCase().includes(vSearch.toLowerCase())||(v.color||"").toLowerCase().includes(vSearch.toLowerCase())
  );
  const vehicleTasks=tasks.filter(t=>t.vehicleId===selectedVid);
  ...
  const save=async(fromStockItem=null)=>{
    ...
    if(!fromStockItem&&!name.trim()) return;
    setSaving(true);
    const currentMats=tasks.find(t=>t.id===selectedTid)?.materials||[];
    const mat=fromStockItem
      ?{name:fromStockItem.name,brand:fromStockItem.brand||"",cost:fromStockItem.salePrice,qty:qtyNum,freight:0,fromStock:true,stockItemId:fromStockItem.id}
      :{name:name.trim(),brand:brand.trim(),cost:costNum,markup:markupNum,qty:qtyNum,freight:0,fromStock:false};
    await onAddMaterial(selectedTid,mat,currentMats);
    if(!fromStockItem){ setName(""); setBrand(""); setCost(""); setMarkup("50"); setQty("1"); }
    setSaving(false);
  ...
  return (<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:250,padding:16}} onClick={onClose}>
    <div style={{background:B.gray800,borderRadius:16,maxWidth:460,width:"100%",border:`2px solid ${B.blue}55`,overflow:"hidden",maxHeight:"90vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{background:B.gray900,borderBottom:`2px solid ${B.blue}`,padding:"14px 18px",display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        <div style={{width:36,height:36,borderRadius:9,background:`${B.blue}22`,display:"flex",alignItems:"center",justifyContent:"center",fontSize:18,flexShrink:0}}>📦</div>
        <div style={{flex:1}}>
          <div style={{fontWeight:800,fontSize:15,color:B.white}}>Material rápido</div>
          <div style={{fontSize:11,color:B.gray400}}>Adicione sem precisar abrir o veículo</div>
        </div>
        <button onClick={onClose} style={{background:"none",border:"none",cursor:"pointer",color:B.gray400}}><IX s={18}/></button>
      </div>
      ...
      <div style={{padding:18,overflowY:"auto",flex:1,display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div>
          <label style={{fontSize:11,color:B.gray400,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:6,display:"block"}}>Veículo</label>
          {activeVehicles.length>3&&<div style={{position:"relative",marginBottom:8}}>
            <input value={vSearch} onChange={e=>setVSearch(e.target.value)} placeholder="Buscar modelo, placa ou cor…"
              style={{width:"100%",padding:"8px 12px 8px 32px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none",boxSizing:"border-box"}}/>
            <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:B.gray500,fontSize:14}}>🔍</span>
          </div>}
          <div style={{display:"flex",flexDirection:"column",gap:5,maxHeight:150,overflowY:"auto"}}>
            {filteredVehicles.map(v=>(
              <button key={v.id} onClick={()=>{setSelectedVid(v.id);setSelectedTid("");}}
                style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",borderRadius:9,border:`2px solid ${selectedVid===v.id?B.blue:B.gray600}`,background:selectedVid===v.id?`${B.blue}15`:B.gray900,cursor:"pointer",textAlign:"left",width:"100%"}}>
                <span style={{fontSize:15,flexShrink:0}}>🚗</span>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:13,fontWeight:700,color:selectedVid===v.id?B.blue:B.white,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{v.model}</div>
                  <div style={{fontSize:11,color:B.gray400,fontFamily:"monospace"}}>{v.plate}{v.color?` · ${v.color}`:""}</div>
                </div>
                {selectedVid===v.id&&<span style={{color:B.blue,fontWeight:800}}>✓</span>}
              </button>
            ))}
          </div>
        </div>
        ...
        {/* Task selector */}
        {selectedVid&&<div>
          <label style={{fontSize:11,color:B.gray400,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:6,display:"block"}}>Tarefa</label>
          {vehicleTasks.length===0
            ?<div style={{fontSize:13,color:B.gray500,padding:"10px 12px",background:B.gray900,borderRadius:8}}>Nenhuma tarefa neste veículo</div>
            :<div style={{display:"flex",flexDirection:"column",gap:4,maxHeight:130,overflowY:"auto"}}>
              {vehicleTasks.map(t=>(
                <button key={t.id} onClick={()=>setSelectedTid(t.id)}
                  style={{display:"flex",alignItems:"center",gap:8,padding:"8px 12px",borderRadius:8,border:`2px solid ${selectedTid===t.id?B.blue:B.gray600}`,background:selectedTid===t.id?`${B.blue}15`:B.gray900,cursor:"pointer",textAlign:"left",width:"100%"}}>
                  <span style={{fontSize:13,flexShrink:0}}>{t.done?"✅":"⬜"}</span>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,color:selectedTid===t.id?B.blue:B.white,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{t.label}</div>
                    {t.category&&<div style={{fontSize:10,color:CAT_MAP[t.category]||B.gray500,fontWeight:700}}>{t.category}</div>}
                  </div>
                  {selectedTid===t.id&&<span style={{color:B.blue,fontWeight:800,flexShrink:0}}>✓</span>}
                </button>
              ))}
            </div>}
        </div>}
        ...
        {selectedTid&&<>
          {/* Mode toggle */}
          <div style={{display:"flex",gap:6}}>
            <button onClick={()=>setMatMode("manual")} style={{flex:1,padding:"7px 0",borderRadius:7,border:`2px solid ${matMode==="manual"?B.blue:B.gray600}`,background:matMode==="manual"?`${B.blue}22`:"none",color:matMode==="manual"?B.blue:B.gray400,fontWeight:700,fontSize:12,cursor:"pointer"}}>
              ✏️ Manual
            </button>
            <button onClick={()=>setMatMode("stock")} style={{flex:1,padding:"7px 0",borderRadius:7,border:`2px solid ${matMode==="stock"?B.purple:B.gray600}`,background:matMode==="stock"?`${B.purple}22`:"none",color:matMode==="stock"?B.purple:B.gray400,fontWeight:700,fontSize:12,cursor:"pointer"}}>
              🏪 Do estoque
            </button>
          </div>
          ...
          {matMode==="stock"&&<>
            <div style={{position:"relative"}}>
              <input value={stockSearch} onChange={e=>setStockSearch(e.target.value)} placeholder="Buscar no estoque…" autoFocus
                style={{width:"100%",padding:"9px 12px 9px 32px",borderRadius:8,border:`1px solid ${B.purple}44`,background:B.gray900,color:B.white,fontSize:13,outline:"none",boxSizing:"border-box"}}/>
              <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:B.gray500,fontSize:14}}>🔍</span>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:4,maxHeight:160,overflowY:"auto"}}>
              {stock.filter(s=>s.qty>0&&(!stockSearch||s.name.toLowerCase().includes(stockSearch.toLowerCase())||(s.brand||"").toLowerCase().includes(stockSearch.toLowerCase()))).map(s=>(
                <button key={s.id} onClick={()=>save(s)}
                  style={{display:"flex",alignItems:"center",gap:10,padding:"8px 12px",borderRadius:8,border:`1px solid ${B.purple}33`,background:B.purpleBg,cursor:"pointer",textAlign:"left",width:"100%"}}>
                  <IWarehouse s={14} c={B.purple}/>
                  <div style={{flex:1,minWidth:0}}>
                    <div style={{fontSize:13,color:B.purple,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{s.name}</div>
                    {s.brand&&<div style={{fontSize:10,color:B.gray400}}>{s.brand}</div>}
                  </div>
                  <div style={{textAlign:"right",flexShrink:0}}>
                    <div style={{fontSize:12,color:B.amber,fontWeight:700}}>{fmtBRL(s.salePrice)}</div>
                    <div style={{fontSize:10,color:B.gray500}}>Estoque: {s.qty}</div>
                  </div>
                </button>
              ))}
              {stock.filter(s=>s.qty>0&&(!stockSearch||s.name.toLowerCase().includes(stockSearch.toLowerCase()))).length===0&&
                <div style={{fontSize:13,color:B.gray500,padding:"10px 12px"}}>Nenhum item encontrado</div>}
            </div>
          </>}

          {matMode==="manual"&&<>
            <div>
              <label style={{fontSize:11,color:B.gray400,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:6,display:"block"}}>Nome do material</label>
              <input ref={nameRef} value={name} onChange={e=>setName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&save()}
                placeholder="Ex: Filtro de óleo, Correia…"
                style={{width:"100%",padding:"10px 12px",borderRadius:8,border:`1px solid ${name.trim()?B.blue:B.gray600}`,background:B.gray900,color:B.white,fontSize:14,outline:"none",boxSizing:"border-box"}}/>
            </div>
            <div>
              <label style={{fontSize:11,color:B.gray400,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:6,display:"block"}}>Marca (opcional)</label>
              <input value={brand} onChange={e=>setBrand(e.target.value)} placeholder="Ex: Mahle, Bosch, Mann…"
                style={{width:"100%",padding:"10px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:14,outline:"none",boxSizing:"border-box"}}/>
            </div>
            <div style={{display:"flex",gap:10}}>
              <div style={{flex:1}}>
                <label style={{fontSize:11,color:B.gray400,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:6,display:"block"}}>Qtd</label>
                <input value={qty} onChange={e=>setQty(e.target.value)} type="number" min="1"
                  style={{width:"100%",padding:"10px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:14,outline:"none",boxSizing:"border-box"}}/>
              </div>
              <div style={{flex:2}}>
                <label style={{fontSize:11,color:B.gray400,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:6,display:"block"}}>Custo R$</label>
                <input value={cost} onChange={e=>setCost(e.target.value)} placeholder="0,00" type="number"
                  style={{width:"100%",padding:"10px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:14,outline:"none",boxSizing:"border-box"}}/>
              </div>
              <div style={{flex:2}}>
                <label style={{fontSize:11,color:B.gray400,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:6,display:"block"}}>Markup %</label>
                <input value={markup} onChange={e=>setMarkup(e.target.value)} type="number"
                  style={{width:"100%",padding:"10px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:14,outline:"none",boxSizing:"border-box"}}/>
              </div>
            </div>
            {costNum>0&&<div style={{fontSize:12,color:B.amber,background:`${B.amber}12`,borderRadius:7,padding:"6px 10px"}}>
              Venda: <b>{fmtBRL(salePrice)}/un</b>{qtyNum>1?<> · Total: <b>{fmtBRL(salePrice*qtyNum)}</b></>:null}
            </div>}
          </>}
        </>}

        {/* Save (only for manual mode) */}
        {selectedTid&&matMode==="manual"&&<button onClick={()=>save()} disabled={!name.trim()||saving}
          style={{width:"100%",padding:"13px 0",borderRadius:10,background:name.trim()?B.blue:B.gray700,border:"none",color:name.trim()?B.white:B.gray500,fontWeight:800,fontSize:15,cursor:name.trim()?"pointer":"not-allowed",flexShrink:0}}>
          {saving?"Salvando…":"📦 Adicionar material"}
        </button>}
        {selectedTid&&matMode==="manual"&&<div style={{fontSize:11,color:B.gray500,textAlign:"center",marginTop:-8}}>Enter para salvar e adicionar outro na mesma tarefa</div>}
      </div>
    </div>
  </div>);
}
...
// ─── Quick Payment Modal ──────────────────────────────────────────────────────
const PAY_METHODS = ["Dinheiro","PIX","Cartão de Débito","Cartão de Crédito","TED / DOC","Outro"];

function QuickPayModal({vehicles,tasks,clients,onClose,onAddPayment}) {
  const activeVehicles=vehicles.filter(v=>v.enteredAt||v.enteredAtFinishing||tasks.some(t=>t.vehicleId===v.id));
  const [selectedId,setSelectedId]=useState(activeVehicles.length===1?activeVehicles[0].id:"");
  const [payDiv,setPayDiv]=useState("performance");
  const [method,setMethod]=useState("PIX");
  const [amount,setAmount]=useState("");
  const [note,setNote]=useState("");
  const [date,setDate]=useState(new Date().toLocaleDateString("pt-BR"));
  ...
  const client=selectedVehicle?clients.find(c=>c.id===selectedVehicle.clientId):null;

  const save=async()=>{
    if(!selectedId||!amountNum) return;
    setSaving(true);
    const [d,m,y]=date.split("/");
    const paidAt=`${y}-${m?.padStart(2,"0")}-${d?.padStart(2,"0")}T12:00:00`;
    await onAddPayment(selectedId,{amount:amountNum,method,note:note.trim(),paidAt,division:payDiv});
    onClose();
  };
    ...
    <div style={{background:B.gray800,borderRadius:16,maxWidth:440,width:"100%",border:`2px solid ${B.green}55`,overflow:"hidden",maxHeight:"90vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{padding:18,overflowY:"auto",flex:1,display:"flex",flexDirection:"column",gap:14}}>
        ...
        <div>
          <label style={{fontSize:11,color:B.gray400,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:6,display:"block"}}>Veículo</label>
          {activeVehicles.length>3&&<div style={{position:"relative",marginBottom:8}}>
            <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Buscar modelo, placa, cor ou cliente…"
              style={{width:"100%",padding:"8px 12px 8px 32px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none",boxSizing:"border-box"}}/>
            <span style={{position:"absolute",left:10,top:"50%",transform:"translateY(-50%)",color:B.gray500,fontSize:14}}>🔍</span>
          </div>}
          <div style={{display:"flex",flexDirection:"column",gap:5,maxHeight:180,overflowY:"auto"}}>
            {filtered.map(v=>{
              const cli=clients.find(c=>c.id===v.clientId);
              return (<button key={v.id} onClick={()=>setSelectedId(v.id)}
                style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",borderRadius:9,border:`2px solid ${selectedId===v.id?B.green:B.gray600}`,background:selectedId===v.id?`${B.green}15`:B.gray900,cursor:"pointer",textAlign:"left",width:"100%"}}>
                <span style={{fontSize:16,flexShrink:0}}>🚗</span>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{fontSize:13,fontWeight:700,color:selectedId===v.id?B.green:B.white,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{v.model}</div>
                  <div style={{fontSize:11,color:B.gray400}}>{v.plate}{cli?` · ${cli.name}`:""}</div>
                </div>
                {selectedId===v.id&&<span style={{color:B.green,fontWeight:800,flexShrink:0}}>✓</span>}
              </button>);
            })}
        ...
        {/* Method */}
        <div>
          <label style={{fontSize:11,color:B.gray400,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:6,display:"block"}}>Forma de pagamento</label>
          <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
            {PAY_METHODS.map(m=>(
              <button key={m} onClick={()=>setMethod(m)}
                style={{padding:"5px 12px",borderRadius:7,border:`1px solid ${method===m?B.green+"66":B.gray600}`,background:method===m?`${B.green}22`:"none",color:method===m?B.green:B.gray400,cursor:"pointer",fontSize:12,fontWeight:method===m?700:400}}>
                {m}
              </button>
            ))}
          </div>
        ...
        <div style={{display:"flex",gap:10}}>
          <div style={{flex:2}}>
            ...
            <input value={amount} onChange={e=>setAmount(e.target.value)} placeholder="0,00" type="number"
              style={{width:"100%",padding:"10px 12px",borderRadius:8,border:`1px solid ${amountNum>0?B.green:B.gray600}`,background:B.gray900,color:B.white,fontSize:18,fontWeight:700,outline:"none",boxSizing:"border-box"}}/>
          </div>
          <div style={{flex:1}}>
            <label style={{fontSize:11,color:B.gray400,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:6,display:"block"}}>Data</label>
            <input value={date} onChange={e=>setDate(e.target.value)} placeholder="DD/MM/AAAA"
              style={{width:"100%",padding:"10px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none",boxSizing:"border-box"}}/>
          </div>
        </div>

        {/* Note */}
        <div>
          <label style={{fontSize:11,color:B.gray400,fontWeight:700,textTransform:"uppercase",letterSpacing:.5,marginBottom:6,display:"block"}}>Observação (opcional)</label>
          <input value={note} onChange={e=>setNote(e.target.value)} onKeyDown={e=>e.key==="Enter"&&save()} placeholder="Ex: entrada, parcela 1/3…"
            style={{width:"100%",padding:"10px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none",boxSizing:"border-box"}}/>
        </div>

        {/* Save */}
        <button onClick={save} disabled={!selectedId||!amountNum||saving}
          style={{width:"100%",padding:"13px 0",borderRadius:10,background:selectedId&&amountNum?B.green:B.gray700,border:"none",color:selectedId&&amountNum?B.white:B.gray500,fontWeight:800,fontSize:15,cursor:selectedId&&amountNum&&!saving?"pointer":"not-allowed"}}>
          {saving?"Registrando…":`💰 Registrar ${amountNum>0?fmtBRL(amountNum):""}`}
        </button>
      </div>
    </div>
  </div>);
...
// ─── Theme Button (floating, visible everywhere) ─────────────────────────────
function ThemeBtn({toggleTheme, theme, themePref}) {
  const icon = themePref==="auto"
  ...
  const label = themePref==="auto"?"Auto":theme==="dark"?"Escuro":"Claro";
  const color = themePref==="auto"?B.blue:theme==="dark"?B.amber:B.amber;
  return (<button onClick={toggleTheme} title={`Tema: ${label} — toque para alternar`}
    style={{position:"fixed",bottom:"calc(20px + env(safe-area-inset-bottom))",right:16,height:38,borderRadius:99,padding:"0 12px",background:theme==="dark"?"#1a1a1a":"#ffffff",border:`1px solid ${color}44`,display:"flex",alignItems:"center",justifyContent:"center",gap:6,cursor:"pointer",color,boxShadow:"0 4px 16px rgba(0,0,0,.3)",zIndex:999,fontSize:11,fontWeight:700}}>
    {icon}<span>{label}</span>
  </button>);
}

// Global error catcher for non-render errors (event handlers etc.)
function GlobalErrorDisplay() {
  const [err,setErr]=useState(null);
  useEffect(()=>{
    const h=(e)=>{
      // Ignore cross-origin "Script error" — no useful info and not our code
      if(!e.message||e.message==="Script error"||e.message==="Script error.") return;
      setErr(e.message||String(e.error)||"Erro desconhecido");
    };
    window.addEventListener("error",h);
    return()=>window.removeEventListener("error",h);
  },[]);
  if(!err) return null;
  return (<div style={{position:"fixed",top:60,left:16,right:16,zIndex:9999,padding:16,background:"#1a0000",border:"1px solid #f87171",borderRadius:10,color:"#fca5a5",fontFamily:"monospace",fontSize:12}}>
    <div style={{fontWeight:800,color:"#f87171",marginBottom:6}}>⚠ Erro (handler)</div>
    <pre style={{whiteSpace:"pre-wrap",wordBreak:"break-all",margin:0}}>{err}</pre>
    <button onClick={()=>setErr(null)} style={{marginTop:8,padding:"4px 12px",borderRadius:6,background:"#7f1d1d",border:"1px solid #f87171",color:"#fff",cursor:"pointer",fontSize:11}}>Fechar</button>
  </div>);
}
...
class ErrorBoundary extends React.Component {
  ...
  componentDidCatch(e,info) { console.error("OSC Error:", e, info); }
  render() {
    if (this.state.error) return (
      <div style={{padding:24,background:"#1a0000",border:"1px solid #f87171",borderRadius:12,margin:16,color:"#fca5a5",fontFamily:"monospace",fontSize:12}}>
        <div style={{fontWeight:800,color:"#f87171",fontSize:14,marginBottom:8}}>⚠ Erro de renderização</div>
        <pre style={{whiteSpace:"pre-wrap",wordBreak:"break-all"}}>{this.state.error?.message}</pre>
        <button onClick={()=>this.setState({error:null})} style={{marginTop:12,padding:"6px 14px",borderRadius:7,background:"#7f1d1d",border:"1px solid #f87171",color:"#fff",cursor:"pointer"}}>Tentar novamente</button>
      </div>
    );
...
// ─── Tab Section Header ───────────────────────────────────────────────────────
function TabHeader({color,title,subtitle}){
  return(
    <div style={{marginBottom:20}}>
      <div style={{fontSize:20,fontWeight:900,color:B.white,letterSpacing:-.3}}>{title}</div>
      <div style={{fontSize:12,color:color||B.gray400,marginTop:3,fontWeight:500}}>{subtitle}</div>
    </div>
  );
}

// ─── Dashboard Stat Card ──────────────────────────────────────────────────────
function DashCard({color,icon,label,value,sub,onClick}){
  return(
    <button onClick={onClick} style={{background:B.gray800,border:`1px solid ${B.gray700}`,borderRadius:14,padding:"14px 16px",textAlign:"left",cursor:onClick?"pointer":"default",position:"relative",overflow:"hidden",width:"100%"}}>
      <div style={{position:"absolute",top:0,left:0,right:0,height:3,background:color,borderRadius:"14px 14px 0 0",opacity:.7}}/>
      <div style={{width:34,height:34,borderRadius:9,background:`${color}1a`,display:"flex",alignItems:"center",justifyContent:"center",marginBottom:10}}>{icon}</div>
      <div style={{fontSize:20,fontWeight:900,color:B.white,letterSpacing:-.3,marginBottom:2}}>{value}</div>
      <div style={{fontSize:11,fontWeight:700,color:B.gray400,textTransform:"uppercase",letterSpacing:.4}}>{label}</div>
      {sub&&<div style={{fontSize:10,color:B.gray600,marginTop:2}}>{sub}</div>}
    </button>
  );
}

function DashAlert({color,label,sub,icon,onClick}){
  return(
    <div onClick={onClick} style={{display:"flex",alignItems:"center",gap:12,padding:"11px 0",borderBottom:`1px solid ${B.gray700}`,cursor:onClick?"pointer":"default"}}>
      <div style={{width:34,height:34,borderRadius:9,background:`${color}18`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>{icon}</div>
      <div style={{flex:1}}>
        <div style={{fontSize:13,fontWeight:700,color:B.white}}>{label}</div>
        {sub&&<div style={{fontSize:11,color:B.gray400,marginTop:1}}>{sub}</div>}
      </div>
...
function QuickActionSheet({onClose,adminRole,onFuel,onTask,onMat,onPay,onNovaOS,onPedidoOS,onPecasVeiculo,onMatSortidos,theme}){
  ...
  const GROUPS=[
    ...
    ...(adminRole==="owner"?[{label:"Financeiro",color:B.green,actions:[
      ...
      {icon:<Ico d={["M3 22V8a2 2 0 0 1 2-2h10a2 2 0 0 1 2 2v14","M3 22h14","M17 8h2a2 2 0 0 1 2 2v4a2 2 0 0 1-2 2h-2","M7 14v-4","M11 14v-4"]} c={B.amber}/>, color:B.amber, label:"Combustível",    onClick:()=>{onClose();onFuel();}},
    ]}]:[]),
  ];
  return(
...
function SalesTab({shelfItems,sales,stock,onAddShelfItem,onUpdateShelfItem,onDeleteShelfItem,onAddSale,onDeleteSale}){
  ...
  return(<div>
    ...
    {view==="shelf"&&<>
      ...
      {showShelfForm&&createPortal(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:9999,display:"flex",alignItems:"flex-end"}} onClick={()=>setShowShelfForm(false)}>
        <div style={{width:"100%",background:B.gray900,borderRadius:"20px 20px 0 0",padding:"20px 16px",paddingBottom:"calc(40px + env(safe-area-inset-bottom))",border:`1px solid ${B.gray700}`}} onClick={e=>e.stopPropagation()}>
      ...
      {confirmDel&&createPortal(<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.6)",zIndex:9999,display:"flex",alignItems:"center",justifyContent:"center"}} onClick={()=>setConfirmDel(null)}>
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    s.textContent=`
      @media (max-width: 600px) {
        ...
        .osc-topbar-inner { height: auto !important; padding: 10px 14px !important; flex-wrap: wrap; gap: 6px !important; justify-content: center; }
        .osc-topbar-brand { flex: 1 1 100%; justify-content: center; }
        .osc-topbar-icon { display: none !important; }
        .osc-topbar-brand-text { text-align: center; }
        .osc-topbar-actions { margin-top: 4px !important; }
        .osc-topbar-actions { flex-wrap: nowrap !important; margin-left: 0 !important; justify-content: center; width: 100%; }
  ...
  useEffect(()=>{
    ...
    const handler=()=>{
      ...
      setTheme(resolved);
      const bg=resolved==="light"?"#f0f2f5":"#0a0a0a";
      try{
  ...
  const toggleTheme=()=>{
    const next = themePref==="auto"?"dark":themePref==="dark"?"light":"auto";
  ...
  const [quickMatModal,setQuickMatModal]=useState(false);
  ...
  // ── Admin push registration — offer when admin logs in ──
  const [adminPushStatus,setAdminPushStatus]=useState(null);
  useEffect(()=>{
    if(!adminRole||!VAPID_PUBLIC_KEY) return;
    if(!("PushManager" in window)) return;
    navigator.serviceWorker.register("/sw.js",{scope:"/"}).then(()=>{
      getPushSubscription().then(sub=>{
        setAdminPushStatus(sub?"granted":"asking");
      });
    }).catch(()=>{});
  },[adminRole]);
  ...
  const registerAdminPush=async()=>{
    if(!adminRole) return;
    const perm=await Notification.requestPermission();
    if(perm!=="granted"){ setAdminPushStatus("denied"); return; }
    let reg=await navigator.serviceWorker.getRegistration("/sw.js");
    if(!reg) reg=await navigator.serviceWorker.register("/sw.js",{scope:"/"});
    await navigator.serviceWorker.ready;
    const sub=await reg.pushManager.subscribe({userVisibleOnly:true,applicationServerKey:urlBase64ToUint8Array(VAPID_PUBLIC_KEY)});
    await db.saveAdminPushSubscription(adminRole,sub);
    setAdminPushStatus("granted");
  };
  ...
  // ── Realtime — live updates when DB changes ──
  useEffect(()=>{
    const unsub = subscribeToChanges(({table, eventType, new: rec, old: oldRec})=>{
      if(!rec && !oldRec) return;
      const id = rec?.id || oldRec?.id;
      if(table==="tasks"){
        if(eventType==="INSERT") setTsk(p=>[...p.filter(t=>t.id!==rec.id), db._mapTask(rec)]);
        if(eventType==="UPDATE") setTsk(p=>p.map(t=>t.id===id?db._mapTask(rec):t));
        if(eventType==="DELETE") setTsk(p=>p.filter(t=>t.id!==id));
      }
      if(table==="vehicles"){
        if(eventType==="INSERT") setVeh(p=>[...p.filter(v=>v.id!==rec.id), db._mapVehicle(rec)]);
        if(eventType==="UPDATE") setVeh(p=>p.map(v=>{
          if(v.id!==id) return v;
          const mapped=db._mapVehicle(rec);
          // Only preserve mechanicIds from local state if OS is still active
          // If enteredAt was cleared (OS delivered), trust the DB reset
          if(!mapped.enteredAt && !mapped.enteredAtFinishing) return mapped;
          return {...mapped, mechanicIds:v.mechanicIds||[]};
        }));
        if(eventType==="DELETE") setVeh(p=>p.filter(v=>v.id!==id));
      }
      if(table==="payments"){
        if(eventType==="INSERT") setPay(p=>[...p.filter(x=>x.id!==rec.id), db._mapPayment(rec)]);
        if(eventType==="UPDATE") setPay(p=>p.map(x=>x.id===id?db._mapPayment(rec):x));
        if(eventType==="DELETE") setPay(p=>p.filter(x=>x.id!==id));
      }
      if(table==="employees"){
        if(eventType==="INSERT") setEmp(p=>[...p.filter(e=>e.id!==rec.id), db._mapEmployee(rec)]);
        if(eventType==="UPDATE") setEmp(p=>p.map(e=>e.id===id?db._mapEmployee(rec):e));
        if(eventType==="DELETE") setEmp(p=>p.filter(e=>e.id!==id));
      }
      if(table==="stock"){
  ...
  const addEmp=async()=>{
    if(!eN.trim())return;
    try{ const row=await db.addEmployee(eN.trim(),eP.trim(),eDiv); setEmp(p=>[...p,row]); setEN("");setEP("");setEDiv("performance");toast_("Mecânico cadastrado ✓"); }
    catch(e){errToast(e);}
  };
  ...
  const updEmpDiv=async(id,division)=>{
    setEmp(p=>p.map(e=>e.id===id?{...e,division}:e));
    try{ await db.updateEmployee(id,{division}); toast_(`Empresa atualizada ✓`); }catch(e){errToast(e);}
  };
  ...
  // ── Push helper — sends to all mechanics assigned to a vehicle ──
  const pushToVehicleMechs=(vid,title,body,div=null)=>{
    if(!VAPID_PUBLIC_KEY) return;
    const v=vehicles.find(x=>x.id===vid);
    if(!v) return;
    let mechIds=(v.mechanicIds||[]).filter(Boolean);
    if(div==="finishing") mechIds=employees.filter(e=>e.division==="finishing"&&mechIds.includes(e.id)).map(e=>e.id);
    mechIds.forEach(mid=>db.sendPushToEmployee(mid,title,body,"/?portal=mecanico").catch(()=>{}));
  };
  ...
  const deliverVehicle=async(vid)=>{
    ...
    // Snapshot current tasks for history (Performance only)
    const vTasks=tasks.filter(t=>t.vehicleId===vid&&(t.division||"performance")==="performance");
    const tasksValue=vTasks.reduce((s,t)=>s+taskCost(t,defaultRate).total,0);
    const towTotal=(v.tows||[]).reduce((s,t)=>s+Number(t.value||0),0);
    const fuelTotal=(v.fuels||[]).reduce((s,f)=>s+Number(f.value||0),0);
    ...
    const totalValue=tasksValue+Number(v.fuelCost||0)+fuelTotal+towTotal-osDiscountAmt;
    ...
    try{
      ...
      const vModel=v?.model||v?.plate||"Veículo";
      pushToVehicleMechs(vid,`🏁 OS encerrada — ${vModel}`,"O veículo foi entregue ao cliente.","performance");
  ...
  const openNewOS=async(vid,employeeId,enteredAtStr)=>{
    try{
      ...
      const v=vehicles.find(x=>x.id===vid);
      const vModel=v?.model||v?.plate||"Veículo";
      if(employeeId) db.sendPushToEmployee(employeeId,`🔧 Nova OS — ${vModel}`,`${fmtOS(osNumber)} foi aberta para você.`,"/?portal=mecanico").catch(()=>{});
      db.sendPushToAdmins(`🔧 OS aberta — ${vModel}`,`${fmtOS(osNumber)} iniciada.`,"/?").catch(()=>{});
    }catch(e){ errToast(e); }
  };

  const openNewOSFinishing=async(vid,employeeId,enteredAtStr)=>{
    try{
      ...
      const v=vehicles.find(x=>x.id===vid);
      const vModel=v?.model||v?.plate||"Veículo";
      if(employeeId) db.sendPushToEmployee(employeeId,`🎨 Nova OS Finishing — ${vModel}`,`${fmtOSFD(osNumber)} foi aberta para você.`,"/?portal=mecanico").catch(()=>{});
      db.sendPushToAdmins(`🎨 OS Finishing aberta — ${vModel}`,`${fmtOSFD(osNumber)} iniciada.`,"/?").catch(()=>{});
    }catch(e){ errToast(e); }
  };

  const deliverVehicleFinishing=async(vid)=>{
    const v=vehicles.find(x=>x.id===vid);
    if(!v||!v.enteredAtFinishing) return;
    const deliveredAt=new Date().toISOString();
    const finTasks=tasks.filter(t=>t.vehicleId===vid&&t.division==="finishing");
    const finTasksValue=finTasks.reduce((s,t)=>s+taskCost(t,defaultRate).total,0);
    const laborSum=finTasks.reduce((s,t)=>s+taskCost(t,defaultRate).labor,0);
    const osDiscAmt=laborSum*Number(v.osDiscountPctFinishing||0)/100;
    const totalValue=finTasksValue-osDiscAmt;
    const historyRecord={
      osNumber:v.osNumberFinishing, clientId:v.clientId,
      mechanicIds:(v.mechanicIds||[]).filter(id=>{const e=employees.find(e=>e.id===id);return e?.division==="finishing";}),
    ...
    try{
      ...
      const d=await db.loadAll();
      setVeh(d.vehicles); setTsk(d.tasks); setOsHistory(d.osHistory||[]); setPay(d.payments||[]);
      toast_("Finishing encerrada e OS arquivada ✓");
  ...
  const toggleT=async(id, signerEmployeeId)=>{
    ...
    try{
      await db.updateTask(id,{done:nowDone,completedAt,completedByEmployeeId});
      const v=vehicles.find(x=>x.id===t.vehicleId);
      const vModel=v?.model||v?.plate||"Veículo";
      if(nowDone) {
        pushToVehicleMechs(t.vehicleId,`✅ Tarefa concluída — ${vModel}`,t.label,t.division);
  ...
  const delTask=async id=>{
    ...
    try{
      ...
      const v=vehicles.find(x=>x.id===t?.vehicleId);
      const vModel=v?.model||v?.plate||"Veículo";
      if(t) pushToVehicleMechs(t.vehicleId,`🗑 Tarefa removida — ${vModel}`,t.label,t.division);
      if(t) db.sendPushToAdmins(`🗑 Tarefa removida — ${vModel}`,t.label,"/?").catch(()=>{});
      if(t&&v?.clientId) db.sendPushToClient(v.clientId,`🗑 ${vModel}`,`Serviço removido: ${t.label}`,`/?v=${t.vehicleId}`).catch(()=>{});
    }catch(e){errToast(e);}
  };
  const updTask=async(id,patch)=>{
    ...
    try{
      ...
      if(patch.materials){
        ...
        const v=vehicles.find(x=>x.id===t?.vehicleId);
        const vModel=v?.model||v?.plate||"Veículo";
        const matNames=(patch.materials||[]).filter(m=>m.name).map(m=>m.name).join(", ");
        if(matNames) db.sendPushToAdmins(`📦 Material — ${vModel}`,`Materiais atualizados: ${matNames}`,"/?").catch(()=>{});
      }
    }catch(e){errToast(e);}
  };
  ...
  const addPayment=async(p)=>{
    try{
      const row=await db.addPayment(p);
      setPay(prev=>[...prev,row]);
  ...
  const activeVehicleCount=vehicles.filter(v=>v.enteredAt&&v.status!=="ready").length;
  const finishingVehicleCount=vehicles.filter(v=>v.enteredAtFinishing&&v.statusFinishing!=="ready").length;
  const pendingPurchaseCount=purchaseOrders.filter(o=>o.status==="pending"||o.status==="ready_to_buy").length;
  const pendingInvestmentCount=investments.filter(i=>i.status==="pending").length;
  ...
  const SL2=({label})=><div style={{fontSize:10,fontWeight:800,color:B.gray600,textTransform:"uppercase",letterSpacing:1.2,marginBottom:10,marginTop:20}}>{label}</div>;

  // Shared header for all roles
  const DashHeader=({role,onQuick})=>(
    <div style={{marginBottom:20}}>
      <div style={{fontSize:10,fontWeight:800,color:B.gray500,textTransform:"uppercase",letterSpacing:1.5,marginBottom:6}}>{new Date().toLocaleDateString("pt-BR",{weekday:"long",day:"2-digit",month:"long"}).replace(/^\w/,c=>c.toUpperCase())}</div>
      <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <div>
          <div style={{fontSize:22,fontWeight:900,color:B.white,letterSpacing:-.5,marginBottom:4}}>OSC <span style={{color:B.orange}}>Performance</span></div>
          <div style={{fontSize:10,fontWeight:800,color:B.gray500,textTransform:"uppercase",letterSpacing:1.5}}>{ROLE_CONFIG[role]?.label}</div>
        </div>
        {onQuick&&<button onClick={onQuick} style={{width:42,height:42,borderRadius:12,background:`${B.orange}18`,border:`1px solid ${B.orange}33`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",boxShadow:`0 0 16px ${B.orange}20`,flexShrink:0}}>
          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={B.orange} strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>
        </button>}
      </div>
    </div>
  );
  ...
  const pendingNotesCount=vehicles.reduce((s,v)=>{
    const notes=(v.clientVehicleNotes||[]).filter(n=>!n.acknowledged);
    return s+notes.length;
  },0);

  const DashHome=()=>{
    const iconWrench=<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M14.7 6.3a1 1 0 0 0 0 1.4l1.6 1.6a1 1 0 0 0 1.4 0l3.77-3.77a6 6 0 0 1-7.94 7.94l-6.91 6.91a2.12 2.12 0 0 1-3-3l6.91-6.91a6 6 0 0 1 7.94-7.94l-3.76 3.76z"/></svg>;
    const iconCart=<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="9" cy="21" r="1"/><circle cx="20" cy="21" r="1"/><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"/></svg>;
    ...
    const iconPeople=<svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>;
    ...
    const AlertsBlock=()=>{
      const hasPurchases=pendingPurchaseCount>0;
    ...
    if(adminRole==="supervisor"){
      const mechProgress=employees.map(emp=>{
        const vts=vehicles.filter(v=>(v.mechanicIds||[]).includes(emp.id)&&v.enteredAt);
        const tTotal=tasks.filter(t=>vts.some(v=>v.id===t.vehicleId)).length;
        const tDone=tasks.filter(t=>vts.some(v=>v.id===t.vehicleId)&&t.done).length;
        return {emp,vts:vts.length,tDone,tTotal};
      }).filter(m=>m.vts>0||m.tTotal>0);
      return(<div>
        <DashHeader role={adminRole}/>
        <SL2 label="Operação atual"/>
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:4}}>
          <DashCard color={B.orange} icon={iconWrench} label="OSs ativas" value={activeVehicleCount} sub="Performance" onClick={()=>goSection("oficina","clients")}/>
          ...
          <DashCard color={B.blue} icon={iconChart} label="Tarefas abertas" value={tasks.filter(t=>!t.done).length}/>
          <DashCard color={B.green} icon={iconChart} label="Concluídas hoje" value={tasks.filter(t=>t.done&&t.completedAt&&new Date(t.completedAt).toDateString()===new Date().toDateString()).length}/>
        </div>
        {mechProgress.length>0&&<>
          <SL2 label="Progresso da equipe"/>
          {mechProgress.map(({emp,tDone,tTotal})=>{
            const pct=tTotal>0?Math.round(tDone/tTotal*100):0;
            const c=pct===100?B.green:pct>50?B.orange:B.blue;
            return(<div key={emp.id} style={{background:B.gray800,borderRadius:12,padding:"12px 14px",border:`1px solid ${B.gray700}`,marginBottom:8}}>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
                <div style={{fontWeight:700,fontSize:13,color:B.white}}>{emp.name}</div>
                <div style={{fontSize:11,color:B.gray400}}>{tDone}/{tTotal} tarefas</div>
              </div>
              <div style={{height:4,borderRadius:99,background:B.gray700}}><div style={{height:4,borderRadius:99,background:c,width:`${pct}%`,transition:"width .4s"}}/></div>
            </div>);
          })}
        </>}
    ...
    if(adminRole==="admin"){
      return(<div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10,marginBottom:4}}>
          <DashCard color={B.orange} icon={iconWrench} label="OSs ativas" value={activeVehicleCount} sub="Performance" onClick={()=>goSection("oficina","clients")}/>
          <DashCard color={FD.primary} icon={iconWrench} label="OSs ativas" value={finishingVehicleCount} sub="Finishing" onClick={()=>goSection("oficina","finishing")}/>
          ...
          <DashCard color={B.purple} icon={iconBox} label="Itens estoque" value={stock.length} onClick={()=>goSection("gestao","stock")}/>
        </div>
        <SL2 label="Atenção necessária"/>
        <AlertsBlock/>
      </div>);
    ...
    // Owner
    const now=new Date();
    const totalRevenue=payments.filter(p=>{const d=new Date(p.paidAt||p.createdAt);return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear();}).reduce((s,p)=>s+Number(p.amount),0);
    const weekRevenue=payments.filter(p=>{const d=new Date(p.paidAt||p.createdAt);return(now-d)<7*24*60*60*1000;}).reduce((s,p)=>s+Number(p.amount),0);
    const monthTasks=tasks.filter(t=>{if(!t.completedAt) return false;const d=new Date(t.completedAt);return d.getMonth()===now.getMonth()&&d.getFullYear()===now.getFullYear();});
    const monthVehicles=new Set(monthTasks.map(t=>t.vehicleId)).size;
    return(<div>
      <DashHeader role={adminRole} onQuick={()=>setShowQuickSheet(true)}/>
  ...
  const MaisPage=()=>{
    ...
    const clientPortalUrl=`${window.location.origin}/?portal=cliente`;
    const mechPortalUrl=`${window.location.origin}/?portal=mecanico`;
    const Item=({icon,label,sub,color=B.orange,onClick,right})=>(
      <div onClick={onClick} style={{display:"flex",alignItems:"center",gap:14,padding:"14px 16px",background:B.gray800,borderRadius:14,marginBottom:10,border:`1px solid ${B.gray700}`,cursor:onClick?"pointer":"default"}}>
        <div style={{width:40,height:40,borderRadius:12,background:`${color}18`,display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,fontSize:18}}>{icon}</div>
        <div style={{flex:1}}>
          <div style={{fontWeight:700,fontSize:14,color:B.white}}>{label}</div>
          {sub&&<div style={{fontSize:11,color:B.gray400,marginTop:1}}>{sub}</div>}
        </div>
        {right||<span style={{color:B.gray600,fontSize:18}}>›</span>}
      </div>
    );
    return(<div>
      <div style={{fontSize:22,fontWeight:900,color:B.white,letterSpacing:-.5,marginBottom:20}}>Mais</div>
  ...
  return (<div key={theme} style={{height:"100%",overflow:"hidden",background:B.black,fontFamily:"'Inter','Segoe UI',sans-serif",color:B.white,display:"flex"}}>
    ...
    <div className="osc-sidebar" style={{display:"none",flexDirection:"column",width:220,flexShrink:0,background:B.gray900,borderRight:`1px solid ${B.gray700}`,padding:"28px 12px",paddingTop:"calc(28px + env(safe-area-inset-top))",paddingBottom:"calc(12px + env(safe-area-inset-bottom))",gap:4,overflowY:"auto"}}>
      {/* Brand */}
      <div style={{padding:"0 8px",marginBottom:28}}>
        <div style={{fontWeight:900,fontSize:16,color:B.white,letterSpacing:-.3}}>OSC <span style={{color:B.orange}}>Performance</span></div>
        <div style={{fontSize:10,fontWeight:800,color:B.gray500,textTransform:"uppercase",letterSpacing:1.2,marginTop:3}}>{ROLE_CONFIG[adminRole]?.label}</div>
      </div>
      {sidebarItems.map(item=>(
        <button key={item.id} onClick={()=>{
          ...
          background:navSection===item.id?`${B.orange}18`:"transparent",
          color:navSection===item.id?B.orange:B.gray400,
    ...
    <div style={{minHeight:"calc(100vh - env(safe-area-inset-top) - 20px)",background:B.black,paddingBottom:"80px"}}>
      ...
      {adminPushStatus==="asking"&&VAPID_PUBLIC_KEY&&<div style={{marginBottom:14,padding:"10px 14px",background:`${B.orange}12`,border:`1px solid ${B.orange}33`,borderRadius:10,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
        ...
        <button onClick={registerAdminPush} style={{padding:"6px 14px",borderRadius:7,background:B.orange,border:"none",color:B.white,fontWeight:700,fontSize:11,cursor:"pointer",flexShrink:0}}>Ativar</button>
        <button onClick={()=>setAdminPushStatus("denied")} style={{padding:"6px 10px",borderRadius:7,background:"none",border:`1px solid ${B.gray600}`,color:B.gray400,fontSize:11,cursor:"pointer",flexShrink:0}}>Agora não</button>
      </div>}

      {/* ══ HOME DASHBOARD ══ */}
      {navSection==="home"&&<DashHome/>}
              ...
              <div style={{display:"flex",flexDirection:"column",gap:10}}>
                {testVehicles.map(v=>{
                  ...
                  return(<div key={v.id} style={{background:`linear-gradient(135deg,${B.gray900} 0%,${B.green}08 100%)`,borderRadius:14,border:`1px solid ${B.green}33`,overflow:"hidden",boxShadow:`0 0 20px ${B.green}0a`,position:"relative"}}>
                    ...
                    {/* Row 1: photo + model name full width */}
                    <div style={{padding:"14px 16px 10px 20px",display:"flex",alignItems:"center",gap:12,borderBottom:`1px solid ${B.green}11`}}>
                      {v.photo&&<img src={v.photo} alt="" style={{width:44,height:44,borderRadius:9,objectFit:"cover",border:`1px solid ${B.green}44`,flexShrink:0}}/>}
                      <span style={{fontWeight:900,fontSize:17,color:B.white,flex:1,lineHeight:1.2}}>{v.model}</span>
                    </div>
                    {/* Row 2: badges + info + date + button */}
                    <div style={{padding:"10px 16px 14px 20px",display:"flex",flexWrap:"wrap",gap:10,alignItems:"flex-start"}}>
                      {/* Left: badges and info */}
                      <div style={{flex:"1 1 200px",display:"flex",flexDirection:"column",gap:6}}>
                        <div style={{display:"flex",flexWrap:"wrap",gap:5,alignItems:"center"}}>
                          {v.plate&&<span style={{fontSize:11,color:B.gray400,background:B.gray800,borderRadius:5,padding:"1px 7px"}}>{v.plate}</span>}
                        ...
                        <div style={{display:"flex",flexWrap:"wrap",gap:8,fontSize:11,color:B.gray400}}>
                      ...
                      <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:8,flexShrink:0}}>
                        <div style={{textAlign:"right"}}>
                          <div style={{fontSize:10,color:B.gray500}}>Em teste há</div>
                        ...
                        {v.status==="ready"&&adminRole==="owner"&&<button onClick={()=>deliverVehicle(v.id)}
                          style={{padding:"7px 16px",borderRadius:8,background:`${B.green}22`,border:`1px solid ${B.green}44`,color:B.green,fontWeight:800,fontSize:12,cursor:"pointer",display:"flex",alignItems:"center",gap:5}}>
                          <ICar2 s={12} c={B.green}/>Entregar
                        </button>}
                        {v.status==="ready"&&adminRole!=="owner"&&<span style={{fontSize:10,color:B.green,fontWeight:700,display:"flex",alignItems:"center",gap:3}}><ICheck s={10} c={B.green}/>Pronto</span>}
                      </div>
                    </div>
                  </div>);
                })}
      ...
      {tab==="finishing"&&allowedTabs.includes("finishing")&&<>
        ...
        {(()=>{
          ...
          if(activeFin.length===0) return (<div style={{textAlign:"center",padding:"56px 0",color:B.gray400}}>
            <div style={{fontSize:44,marginBottom:12}}>🎨</div>
            <div style={{fontWeight:700,fontSize:15,color:B.gray200,marginBottom:4}}>Nenhuma OS da Finishing em andamento</div>
          </div>);
          if(filtered.length===0&&finSearch) return (<div style={{textAlign:"center",padding:"32px 0",color:B.gray400,fontSize:13}}>Nenhum resultado para "<b style={{color:B.white}}>{finSearch}</b>"</div>);
          const finEmployees=employees.filter(e=>e.division==="finishing");
          const sortedFin=[...filtered].sort((a,b)=>Number(a.sortOrder||0)-Number(b.sortOrder||0));
          return <div style={{display:"flex",flexDirection:"column",gap:8}}>
            {sortedFin.map(v=><VehicleCard key={v.id} vehicle={v} tasks={tasks} employees={finEmployees} clients={clients} stock={stock} defaultRate={defaultRate} managerMode={adminRole==="owner"||adminRole==="admin"}
              onAddTask={addTask} onToggleTask={toggleT} onDeleteTask={delTask} onUpdateTask={updTask} onUpdateVehicle={updVeh} onDeleteVehicle={delVeh}
              onTransferMechanic={xferMech} onTransferOwner={xferOwn}
              onConsumeStock={consumeStock} onReturnStock={returnStock}
              payments={payments} onAddPayment={addPayment} onDeletePayment={deletePayment} onUpdatePayment={updatePayment} company={company}
      ...
      {tab==="purchases"&&allowedTabs.includes("purchases")&&<>
        ...
        <PurchaseOrdersTab orders={purchaseOrders} vehicles={vehicles} employees={employees} tasks={tasks} adminRole={adminRole}
          onUpdate={async(id,patch)=>{
            try{
              const updated=await db.updatePurchaseOrder(id,patch);
              setPurchaseOrders(p=>p.map(o=>o.id===id?updated:o));
              if(patch.status){
                const o=purchaseOrders.find(x=>x.id===id);
                const v=vehicles.find(x=>x.id===o?.vehicleId);
                const st=PO_STATUS[patch.status]||{icon:"↩",label:"Devolvido"};
                if(o?.employeeId) db.sendPushToEmployee(o.employeeId,`${st.icon} Pedido atualizado — ${v?.model||"Veículo"}`,`${o.partName}: ${patch.status==="pending"?"Item devolvido — pedido reiniciado":st.label}`,"/?portal=mecanico").catch(()=>{});
              }
            }catch(err){errToast(err);}
          }}
          onDelete={async id=>{
            try{await db.deletePurchaseOrder(id);setPurchaseOrders(p=>p.filter(o=>o.id!==id));}catch(err){errToast(err);}
          }}
          onReserve={async(o,loc)=>{
            // Add to vehicle's partsList (reserved parts section)
            const v=vehicles.find(x=>x.id===o.vehicleId);
            if(!v) return;
            const salePrice=o.costPrice!=null?o.costPrice*(1+(o.markupPct||30)/100):0;
      ...
      {tab==="investments"&&allowedTabs.includes("investments")&&<>
        ...
        <InvestmentsTab investments={investments.filter(i=>i.category!=="materiais")} adminRole={adminRole}
          ...
          onUpdate={async(id,patch)=>{try{
            ...
            // When marked as bought, add as expense in finance
            if(patch.status==="bought"){
              const inv=investments.find(i=>i.id===id);
              if(inv&&inv.value>0){
                const total=inv.value*(inv.quantity||1);
                const exp=await db.addExpense({
                  name:`Investimento: ${inv.name}`,
                  amount:total,
                  category:"outros",
    ...
    {quickMatModal&&<QuickMatModal vehicles={vehicles} tasks={tasks} stock={stock} onClose={()=>setQuickMatModal(false)} onAddMaterial={async(taskId,mat,currentMats)=>{
      const newMats=[...currentMats,mat];
      setTsk(p=>p.map(t=>t.id===taskId?{...t,materials:newMats}:t));
      try{ await db.updateTask(taskId,{materials:newMats}); toast_(`Material "${mat.name}" adicionado ✓`); }catch(e){errToast(e);}
    }}/>}
    {quickPayModal&&<QuickPayModal vehicles={vehicles} tasks={tasks} clients={clients} onClose={()=>setQuickPayModal(false)} onAddPayment={async(vehicleId,payment)=>{
