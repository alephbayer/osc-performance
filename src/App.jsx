...
async function generateQuotePDF(vehicle, tasks, client, employee, company, defaultRate) {
  ...
  const cQty   = marginX + contentW - 68;
  const cUnit  = marginX + contentW - 42;
  const cTotal = marginX + contentW;
  const cDescW = cQty - marginX - 4;   // usable width for description text
    ...
    <div style={{background:B.gray800,borderRadius:10,border:`1px solid ${B.gray700}`,overflow:"hidden",marginBottom:10}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,background:B.gray700,padding:"10px 14px",cursor:"pointer"}} onClick={()=>setOpen(o=>!o)}>
        ...
        <div style={{display:"flex",gap:5,flexShrink:0}} onClick={e=>e.stopPropagation()}>
          ...
          {!hideManagerButtons&&<button onClick={()=>setConfirmDelV(true)} style={{background:"none",border:"none",cursor:"pointer",color:B.gray600,padding:4,display:"flex"}}
...
const ROLE_CONFIG = {
  owner:      { label:"Gestor",            tabs:["mechanics","clients","stock","vehicles","finance"], envVar:"VITE_OWNER_PASSWORD" },
  admin:      { label:"Administrativo",    tabs:["mechanics","clients","stock","vehicles"],           envVar:"VITE_ADMIN_PASSWORD" },
  supervisor: { label:"Chefe de Oficina",  tabs:["mechanics","stock","vehicles"],                     envVar:"VITE_SUPERVISOR_PASSWORD" },
};
