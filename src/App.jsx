...
function TaskItemMechanic({task,onToggle,onDelete,onUpdate,employees=[]}) {
  ...
  return (<><div style={{padding:"8px 0",borderBottom:`1px solid ${B.gray700}`}}>
    <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
      ...
      {signer&&task.done&&<span style={{fontSize:10,color:B.green,background:B.greenBg,border:`1px solid ${B.green}33`,borderRadius:5,padding:"1px 6px",flexShrink:0,whiteSpace:"nowrap"}}>✓ {signer.name}</span>}
  ...
  return (<>
    <div style={{background:B.gray800,borderRadius:10,border:`1px solid ${B.gray700}`,overflow:"hidden",marginBottom:10}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,background:B.gray700,padding:"10px 14px",cursor:"pointer"}} onClick={()=>setOpen(o=>!o)}>
        ...
        <div style={{display:"flex",gap:5,flexShrink:0}} onClick={e=>e.stopPropagation()}>
          ...
          {!hideManagerButtons&&isOwner&&onSetStatus&&<>
            {vehicle.status!=="paused"&&<button onClick={()=>onSetStatus(vehicle.id,"paused")} style={{background:`${B.amber}22`,border:`1px solid ${B.amber}44`,borderRadius:6,padding:"4px 8px",cursor:"pointer",color:B.amber,display:"flex",alignItems:"center",gap:4,fontSize:11,fontWeight:700}} title="Pausar — aguardando cliente">
              ⏸ Pausar
            </button>}
            {vehicle.status==="paused"&&<button onClick={()=>onSetStatus(vehicle.id,"active")} style={{background:B.greenBg,border:`1px solid ${B.green}44`,borderRadius:6,padding:"4px 8px",cursor:"pointer",color:B.green,display:"flex",alignItems:"center",gap:4,fontSize:11,fontWeight:700}} title="Retomar trabalho">
              ▶ Retomar
            </button>}
            {vehicle.status!=="ready"&&<button onClick={()=>onSetStatus(vehicle.id,"ready")} style={{background:B.blueBg,border:`1px solid ${B.blue}44`,borderRadius:6,padding:"4px 8px",cursor:"pointer",color:B.blue,display:"flex",alignItems:"center",gap:4,fontSize:11,fontWeight:700}} title="Marcar como pronto para entrega">
              ✓ Pronto
            </button>}
            {vehicle.status==="ready"&&<button onClick={()=>onSetStatus(vehicle.id,"active")} style={{background:`${B.orange}22`,border:`1px solid ${B.orange}44`,borderRadius:6,padding:"4px 8px",cursor:"pointer",color:B.orange,display:"flex",alignItems:"center",gap:4,fontSize:11,fontWeight:700}}>
              ↩ Reabrir
            </button>}
          </>}
      ...
      {open&&<div style={{padding:"12px 14px"}}>
        ...
        {/* Mechanic chips (manager mode) */}
        {managerMode&&mechs.length>0&&<div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:8}}>
          {mechs.map(m=><div key={m.id} style={{display:"flex",alignItems:"center",gap:5,background:`${B.orange}18`,border:`1px solid ${B.orange}44`,borderRadius:6,padding:"3px 8px"}}>
            <IWrench s={11} c={B.orange}/>
            <span style={{fontSize:11,color:B.orange,fontWeight:600}}>{m.name}</span>
            {onRemoveMechanic&&<button onClick={()=>onRemoveMechanic(vehicle.id,m.id)} style={{background:"none",border:"none",cursor:"pointer",color:`${B.orange}88`,padding:0,display:"flex",marginLeft:2}}
              onMouseEnter={e=>e.currentTarget.style.color=B.red} onMouseLeave={e=>e.currentTarget.style.color=`${B.orange}88`}><IX s={10}/></button>}
          </div>)}
        </div>}
...
function EmployeeCard({employee,vehicles,tasks,employees,clients,stock,defaultRate,onAddVehicle,onDeleteVehicle,onTransferMechanic,onTransferOwner,onAddTask,onToggleTask,onDeleteTask,onUpdateTask,onUpdateVehicle,onConsumeStock,onReturnStock,onDelete,onSendWA,onUpdatePhone,onUpdateName,onAddMechanic,onRemoveMechanic,onSetStatus,isOwner=false}) {
  ...
  const empV=[...vehicles.filter(v=>(v.mechanicIds||[v.employeeId]).includes(employee.id))]
