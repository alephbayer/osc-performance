...
const taskCost=(t,defaultRate)=>{
  ...
  const mat   = mats.reduce((s,m)=>s+Number(m.cost||0),0);
...
async function generateQuotePDF(vehicle, tasks, client, employee, company, defaultRate) {
  ...
  const orange = [255,107,0], black = [10,10,10], gray = [110,110,110], lightGray = [240,240,240], green=[22,163,74];
  ...
  doc.text(company?.name || "OSC Performance", marginX, 16);
  ...
  if (company?.document) contactBits.push(`CNPJ: ${company.document}`);
  if (contactBits.length) headerLines.push(contactBits.join("   "));
  headerLines.forEach((line,i)=>doc.text(line, marginX, 22 + i*5));
  ...
  doc.text(`Emitido em ${fmtD()}`, pageW - marginX, y, { align: "right" });
  y += 8;

  // ── Client + Vehicle info box ──
  doc.setDrawColor(220,220,220);
  doc.setFillColor(...lightGray);
  doc.roundedRect(marginX, y, pageW - marginX*2, 24, 2, 2, "F");
  doc.setFontSize(9); doc.setTextColor(...gray);
  doc.text("CLIENTE", marginX+5, y+6);
  doc.text("VEÍCULO", pageW/2+3, y+6);
  doc.setFont("helvetica","bold"); doc.setFontSize(11); doc.setTextColor(...black);
  doc.text(client?.name || "Não informado", marginX+5, y+12);
  doc.text(`${vehicle.model}`, pageW/2+3, y+12);
  doc.setFont("helvetica","normal"); doc.setFontSize(9); doc.setTextColor(...gray);
  doc.text(client?.phone ? `Tel: ${client.phone}` : "", marginX+5, y+18);
  doc.text(`Placa: ${vehicle.plate}`, pageW/2+3, y+18);
  if (employee?.name) doc.text(`Mecânico: ${employee.name}`, pageW/2+3+55, y+18);
  y += 32;

  // ── Table header ──
  const colDesc = marginX, colQty = pageW-marginX-70, colUnit = pageW-marginX-45, colTotal = pageW-marginX;
  const drawTableHeader = () => {
    doc.setFillColor(...orange);
    doc.rect(marginX, y, pageW-marginX*2, 8, "F");
    doc.setTextColor(255,255,255); doc.setFont("helvetica","bold"); doc.setFontSize(9);
    doc.text("DESCRIÇÃO", colDesc+3, y+5.5);
    doc.text("QTD/HORAS", colQty, y+5.5, { align: "right" });
    doc.text("VALOR UNIT.", colUnit, y+5.5, { align: "right" });
    doc.text("TOTAL", colTotal-1, y+5.5, { align: "right" });
    y += 8;
  };
  ...
  ts.forEach((t, idx) => {
    ...
    const rowH = 7 + ((t.materials||[]).length * 5.5);
    doc.rect(marginX, y, pageW-marginX*2, rowH, "S");

    doc.setFont("helvetica","bold"); doc.setFontSize(9.5); doc.setTextColor(...black);
    const labelLines = doc.splitTextToSize(t.label, colQty-colDesc-50);
    doc.text(labelLines[0]||t.label, colDesc+3, y+5);
    if (t.hours>0) doc.text(`${t.hours}h`, colQty, y+5, { align: "right" });
    const rate = t.ratePerHour!=null ? t.ratePerHour : defaultRate;
    doc.setFont("helvetica","normal");
    doc.text(t.hours>0 ? fmtBRL(rate) : "—", colUnit, y+5, { align: "right" });
    doc.setFont("helvetica","bold");
    doc.text(fmtBRL(c.labor), colTotal-1, y+5, { align: "right" });
    let innerY = y + 5;
    ...
    (t.materials||[]).forEach(m => {
      ...
      doc.text(`· ${m.name}${m.fromStock?" (estoque)":""}`, colDesc+7, innerY);
      doc.text("1", colQty, innerY, { align: "right" });
      doc.text(fmtBRL(m.cost||0), colUnit, innerY, { align: "right" });
      doc.text(fmtBRL(m.cost||0), colTotal-1, innerY, { align: "right" });
    });
  ...
  // ── Totals box ──
  const boxW = 80, boxX = pageW - marginX - boxW;
  doc.setDrawColor(220,220,220);
  doc.setFillColor(250,250,250);
  doc.roundedRect(boxX, y, boxW, 32, 2, 2, "FD");
  doc.setFontSize(9.5);
  doc.setFont("helvetica","normal"); doc.setTextColor(...gray);
  doc.text("Total de Peças/Materiais", boxX+5, y+8);
  doc.setFont("helvetica","bold"); doc.setTextColor(...black);
  doc.text(fmtBRL(partsTotal), boxX+boxW-5, y+8, { align: "right" });

  doc.setFont("helvetica","normal"); doc.setTextColor(...gray);
  doc.text("Total de Mão de Obra", boxX+5, y+15);
  doc.setFont("helvetica","bold"); doc.setTextColor(...black);
  doc.text(fmtBRL(laborTotal), boxX+boxW-5, y+15, { align: "right" });

  doc.setDrawColor(...orange);
  doc.line(boxX+5, y+19, boxX+boxW-5, y+19);

  doc.setFillColor(...orange);
  doc.roundedRect(boxX, y+21, boxW, 11, 2, 2, "F");
  doc.setFont("helvetica","bold"); doc.setFontSize(11); doc.setTextColor(255,255,255);
  doc.text("TOTAL GERAL", boxX+5, y+27.5);
  doc.text(fmtBRL(partsTotal+laborTotal), boxX+boxW-5, y+27.5, { align: "right" });
  ...
  doc.text("Este orçamento é uma estimativa e pode sofrer alterações conforme a necessidade do serviço.", marginX, y);
  doc.text(`${company?.name || "OSC Performance"} — Excelência em cada revisão`, marginX, y+5);
...
function MaterialChip({mat,idx,onUpdate,onRemove,showCost=false,readOnlyName=false}) {
  return (<div style={{display:"flex",alignItems:"center",gap:5,background:mat.fromStock?B.purpleBg:B.gray700,border:mat.fromStock?`1px solid ${B.purple}44`:"none",borderRadius:6,padding:"4px 9px"}}>
    {mat.fromStock?<IWarehouse s={12} c={B.purple}/>:<IBox s={12} c={B.gray400}/>}
    {readOnlyName||mat.fromStock
      ?<span style={{fontSize:12,color:mat.fromStock?B.purple:B.gray200,fontWeight:mat.fromStock?600:400}}>{mat.name}</span>
    ...
    {showCost&&mat.fromStock&&<span style={{fontSize:11,color:B.purple,marginLeft:4,paddingLeft:6,borderLeft:`1px solid ${B.purple}44`}}>{fmtBRL(mat.cost)}</span>}
...
function TaskItemMechanic({task,onToggle,onDelete,onUpdate}) {
  ...
  const addMat=()=>{ if(!newMat.trim())return; onUpdate(task.id,{materials:[...mats,{name:newMat.trim(),cost:0,fromStock:false,stockItemId:null}]}); setNewMat(""); };
  ...
  return (<div style={{padding:"8px 0",borderBottom:`1px solid ${B.gray700}`}}>
    ...
    <div style={{marginLeft:30,marginTop:6}}>
      {/* materials list */}
      <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:mats.length?6:0}}>
        {mats.map((m,idx)=><MaterialChip key={idx} mat={m} idx={idx} onUpdate={updMat} onRemove={rmMat} readOnlyName/>)}
      </div>
  ...
  return (<>
    <div style={{padding:"8px 0",borderBottom:`1px solid ${B.gray700}`}}>
      ...
      <div style={{marginLeft:30,marginTop:7}}>
        {/* materials list */}
        <div style={{display:"flex",gap:6,flexWrap:"wrap",marginBottom:mats.length?6:0}}>
          {mats.map((m,idx)=><MaterialChip key={idx} mat={m} idx={idx} onUpdate={updMat} onRemove={rmMat} showCost/>)}
        </div>
...
// ─── VehicleCard ──────────────────────────────────────────────────────────────
function VehicleCard({vehicle,tasks,employees,clients,stock,defaultRate,managerMode,onAddTask,onToggleTask,onDeleteTask,onUpdateTask,onDeleteVehicle,onTransferMechanic,onTransferOwner,onUpdateVehicle,onConsumeStock,onReturnStock,hideManagerButtons=false,payments=[],onAddPayment,onDeletePayment,company}) {
    ...
    <div style={{background:B.gray800,borderRadius:10,border:`1px solid ${B.gray700}`,overflow:"hidden",marginBottom:10}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,background:B.gray700,padding:"10px 14px",cursor:"pointer"}} onClick={()=>setOpen(o=>!o)}>
        ...
        <div style={{display:"flex",gap:5,flexShrink:0}} onClick={e=>e.stopPropagation()}>
          {managerMode&&<button onClick={doPDF} disabled={pdfLoading} title="Gerar PDF do orçamento" style={{background:pdfLoading?B.gray700:`${B.amber}22`,border:`1px solid ${B.amber}44`,borderRadius:6,padding:"4px 8px",cursor:pdfLoading?"wait":"pointer",color:pdfLoading?B.gray400:B.amber,display:"flex",alignItems:"center",gap:4,fontSize:11,fontWeight:700}}>
...
export default function App() {
  ...
  if(isMechanicPortal){
    ...
    return <MechanicPortal employee={liveEmp} vehicles={vehicles} tasks={tasks} employees={employees} clients={clients} stock={stock}
      ...
      onToggleTask={async id=>{const t=tasks.find(x=>x.id===id);const nowDone=!t.done;const completedAt=nowDone?new Date().toISOString():null;setTsk(p=>p.map(t=>t.id===id?{...t,done:nowDone,completedAt}:t));try{await db.updateTask(id,{done:nowDone,completedAt});}catch(e){errToast(e);}}}
  ...
  const toggleT=async id=>{
    ...
    setTsk(p=>p.map(t=>t.id===id?{...t,done:nowDone,completedAt}:t));
    try{ await db.updateTask(id,{done:nowDone,completedAt}); }catch(e){errToast(e);}
  };
  ...
  const consumeStock=async(taskId,item,currentMats)=>{
    ...
    const newMats = [...mats,{name:item.name,cost:item.salePrice,fromStock:true,stockItemId:item.id}];
  ...
  // Removes one specific material (by index) from a task's list and returns it to stock if applicable
