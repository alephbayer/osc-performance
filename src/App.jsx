...
function VehicleCard({vehicle,tasks,employees,clients,stock,defaultRate,managerMode,onAddTask,onToggleTask,onDeleteTask,onUpdateTask,onDeleteVehicle,onTransferMechanic,onTransferOwner,onUpdateVehicle,onConsumeStock,onReturnStock,hideManagerButtons=false,payments=[],onAddPayment,onDeletePayment,company,onAddMechanic,onRemoveMechanic,onSetStatus,onDeliver,isOwner=false}) {
  ...
  return (<>
    <div style={{background:B.gray800,borderRadius:10,border:`1px solid ${B.gray700}`,overflow:"hidden",marginBottom:10}}>
      ...
      <div style={{padding:"6px 10px",background:B.gray800,borderBottom:`1px solid ${B.gray700}66`,display:"flex",flexWrap:"wrap",gap:4}} onClick={e=>e.stopPropagation()}>
        ...
        {!hideManagerButtons&&isOwner&&onSetStatus&&<>
          ...
          {vehicle.deliveredAt&&<span style={{fontSize:10,color:B.gray400,padding:"4px 6px",flex:"0 0 auto"}}>
            Entregue em {new Date(vehicle.deliveredAt).toLocaleDateString("pt-BR")}
          </span>}
        </>}
    ...
    {confirmDeliver&&<ConfirmModal title="Entregar veículo?" danger={false} message={<>Confirmar a entrega de <b style={{color:B.white}}>{vehicle.model} — {vehicle.plate}</b>? O timer total será encerrado e o veículo será movido para a seção de entregues.</>} confirmLabel="Confirmar entrega" onConfirm={()=>{onDeliver&&onDeliver(vehicle.id);setConfirmDeliver(false);}} onCancel={()=>setConfirmDeliver(false)}/>}
  </>);
}
...
function VehiclesTab({vehicles,tasks,employees,clients,defaultRate,onUpdateVehicle}) {
  ...
  // Split: delivered vs in-progress
  const inProgress=[...vehicles.filter(v=>!v.deliveredAt&&matches(v))]
    .sort((a,b)=>{
  ...
  const delivered=[...vehicles.filter(v=>v.deliveredAt&&matches(v))]
  ...
  return (<div>
    ...
    <div style={{display:"flex",gap:8,marginBottom:16,alignItems:"center"}}>
      ...
      <div style={{marginLeft:"auto",fontSize:11,color:B.gray400}}>{inProgress.length} em andamento · {delivered.length} entregue{delivered.length!==1?"s":""}</div>
    </div>

    {/* ── Em andamento ── */}
    <div style={{fontSize:11,color:B.orange,fontWeight:700,marginBottom:8,textTransform:"uppercase",letterSpacing:.5}}>🔧 Em andamento ({inProgress.length})</div>
    {inProgress.length===0&&<div style={{textAlign:"center",padding:"32px 0",color:B.gray400,marginBottom:24}}>
        <div style={{fontSize:32,marginBottom:8}}>🚗</div>
        <div style={{fontWeight:700,color:B.gray200,marginBottom:4}}>{search?"Nenhum veículo encontrado":"Nenhum veículo em andamento"}</div>
        <div style={{fontSize:13}}>Veículos são criados na aba <b style={{color:B.orange}}>Mecânicos</b>.</div>
      </div>}
    {inProgress.length>0&&<div style={{display:"flex",flexDirection:"column",gap:10,marginBottom:24}}>
      {inProgress.map(v=><VehicleHistoryCard key={v.id} vehicle={v} tasks={tasks} employees={employees} clients={clients} defaultRate={defaultRate} onUpdateVehicle={onUpdateVehicle} now={now}/>)}
    </div>}

    {/* ── Entregues ── */}
    <div style={{borderTop:`1px solid ${B.gray700}`,paddingTop:16}}>
      <button onClick={()=>setShowDelivered(p=>!p)} style={{display:"flex",alignItems:"center",gap:8,background:"none",border:"none",cursor:"pointer",padding:0,marginBottom:showDelivered?12:0}}>
        <div style={{fontSize:11,color:B.green,fontWeight:700,textTransform:"uppercase",letterSpacing:.5}}>✅ Entregues ({delivered.length})</div>
        <div style={{color:B.gray400}}>{showDelivered?<IChevU s={13}/>:<IChevD s={13}/>}</div>
      </button>
      {showDelivered&&delivered.length===0&&<div style={{textAlign:"center",padding:"20px 0",color:B.gray400,fontSize:13}}>Nenhum veículo entregue ainda.</div>}
      {showDelivered&&delivered.length>0&&<div style={{display:"flex",flexDirection:"column",gap:10}}>
        {delivered.map(v=><VehicleHistoryCard key={v.id} vehicle={v} tasks={tasks} employees={employees} clients={clients} defaultRate={defaultRate} onUpdateVehicle={onUpdateVehicle} now={now}/>)}
      </div>}
    </div>
  </div>);
}
...
function VehicleHistoryCard({vehicle,tasks,employees,clients,defaultRate,onUpdateVehicle,now}) {
  ...
  const isDelivered = !!vehicle.deliveredAt;

  // Timer: if delivered, use deliveredAt as end; otherwise use now
  const timerEnd = isDelivered ? new Date(vehicle.deliveredAt).getTime() : now;
  const elapsed = vehicle.enteredAt ? elapsedTime(vehicle.enteredAt, timerEnd) : null;
  // Effective work time = total − paused time
  const totalMs = vehicle.enteredAt ? timerEnd - new Date(vehicle.enteredAt).getTime() : 0;
  const pausedMs = vehicle.totalPausedMs || 0;
  const workMs = Math.max(0, totalMs - pausedMs);
  const workElapsed = workMs > 0 ? formatDuration(workMs) : null;
      ...
      <div style={{flexShrink:0,textAlign:"right",marginRight:4}}>
        {isDelivered
          ?<div style={{background:`${B.green}18`,border:`1px solid ${B.green}44`,borderRadius:8,padding:"4px 10px",textAlign:"center"}}>
              ...
              {elapsed&&<div style={{fontSize:12,fontWeight:700,color:B.green}}>{elapsed.label} total</div>}
              {workElapsed&&<div style={{fontSize:10,color:B.gray400}}>~{workElapsed.label} em serviço</div>}
            </div>
          :elapsed
            ?<div style={{background:`${elapsed.color}18`,border:`1px solid ${elapsed.color}44`,borderRadius:8,padding:"4px 10px",textAlign:"center"}}>
                ...
                {workElapsed&&pausedMs>0&&<div style={{fontSize:10,color:B.gray400}}>{workElapsed.label} em serviço</div>}
              </div>
        ...
        {editEntry&&!isDelivered
          ...
          :<div style={{display:"flex",flexDirection:"column",gap:4}}>
              <div style={{display:"flex",alignItems:"center",gap:10}}>
                ...
                {!isDelivered&&elapsed&&<span style={{fontSize:12,fontWeight:700,color:elapsed.color}}>— {elapsed.label} atrás</span>}
                {!isDelivered&&<button onClick={()=>{setEntryInput(new Date(vehicle.enteredAt).toISOString().slice(0,16));setEditEntry(true);}} style={{marginLeft:"auto",background:"none",border:"none",cursor:"pointer",color:B.gray500,display:"flex",alignItems:"center",gap:3,fontSize:11}}>
                  <IEdit s={11}/>Editar
                </button>}
              </div>
              {isDelivered&&vehicle.deliveredAt&&<div style={{display:"flex",alignItems:"center",gap:8,marginTop:2}}>
                <span style={{fontSize:10,color:B.green,fontWeight:700}}>✅ Entregue em:</span>
                <span style={{fontSize:12,color:B.green,fontWeight:600}}>
                  {new Date(vehicle.deliveredAt).toLocaleString("pt-BR",{day:"2-digit",month:"2-digit",year:"numeric",hour:"2-digit",minute:"2-digit"})}
                </span>
                {elapsed&&<span style={{fontSize:11,color:B.gray400}}>· {elapsed.label} total</span>}
              </div>}
            </div>}
      </div>}

      {!vehicle.enteredAt&&!editEntry&&!isDelivered&&<button onClick={()=>setEditEntry(true)} style={{marginBottom:14,width:"100%",padding:"8px 0",borderRadius:8,background:"transparent",border:`1px dashed ${B.orange}66`,color:B.orange,cursor:"pointer",fontWeight:600,fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
...
export default function App() {
  ...
  const deliverVehicle=async(vid)=>{
    ...
    // If currently paused, accumulate final pause time
    let totalPausedMs=v.totalPausedMs||0;
    if(v.status==="paused"&&v.pausedAt){
      totalPausedMs+=nowMs-new Date(v.pausedAt).getTime();
    }
    const patch={status:"ready",deliveredAt:now,pausedAt:null,totalPausedMs};
    setVeh(p=>p.map(x=>x.id===vid?{...x,...patch}:x));
