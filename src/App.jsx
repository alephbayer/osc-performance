...
const taskCost=(t,defaultRate)=>{
  ...
  const mat   = Number(t.materialCost||0);
...
function waMechanic(emp,vehicles,tasks,clients) {
  ...
  vehicles.filter(v=>v.employeeId===emp.id).forEach((v,i)=>{
    ...
    ts.forEach(t=>{
      ...
      if(t.material) rows.push(`     🔩 ${t.material}${t.fromStock?" (estoque)":""}`);
...
function waClient(cli,vehicles,tasks,employees,defaultRate) {
  ...
  cliV.forEach(v=>{
    ...
    ts.forEach(t=>{ const c=taskCost(t,defaultRate); vt+=c.total; grand+=c.total;
      rows.push(`  ${t.done?"✅":"⬜"} ${t.label}`);
      if(t.material) rows.push(`     🔩 ${t.material} — ${fmtBRL(c.mat)}`);
...
function waVehicle(v,tasks,employees,clients,defaultRate) {
  ...
  ts.forEach(t=>{
    rows.push(`  ${t.done?"✅":"⬜"} ${t.label}`);
    if(t.material) rows.push(`     🔩 ${t.material}`);
  });
...
// ─── TaskItem — Mechanic ──────────────────────────────────────────────────────
function TaskItemMechanic({task,onToggle,onDelete,onUpdate}) {
  return (<div style={{padding:"8px 0",borderBottom:`1px solid ${B.gray700}`}}>
    <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
      ...
      <span style={{flex:1,fontSize:13.5,color:task.done?B.gray400:B.gray200,textDecoration:task.done?"line-through":"none",paddingTop:1}}>{task.label}</span>
      <button onClick={()=>onDelete(task.id)} style={{background:"none",border:"none",cursor:"pointer",color:B.gray600,padding:2,display:"flex",flexShrink:0}}
        onMouseEnter={e=>e.currentTarget.style.color=B.red} onMouseLeave={e=>e.currentTarget.style.color=B.gray600}><ITrash s={14}/></button>
    </div>
    <div style={{display:"flex",gap:8,marginTop:6,marginLeft:30,flexWrap:"wrap"}}>
      {/* material display — read only if from stock */}
      <div style={{display:"flex",alignItems:"center",gap:5,background:B.gray700,borderRadius:6,padding:"4px 9px",flex:"1 1 160px"}}>
        <IBox s={12} c={task.fromStock?B.purple:B.gray400}/>
        {task.fromStock
          ?<span style={{fontSize:12,color:B.purple,fontWeight:600}}>{task.material} <span style={{color:B.gray400,fontWeight:400}}>(estoque)</span></span>
          :<InlineEdit value={task.material} onSave={v=>onUpdate(task.id,{material:v})} placeholder="+ Material avulso"/>}
      </div>
      {/* hours */}
...
// ─── TaskItem — Manager ───────────────────────────────────────────────────────
function TaskItemManager({task,defaultRate,stock,onToggle,onDelete,onUpdate,onConsumeStock,onReturnStock}) {
  ...
  return (<>
    <div style={{padding:"8px 0",borderBottom:`1px solid ${B.gray700}`}}>
      ...
      <div style={{display:"flex",gap:7,marginTop:7,marginLeft:30,flexWrap:"wrap"}}>
        {/* Material — stock or manual */}
        {task.fromStock
          ?<div style={{display:"flex",alignItems:"center",gap:5,background:B.purpleBg,border:`1px solid ${B.purple}44`,borderRadius:6,padding:"4px 9px",flex:"1 1 160px"}}>
              <IWarehouse s={12} c={B.purple}/>
              <span style={{fontSize:12,color:B.purple,fontWeight:600,flex:1}}>{task.material}</span>
              <button onClick={()=>onReturnStock(task.id)} title="Devolver ao estoque" style={{background:"none",border:"none",cursor:"pointer",color:B.gray500,padding:2,display:"flex"}}
        ...
        {/* Material cost */}
        <div style={{display:"flex",alignItems:"center",gap:5,background:B.amberBg,border:`1px solid ${B.amber}44`,borderRadius:6,padding:"4px 9px",minWidth:110}}>
          <IMoney s={12} c={B.amber}/>
          <InlineEdit value={task.materialCost?fmtR2(task.materialCost):""} onSave={v=>onUpdate(task.id,{materialCost:parseFloat(v.replace(",","."))||0})} placeholder="Custo mat." type="number"/>
        </div>
        {/* Hours */}
    ...
    {showSP&&<StockPickerModal stock={stock} onPick={item=>{onConsumeStock(task.id,item);setSP(false);}} onClose={()=>setSP(false)}/>}
  </>);
}

// ─── VehicleCard ──────────────────────────────────────────────────────────────
function VehicleCard({vehicle,tasks,employees,clients,stock,defaultRate,managerMode,onAddTask,onToggleTask,onDeleteTask,onUpdateTask,onDeleteVehicle,onTransferMechanic,onTransferOwner,onUpdateVehicle,onConsumeStock,onReturnStock,hideManagerButtons=false,payments=[],onAddPayment,onDeletePayment}) {
  ...
  const mech  = employees.find(e=>e.id===vehicle.employeeId);
  ...
  return (<>
    <div style={{background:B.gray800,borderRadius:10,border:`1px solid ${B.gray700}`,overflow:"hidden",marginBottom:10}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:10,background:B.gray700,padding:"10px 14px",cursor:"pointer"}} onClick={()=>setOpen(o=>!o)}>
        ...
        <div style={{display:"flex",gap:5,flexShrink:0}} onClick={e=>e.stopPropagation()}>
          {managerMode&&<button onClick={()=>setSA(true)} title="Conta corrente / pagamentos" style={{background:B.greenBg,border:`1px solid ${B.green}44`,borderRadius:6,padding:"4px 8px",cursor:"pointer",color:B.green,display:"flex",alignItems:"center",gap:4,fontSize:11,fontWeight:700}}>
            <IBank s={12} c={B.green}/>Conta
          </button>}
          {managerMode&&<button onClick={copyLink} title="Copiar link do cliente" style={{background:cpLink?B.greenBg:B.purpleBg,border:`1px solid ${cpLink?B.green:B.purple}44`,borderRadius:6,padding:"4px 8px",cursor:"pointer",color:cpLink?B.green:B.purple,display:"flex",alignItems:"center",gap:4,fontSize:11,fontWeight:700}}>
            <ILink s={12} c={cpLink?B.green:B.purple}/>{cpLink?"Copiado!":"Link"}
          </button>}
          <button onClick={()=>setSP(p=>!p)} style={{background:showPhotos?B.purpleBg:`${B.purple}15`,border:`1px solid ${B.purple}44`,borderRadius:6,padding:"4px 8px",cursor:"pointer",color:B.purple,display:"flex",alignItems:"center",gap:4,fontSize:11,fontWeight:700}}>
            <IPhoto s={12} c={B.purple}/>Fotos
          </button>
          {!hideManagerButtons&&<button onClick={()=>setXfO(true)} style={{background:B.blueBg,border:`1px solid ${B.blue}44`,borderRadius:6,padding:"4px 8px",cursor:"pointer",color:B.blue,display:"flex",alignItems:"center",gap:4,fontSize:11,fontWeight:700}}>
            <IUser s={12} c={B.blue}/>Cliente
          </button>}
          {!hideManagerButtons&&<button onClick={()=>setXfM(true)} style={{background:`${B.orange}22`,border:`1px solid ${B.orange}44`,borderRadius:6,padding:"4px 8px",cursor:"pointer",color:B.orange,display:"flex",alignItems:"center",gap:4,fontSize:11,fontWeight:700}}>
            <IWrench s={12} c={B.orange}/>Mec.
          </button>}
          {!hideManagerButtons&&<button onClick={()=>onDeleteVehicle(vehicle.id)} style={{background:"none",border:"none",cursor:"pointer",color:B.gray600,padding:4,display:"flex"}}
            onMouseEnter={e=>e.currentTarget.style.color=B.red} onMouseLeave={e=>e.currentTarget.style.color=B.gray600}><ITrash s={15}/></button>}
        </div>
      ...
      {open&&<div style={{padding:"12px 14px"}}>
        ...
        {vts.map(t=>managerMode
          ?<TaskItemManager key={t.id} task={t} defaultRate={defaultRate} stock={stock} onToggle={onToggleTask} onDelete={onDeleteTask} onUpdate={onUpdateTask} onConsumeStock={onConsumeStock} onReturnStock={onReturnStock}/>
          :<TaskItemMechanic key={t.id} task={t} onToggle={onToggleTask} onDelete={onDeleteTask} onUpdate={onUpdateTask}/>
        )}
    ...
    {xfM&&<TransferModal title="Transferir Mecânico" subtitle={`${vehicle.model} — ${vehicle.plate}`}
      items={employees.filter(e=>e.id!==vehicle.employeeId).map(e=>({id:e.id,label:e.name,sub:e.phone,icon:<IWrench s={15} c={B.orange}/>}))}
      onPick={id=>{onTransferMechanic(vehicle.id,id);setXfM(false);}} onClose={()=>setXfM(false)}/>}
...
// ─── EmployeeCard ─────────────────────────────────────────────────────────────
function EmployeeCard({employee,vehicles,tasks,employees,clients,stock,defaultRate,onAddVehicle,onDeleteVehicle,onTransferMechanic,onTransferOwner,onAddTask,onToggleTask,onDeleteTask,onUpdateTask,onUpdateVehicle,onConsumeStock,onReturnStock,onDelete,onSendWA,onUpdatePhone,onUpdateName}) {
  const [open,setOpen]=useState(true);
  const [showF,setSF]=useState(false);
  const [model,setMod]=useState(""); const [plate,setPlate]=useState("");
  const empV=vehicles.filter(v=>v.employeeId===employee.id);
  ...
  return (<div style={{background:B.gray800,borderRadius:14,border:`1px solid ${B.gray700}`,marginBottom:20,overflow:"hidden",boxShadow:"0 4px 20px rgba(0,0,0,.35)"}}>
    <div style={{padding:"12px 16px",background:B.gray900,borderBottom:`2px solid ${B.orange}`,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
      ...
      <div style={{display:"flex",gap:5,flexShrink:0}}>
        ...
        <button onClick={()=>onDelete(employee.id)} style={{background:`${B.white}10`,border:"none",borderRadius:8,padding:7,cursor:"pointer",color:B.gray400,display:"flex"}}
...
// ─── ClientCard ───────────────────────────────────────────────────────────────
function ClientCard({client,vehicles,tasks,employees,clients,stock,defaultRate,onUpdatePhone,onUpdateName,onDelete,onSendWA,onTransferMechanic,onTransferOwner,onToggleTask,onDeleteTask,onAddTask,onUpdateTask,onUpdateVehicle,onDeleteVehicle,onConsumeStock,onReturnStock,payments=[],onAddPayment,onDeletePayment}) {
  const [open,setOpen]=useState(true);
  ...
  return (<div style={{background:B.gray800,borderRadius:14,border:`1px solid ${B.gray700}`,marginBottom:20,overflow:"hidden",boxShadow:"0 4px 20px rgba(0,0,0,.35)"}}>
    <div style={{padding:"12px 16px",background:B.gray900,borderBottom:`2px solid ${B.blue}`,display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
      ...
      <div style={{display:"flex",gap:5,flexShrink:0}}>
        ...
        <button onClick={()=>onDelete(client.id)} style={{background:`${B.white}10`,border:"none",borderRadius:8,padding:7,cursor:"pointer",color:B.gray400,display:"flex"}}
          onMouseEnter={e=>{e.currentTarget.style.background="#ef444430";e.currentTarget.style.color=B.red;}}
          onMouseLeave={e=>{e.currentTarget.style.background=`${B.white}10`;e.currentTarget.style.color=B.gray400;}}><ITrash s={14}/></button>
        <button onClick={()=>setOpen(o=>!o)} style={{background:`${B.white}10`,border:"none",borderRadius:8,padding:7,cursor:"pointer",color:B.gray400,display:"flex"}}>{open?<IChevU s={14} c={B.gray400}/>:<IChevD s={14} c={B.gray400}/>}</button>
      </div>
      ...
      <div style={{padding:20,overflowY:"auto",flex:1}}>
        ...
        {vPayments.length===0?<div style={{textAlign:"center",padding:"16px 0",color:B.gray500,fontSize:13}}>Nenhum pagamento registrado ainda.</div>
          :vPayments.map(p=>(<div key={p.id} style={{display:"flex",alignItems:"center",gap:10,padding:"9px 12px",background:B.gray700,borderRadius:9,marginBottom:6}}>
            ...
            <button onClick={()=>onDeletePayment(p.id)} style={{background:"none",border:"none",cursor:"pointer",color:B.gray500,padding:4,display:"flex",flexShrink:0}}
...
function StockTab({stock,onAdd,onUpdate,onDelete}) {
  const [form,setForm]=useState({name:"",brand:"",type:"",qty:1,costPrice:0,markup:100,photo:""});
  const [showAdd,setShowAdd]=useState(false);
  const [filterBrand,setFB]=useState("");
  const [filterType,setFT]=useState("");
  const [lightbox,setLB]=useState(null);

  const brands=[...new Set(stock.map(s=>s.brand).filter(Boolean))].sort();
  ...
  const addItem=()=>{
    ...
    const sp=Number(form.costPrice||0)*(1+Number(form.markup||0)/100);
    onAdd({...form,qty:Number(form.qty||0),costPrice:Number(form.costPrice||0),markup:Number(form.markup||0),salePrice:sp});
    setForm({name:"",brand:"",type:"",qty:1,costPrice:0,markup:100,photo:""});
    setShowAdd(false);
  };
  ...
  return (<div>
    ...
    <div style={{display:"flex",gap:8,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
      <select value={filterBrand} onChange={e=>setFB(e.target.value)} style={{padding:"7px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray800,color:filterBrand?B.white:B.gray400,fontSize:13,outline:"none",cursor:"pointer"}}>
      ...
      {(filterBrand||filterType)&&<button onClick={()=>{setFB("");setFT("");}} style={{padding:"7px 12px",borderRadius:8,background:B.gray700,border:`1px solid ${B.gray600}`,color:B.gray200,cursor:"pointer",fontSize:13}}>Limpar filtros</button>}
    ...
    {showAdd&&<div style={{background:B.gray800,borderRadius:12,border:`1px solid ${B.purple}55`,padding:18,marginBottom:20}}>
      <div style={{fontWeight:700,fontSize:12,color:B.purple,marginBottom:14,textTransform:"uppercase",letterSpacing:.6}}>Novo Produto</div>
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        {[
          {k:"name",ph:"Nome do produto",fl:"1 1 180px"},
          {k:"brand",ph:"Marca",fl:"1 1 120px"},
          {k:"type",ph:"Tipo/Categoria",fl:"1 1 130px"},
          {k:"qty",ph:"Qtd",fl:"0 0 70px",tp:"number"},
          {k:"costPrice",ph:"Preço custo (R$)",fl:"0 0 130px",tp:"number"},
          {k:"markup",ph:"Markup (%)",fl:"0 0 110px",tp:"number"},
      ...
      <div style={{marginTop:8,fontSize:12,color:B.gray400}}>
        Preço de custo: <b style={{color:B.white}}>{fmtBRL(form.costPrice||0)}</b> + markup {form.markup||0}% → Preço de venda: <b style={{color:B.amber}}>{fmtBRL(Number(form.costPrice||0)*(1+Number(form.markup||0)/100))}</b>
      </div>
      {/* Photo upload */}
      ...
      <div style={{display:"flex",gap:8,marginTop:14}}>
        <button onClick={addItem} style={{padding:"8px 20px",borderRadius:8,background:B.purple,border:"none",color:B.white,cursor:"pointer",fontWeight:800,fontSize:13}}>Salvar</button>
        <button onClick={()=>setShowAdd(false)} style={{padding:"8px 14px",borderRadius:8,background:B.gray700,border:"none",color:B.gray200,cursor:"pointer",fontSize:13}}>Cancelar</button>
      </div>
    </div>}
    ...
    {filtered.length===0?<div style={{textAlign:"center",padding:"48px 0",color:B.gray400}}><div style={{fontSize:40,marginBottom:12}}>📦</div><div style={{fontWeight:700,color:B.gray200,marginBottom:4}}>Estoque vazio</div><div style={{fontSize:13}}>Adicione produtos clicando em "Novo produto".</div></div>
      :<div style={{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(280px,1fr))",gap:14}}>
        {filtered.map(item=><StockItem key={item.id} item={item} salePrice={salePrice(item)} onUpdate={onUpdate} onDelete={onDelete} onLightbox={setLB}/>)}
      </div>}
...
function StockItem({item,salePrice,onUpdate,onDelete,onLightbox}) {
  const low=item.qty<=2;
  return (<div style={{background:B.gray800,borderRadius:12,border:`1px solid ${low?B.red+"55":B.gray700}`,overflow:"hidden"}}>
    {/* photo */}
    <div style={{position:"relative",height:140,background:B.gray700,cursor:item.photo?"pointer":"default"}} onClick={()=>item.photo&&onLightbox(item.photo)}>
      {item.photo?<img src={item.photo} alt="" style={{width:"100%",height:"100%",objectFit:"cover"}}/>
        :<div style={{width:"100%",height:"100%",display:"flex",alignItems:"center",justifyContent:"center",opacity:.3}}><IBox s={40} c={B.white}/></div>}
      {low&&<span style={{position:"absolute",top:8,right:8,background:B.red,color:B.white,borderRadius:6,padding:"2px 8px",fontSize:11,fontWeight:700}}>⚠ Baixo</span>}
      <UploadBtn onFile={src=>onUpdate(item.id,{photo:src})} folder="stock" label="" accept="image/*" style={{position:"absolute",bottom:6,right:6,padding:"4px 6px",borderRadius:6,fontSize:10,gap:2}}/>
    </div>
    <div style={{padding:"10px 12px"}}>
      <div style={{fontWeight:700,fontSize:13.5,color:B.white,marginBottom:2}}>{item.name}</div>
      <div style={{fontSize:11,color:B.gray400,marginBottom:8,display:"flex",gap:8,flexWrap:"wrap"}}>
      ...
      <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}>
        <span style={{fontSize:11,color:B.gray400}}>Qtd:</span>
        <button onClick={()=>onUpdate(item.id,{qty:Math.max(0,item.qty-1)})} style={{width:24,height:24,borderRadius:5,background:B.gray700,border:"none",color:B.white,cursor:"pointer",fontWeight:700,fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>-</button>
        <span style={{fontWeight:800,fontSize:14,color:low?B.red:B.white,minWidth:24,textAlign:"center"}}>{item.qty}</span>
        <button onClick={()=>onUpdate(item.id,{qty:item.qty+1})} style={{width:24,height:24,borderRadius:5,background:B.gray700,border:"none",color:B.white,cursor:"pointer",fontWeight:700,fontSize:14,display:"flex",alignItems:"center",justifyContent:"center"}}>+</button>
      </div>
      {/* Prices */}
      <div style={{display:"flex",gap:8,marginBottom:8,flexWrap:"wrap"}}>
        <div style={{background:B.gray700,borderRadius:7,padding:"5px 8px",flex:1}}>
          <div style={{fontSize:10,color:B.gray400}}>Custo</div>
          <div style={{fontSize:13,fontWeight:700,color:B.white}}>{fmtBRL(item.costPrice)}</div>
        </div>
        <div style={{background:B.gray700,borderRadius:7,padding:"5px 8px",flex:1}}>
          <div style={{fontSize:10,color:B.gray400}}>Markup</div>
          <div style={{fontSize:13,fontWeight:700,color:B.white}}>{item.markup}%</div>
        </div>
        <div style={{background:B.amberBg,border:`1px solid ${B.amber}44`,borderRadius:7,padding:"5px 8px",flex:1}}>
          <div style={{fontSize:10,color:B.amber}}>Venda</div>
          <div style={{fontSize:13,fontWeight:800,color:B.amber}}>{fmtBRL(salePrice)}</div>
        </div>
      </div>
      <button onClick={()=>onDelete(item.id)} style={{width:"100%",padding:"6px 0",borderRadius:7,background:"transparent",border:`1px solid ${B.gray700}`,color:B.gray500,cursor:"pointer",fontSize:12,display:"flex",alignItems:"center",justifyContent:"center",gap:5}}
        onMouseEnter={e=>{e.currentTarget.style.borderColor=B.red;e.currentTarget.style.color=B.red;}}
        onMouseLeave={e=>{e.currentTarget.style.borderColor=B.gray700;e.currentTarget.style.color=B.gray500;}}>
        <ITrash s={13}/>Remover produto
      </button>
    </div>
  </div>);
}
...
function SettingsPanel({defaultRate,onSave,onClose}) {
  const [v,sV]=useState(String(defaultRate||0));
  return (<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:80,padding:16}} onClick={onClose}>
    <div style={{background:B.gray800,borderRadius:16,maxWidth:380,width:"100%",overflow:"hidden",border:`1px solid ${B.gray700}`}} onClick={e=>e.stopPropagation()}>
      <div style={{padding:"14px 18px",background:B.gray900,borderBottom:`2px solid ${B.amber}`,display:"flex",alignItems:"center",gap:10}}>
        ...
        <div><div style={{fontWeight:700,fontSize:14,color:B.white}}>Preço base por hora</div><div style={{fontSize:12,color:B.gray400}}>Visível apenas ao gestor</div></div>
      </div>
      <div style={{padding:20}}>
        ...
        <div style={{display:"flex",gap:8,marginTop:16}}>
          <button onClick={()=>{onSave(parseFloat(v.replace(",","."))||0);onClose();}} style={{flex:1,padding:"10px 0",borderRadius:9,background:B.amber,border:"none",color:B.black,fontWeight:800,cursor:"pointer",fontSize:14}}>Salvar</button>
      ...
      <div style={{background:B.gray800,borderRadius:14,padding:"16px 20px",marginBottom:20,border:`1px solid ${B.gray700}`}}>
        ...
        {ts.map(t=><div key={t.id} style={{display:"flex",alignItems:"flex-start",gap:8,padding:"7px 0",borderBottom:`1px solid ${B.gray700}`}}>
          ...
          <div>
            ...
            {t.material&&<div style={{fontSize:11,color:B.gray400,marginTop:2}}>🔩 {t.material}</div>}
          </div>
        </div>)}
      </div>
      {/* Photos */}
...
function getAdminPassword() {
  // Set in Vercel: Settings → Environment Variables → VITE_ADMIN_PASSWORD
...
function AdminLoginScreen({onLogin}) {
  ...
  const tryLogin=()=>{
    ...
    if(!real){ setErr("Senha de administrador não configurada. Configure VITE_ADMIN_PASSWORD no Vercel."); return; }
    if(pwd!==real){ setErr("Senha incorreta."); return; }
    onLogin();
  };
  return (<div style={{minHeight:"100vh",background:B.black,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Inter','Segoe UI',sans-serif",padding:20}}>
    <div style={{maxWidth:380,width:"100%"}}>
      ...
      <div style={{background:B.gray800,borderRadius:16,padding:24,border:`1px solid ${B.gray700}`}}>
        <label style={{display:"flex",alignItems:"center",gap:6,fontSize:13,color:B.gray200,fontWeight:600,marginBottom:10}}><ILock s={14} c={B.orange}/>Senha de administrador</label>
        ...
        {err&&<div style={{color:B.red,fontSize:12,marginTop:8}}>{err}</div>}
        <button onClick={tryLogin} style={{width:"100%",marginTop:16,padding:"12px 0",borderRadius:10,background:B.orange,border:"none",color:B.white,fontWeight:800,fontSize:14,cursor:"pointer"}}>Entrar</button>
        <div style={{marginTop:14,fontSize:11.5,color:B.gray500,textAlign:"center",lineHeight:1.5}}>Acesso restrito ao gestor da oficina.</div>
      </div>
    </div>
  </div>);
}
...
export default function App() {
  ...
  const [cN,setCN]=useState(""); const [cP,setCP]=useState("");
  ...
  if(isMechanicPortal){
    ...
    return <MechanicPortal employee={liveEmp} vehicles={vehicles} tasks={tasks} employees={employees} clients={clients} stock={stock}
      onAddTask={async(vid,lbl)=>{try{const row=await db.addTask({vehicleId:vid,label:lbl,done:false,material:"",materialCost:0,hours:0,ratePerHour:null,fromStock:false,stockItemId:null});setTsk(p=>[...p,row]);}catch(e){errToast(e);}}}
      onToggleTask={async id=>{const t=tasks.find(x=>x.id===id);setTsk(p=>p.map(t=>t.id===id?{...t,done:!t.done}:t));try{await db.updateTask(id,{done:!t.done});}catch(e){errToast(e);}}}
  ...
  if(!adminAuthed){
    return <AdminLoginScreen onLogin={()=>{ setAdminAuthed(true); try{ sessionStorage.setItem(ADMIN_SESSION_KEY,"1"); }catch{} }}/>;
  }
  ...
  const addCli=async()=>{
    if(!cN.trim())return;
    try{ const row=await db.addClient(cN.trim(),cP.trim()); setCli(p=>[...p,row]); setCN("");setCP("");toast_("Cliente cadastrado ✓"); }
    catch(e){errToast(e);}
  };
  ...
  const xferMech=async(vid,eid)=>{
    setVeh(p=>p.map(v=>v.id===vid?{...v,employeeId:eid}:v));
    try{ await db.updateVehicle(vid,{employeeId:eid}); toast_(`Transferido para ${employees.find(e=>e.id===eid)?.name} ✓`); }catch(e){errToast(e);}
  };
  const xferOwn=async(vid,cid)=>{
    setVeh(p=>p.map(v=>v.id===vid?{...v,clientId:cid}:v));
    try{ await db.updateVehicle(vid,{clientId:cid}); toast_(`Vinculado a ${clients.find(c=>c.id===cid)?.name} ✓`); }catch(e){errToast(e);}
  };

  // ── Tasks
  const addTask=async(vid,lbl)=>{
    try{
      const row=await db.addTask({vehicleId:vid,label:lbl,done:false,material:"",materialCost:0,hours:0,ratePerHour:null,fromStock:false,stockItemId:null});
      setTsk(p=>[...p,row]);
    }catch(e){errToast(e);}
  };
  const toggleT=async id=>{
    const t=tasks.find(x=>x.id===id);
    setTsk(p=>p.map(t=>t.id===id?{...t,done:!t.done}:t));
    try{ await db.updateTask(id,{done:!t.done}); }catch(e){errToast(e);}
  };
  const delTask=async id=>{
    const t=tasks.find(x=>x.id===id);
    if(t?.fromStock&&t?.stockItemId){
      setStk(p=>p.map(s=>s.id===t.stockItemId?{...s,qty:s.qty+1}:s));
      try{ const item=stock.find(s=>s.id===t.stockItemId); if(item) await db.updateStock(t.stockItemId,{qty:item.qty+1}); }catch(e){errToast(e);}
    }
  ...
  const consumeStock=async(taskId,item)=>{
    const newQty=Math.max(0,item.qty-1);
    setStk(p=>p.map(s=>s.id===item.id?{...s,qty:newQty}:s));
    setTsk(p=>p.map(t=>t.id===taskId?{...t,material:item.name,materialCost:item.salePrice,fromStock:true,stockItemId:item.id}:t));
    try{
      await db.updateStock(item.id,{qty:newQty});
      await db.updateTask(taskId,{material:item.name,materialCost:item.salePrice,fromStock:true,stockItemId:item.id});
      toast_(`${item.name} descontado do estoque ✓`);
    }catch(e){errToast(e);}
  };
  const returnStock=async taskId=>{
    const t=tasks.find(x=>x.id===taskId);
    if(t?.stockItemId){
      const item=stock.find(s=>s.id===t.stockItemId);
      const newQty=(item?.qty||0)+1;
      setStk(p=>p.map(s=>s.id===t.stockItemId?{...s,qty:newQty}:s));
      try{ await db.updateStock(t.stockItemId,{qty:newQty}); }catch(e){errToast(e);}
    }
    setTsk(p=>p.map(t=>t.id===taskId?{...t,material:"",materialCost:0,fromStock:false,stockItemId:null}:t));
    try{ await db.updateTask(taskId,{material:"",materialCost:0,fromStock:false,stockItemId:null}); toast_("Item devolvido ao estoque ✓"); }catch(e){errToast(e);}
  };
  ...
  return (<div style={{minHeight:"100vh",background:B.black,fontFamily:"'Inter','Segoe UI',sans-serif",color:B.white}}>
    ...
    <div style={{background:B.gray900,borderBottom:`1px solid ${B.gray700}`,padding:"0 20px",position:"sticky",top:0,zIndex:20,boxShadow:"0 2px 20px rgba(0,0,0,.6)"}}>
      <div style={{maxWidth:820,margin:"0 auto",display:"flex",alignItems:"center",height:60,gap:12}}>
        ...
        <div><div style={{fontWeight:900,fontSize:15,color:B.white,letterSpacing:"-.5px"}}>OSC <span style={{color:B.orange}}>Performance</span></div><div style={{fontSize:9,color:B.gray400,textTransform:"uppercase",letterSpacing:.6}}>Gestão de Oficina</div></div>
        <div style={{marginLeft:"auto",display:"flex",gap:5,alignItems:"center",flexWrap:"wrap"}}>
          ...
          <button onClick={()=>{setAdminAuthed(false);try{sessionStorage.removeItem(ADMIN_SESSION_KEY);}catch{}}} title="Sair (logout)" style={{width:34,height:34,borderRadius:8,background:`${B.red}1a`,border:`1px solid ${B.red}44`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0,marginLeft:4}}>
    ...
    <div style={{maxWidth:820,margin:"0 auto",padding:"20px 14px"}}>
      ...
      <div style={{display:"flex",gap:3,marginBottom:20,background:B.gray900,padding:4,borderRadius:12,border:`1px solid ${B.gray700}`,width:"fit-content",flexWrap:"wrap"}}>
        {tabBtn("mechanics","Mecânicos",<IWrench s={13}/>,B.orange)}
        {tabBtn("clients","Clientes / OS",<IUser s={13}/>,B.blue)}
        {tabBtn("stock","Estoque",<IWarehouse s={13}/>,B.purple)}
        {tabBtn("finance","Financeiro",<IChart s={13}/>,B.green)}
      </div>
      ...
      {tab==="clients"&&<>
        ...
        {clients.length===0?<div style={{textAlign:"center",padding:"56px 0",color:B.gray400}}><div style={{fontSize:44,marginBottom:12}}>👤</div><div style={{fontWeight:700,fontSize:15,color:B.gray200,marginBottom:4}}>Nenhum cliente</div><div style={{fontSize:13}}>Cadastre clientes para gerar OS e enviar progresso por WhatsApp.</div></div>
          :clients.map(cli=><ClientCard key={cli.id} client={cli} vehicles={vehicles} tasks={tasks} employees={employees} clients={clients} stock={stock} defaultRate={defaultRate}
            onUpdatePhone={updCliP} onUpdateName={updCliN} onDelete={delCli} onSendWA={sendCliWA}
      ...
      {tab==="stock"&&<>
        ...
        <StockTab stock={stock} onAdd={addStock} onUpdate={updStock} onDelete={delStock}/>
      </>}
    ...
    {showCfg&&<SettingsPanel defaultRate={defaultRate} onSave={saveRate} onClose={()=>setSCfg(false)}/>}
