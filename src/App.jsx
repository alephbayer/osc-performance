...
function StockTab({stock,purchases,onAdd,onUpdate,onDelete,onAddPurchase}) {
  ...
  const addItem=()=>{
    ...
    const costPrice=Number(form.costPrice||0), markup=Number(form.markup||0), minQty=Number(form.minQty||2);
...
function StockProductPanel({item,purchases,onSave,onDelete,onAddPurchase,onClose}) {
  ...
  const save=()=>{
    ...
    onSave(item.id,{
      ...
      minQty:Number(form.minQty||2), costPrice:Number(form.costPrice||0), markup:Number(form.markup||0),
  ...
  return (<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:90,padding:16}} onClick={onClose}>
    <div style={{background:B.gray800,borderRadius:16,maxWidth:560,width:"100%",overflow:"hidden",border:`1px solid ${B.purple}55`,boxShadow:"0 24px 80px rgba(0,0,0,.7)",maxHeight:"92vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
      ...
      <div style={{padding:20,overflowY:"auto",flex:1}}>
        ...
        {/* Product data form */}
        <div style={{fontSize:11,color:B.purple,fontWeight:700,marginBottom:8,textTransform:"uppercase",letterSpacing:.5}}>Dados do produto</div>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          ...
          <div style={{display:"flex",gap:8}}>
            <input value={form.location} onChange={e=>setForm(p=>({...p,location:e.target.value}))} placeholder="Localização (ex: Prateleira A3)"
              style={{flex:1,padding:"9px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none"}}/>
            <input value={form.minQty} onChange={e=>setForm(p=>({...p,minQty:e.target.value}))} placeholder="Qtd mínima" type="number"
              style={{flex:"0 0 120px",padding:"9px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none"}}/>
          </div>
          <div style={{display:"flex",gap:8}}>
            <input value={form.costPrice} onChange={e=>setForm(p=>({...p,costPrice:e.target.value}))} placeholder="Custo unitário (R$)" type="number"
              style={{flex:1,padding:"9px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none"}}/>
        ...
        <div style={{display:"flex",gap:8,marginTop:14}}>
          ...
          <button onClick={()=>onDelete(item.id)} style={{padding:"9px 14px",borderRadius:9,background:"transparent",border:`1px solid ${B.red}44`,color:B.red,cursor:"pointer",fontWeight:600,fontSize:12,display:"flex",alignItems:"center",gap:5}}>
        ...
        <div style={{marginTop:24,paddingTop:18,borderTop:`1px solid ${B.gray700}`}}>
          ...
          {sortedPurchases.length===0?<div style={{textAlign:"center",padding:"16px 0",color:B.gray500,fontSize:13}}>Nenhuma compra registrada ainda.</div>
            :<div style={{display:"flex",flexDirection:"column",gap:6}}>
              {sortedPurchases.map(p=>(<div key={p.id} style={{background:B.gray700,borderRadius:9,padding:"9px 12px",display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
                ...
                <div style={{flex:"1 1 140px",minWidth:0}}>
                  <div style={{fontSize:12.5,fontWeight:700,color:B.white}}>{p.qty}x · {fmtBRL(p.unitCost)} cada · {fmtBRL(p.totalCost)} total</div>
                  <div style={{fontSize:11,color:B.gray400}}>{new Date(p.purchaseDate+"T00:00:00").toLocaleDateString("pt-BR")}{p.supplier?` · ${p.supplier}`:""}{p.invoiceNumber?` · NF ${p.invoiceNumber}`:""}</div>
                </div>
              </div>))}
            </div>}
        </div>
  ...
  return (<div style={{background:B.gray900,borderRadius:12,padding:14,marginBottom:14,border:`1px solid ${B.green}44`}}>
    ...
    <div style={{display:"flex",gap:7,flexWrap:"wrap"}}>
      ...
      <input value={supplier} onChange={e=>setSupplier(e.target.value)} placeholder="Fornecedor"
        style={{flex:"1 1 150px",padding:"8px 11px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none"}}/>
      <input value={invoice} onChange={e=>setInvoice(e.target.value)} placeholder="Número da NF"
        style={{flex:"1 1 120px",padding:"8px 11px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none"}}/>
    </div>
    <div style={{display:"flex",gap:7,flexWrap:"wrap",marginTop:7}}>
      <input value={qty} onChange={e=>setQty(e.target.value)} placeholder="Quantidade" type="number"
        style={{flex:"1 1 110px",padding:"8px 11px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none"}}/>
      <input value={unitCost} onChange={e=>setUnitCost(e.target.value)} placeholder="Custo unitário (R$)" type="number"
        style={{flex:"1 1 150px",padding:"8px 11px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray800,color:B.white,fontSize:13,outline:"none"}}/>
      <div style={{flex:"1 1 140px",padding:"8px 11px",borderRadius:8,background:B.amberBg,border:`1px solid ${B.amber}44`,display:"flex",alignItems:"center",justifyContent:"space-between"}}>
        <span style={{fontSize:11,color:B.amber}}>Total</span>
        <span style={{fontSize:13,fontWeight:800,color:B.amber}}>{fmtBRL(total)}</span>
      </div>
    </div>
    <div style={{fontSize:11,color:B.gray500,marginTop:8}}>Depois de confirmado, este lançamento fica travado para conferência futura.</div>
  ...
  return (<div style={{minHeight:"100vh",background:B.black,fontFamily:"'Inter','Segoe UI',sans-serif",color:B.white}}>
    ...
    <div style={{maxWidth:820,margin:"0 auto",padding:"20px 14px"}}>
      ...
      {tab==="stock"&&allowedTabs.includes("stock")&&<>
        ...
        <StockTab stock={stock} purchases={stockPurchases} onAdd={addStock} onUpdate={updStock} onDelete={delStock} onAddPurchase={addPurchase}/>
      </>}
