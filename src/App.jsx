...
function StockTab({stock,onAdd,onUpdate,onDelete}) {
  const emptyForm = {name:"",brand:"",type:"",qty:"",costPrice:"",markup:"",photo:""};
  ...
  const addItem=()=>{
    ...
    const qty=Number(form.qty||0), costPrice=Number(form.costPrice||0), markup=Number(form.markup||0);
    const sp=costPrice*(1+markup/100);
    onAdd({...form,qty,costPrice,markup,salePrice:sp});
    setForm(emptyForm);
    setShowAdd(false);
  };
  ...
  return (<div>
    ...
    {showAdd&&<div style={{background:B.gray800,borderRadius:12,border:`1px solid ${B.purple}55`,padding:18,marginBottom:20}}>
      ...
      <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
        {[
          ...
          {k:"qty",ph:"Qtd (ex: 5)",fl:"0 0 90px",tp:"number"},
          {k:"costPrice",ph:"Preço custo (R$)",fl:"0 0 140px",tp:"number"},
    ...
    {filtered.length===0?<div style={{textAlign:"center",padding:"48px 0",color:B.gray400}}><div style={{fontSize:40,marginBottom:12}}>📦</div><div style={{fontWeight:700,color:B.gray200,marginBottom:4}}>Estoque vazio</div><div style={{fontSize:13}}>Adicione produtos clicando em "Novo produto".</div></div>
      :<div style={{display:"flex",flexDirection:"column",gap:8}}>
        {filtered.map(item=><StockRow key={item.id} item={item} salePrice={salePrice(item)} onUpdate={onUpdate} onLightbox={setLB} onEdit={()=>setEditItem(item)}/>)}
      </div>}
    ...
    {editItem&&<StockEditModal item={editItem} onSave={(id,patch)=>{onUpdate(id,patch);setEditItem(null);}} onDelete={id=>{onDelete(id);setEditItem(null);}} onClose={()=>setEditItem(null)}/>}
  </div>);
}

// ─── Stock row (list view) ────────────────────────────────────────────────────
function StockRow({item,salePrice,onUpdate,onLightbox,onEdit}) {
  const low=item.qty<=2;
  return (<div style={{background:B.gray800,borderRadius:10,border:`1px solid ${low?B.red+"55":B.gray700}`,padding:"10px 14px",display:"flex",alignItems:"center",gap:12,flexWrap:"wrap"}}>
    {/* photo thumb */}
    <div style={{width:48,height:48,borderRadius:8,background:B.gray700,flexShrink:0,cursor:item.photo?"pointer":"default",overflow:"hidden"}} onClick={()=>item.photo&&onLightbox(item.photo)}>
    ...
    <div style={{display:"flex",alignItems:"center",gap:6,flexShrink:0}}>
      <button onClick={()=>onUpdate(item.id,{qty:Math.max(0,item.qty-1)})} style={{width:24,height:24,borderRadius:5,background:B.gray700,border:"none",color:B.white,cursor:"pointer",fontWeight:700,fontSize:14}}>-</button>
      <span style={{fontWeight:800,fontSize:14,color:low?B.red:B.white,minWidth:22,textAlign:"center"}}>{item.qty}</span>
      <button onClick={()=>onUpdate(item.id,{qty:item.qty+1})} style={{width:24,height:24,borderRadius:5,background:B.gray700,border:"none",color:B.white,cursor:"pointer",fontWeight:700,fontSize:14}}>+</button>
    </div>
    ...
    {/* edit button */}
    <button onClick={onEdit} style={{padding:"7px 13px",borderRadius:8,background:B.purpleBg,border:`1px solid ${B.purple}44`,color:B.purple,cursor:"pointer",fontWeight:700,fontSize:12,display:"flex",alignItems:"center",gap:5,flexShrink:0}}>
      <IEdit s={13} c={B.purple}/>Editar
    </button>
  </div>);
}

// ─── Stock edit modal ──────────────────────────────────────────────────────────
function StockEditModal({item,onSave,onDelete,onClose}) {
  const [form,setForm]=useState({name:item.name,brand:item.brand,type:item.type,qty:String(item.qty),costPrice:String(item.costPrice),markup:String(item.markup),photo:item.photo||""});
  const salePrice=Number(form.costPrice||0)*(1+Number(form.markup||0)/100);

  const save=()=>{
    if(!form.name.trim())return;
    onSave(item.id,{
      name:form.name.trim(), brand:form.brand.trim(), type:form.type.trim(),
      qty:Number(form.qty||0), costPrice:Number(form.costPrice||0), markup:Number(form.markup||0),
      salePrice, photo:form.photo||null,
    });
  };

  return (<div style={{position:"fixed",inset:0,background:"rgba(0,0,0,.85)",display:"flex",alignItems:"center",justifyContent:"center",zIndex:90,padding:16}} onClick={onClose}>
    <div style={{background:B.gray800,borderRadius:16,maxWidth:460,width:"100%",overflow:"hidden",border:`1px solid ${B.purple}55`,boxShadow:"0 24px 80px rgba(0,0,0,.7)",maxHeight:"90vh",display:"flex",flexDirection:"column"}} onClick={e=>e.stopPropagation()}>
      <div style={{padding:"16px 20px",background:B.gray900,borderBottom:`2px solid ${B.purple}`,display:"flex",alignItems:"center",gap:10,flexShrink:0}}>
        <div style={{width:36,height:36,borderRadius:8,background:B.purpleBg,display:"flex",alignItems:"center",justifyContent:"center"}}><IEdit s={17} c={B.purple}/></div>
        <div style={{fontWeight:700,fontSize:15,color:B.white}}>Editar Produto</div>
        <button onClick={onClose} style={{marginLeft:"auto",background:"none",border:"none",cursor:"pointer",color:B.gray400}}><IX s={18}/></button>
      </div>
      <div style={{padding:20,overflowY:"auto",flex:1}}>
        <div style={{display:"flex",flexDirection:"column",gap:10}}>
          ...
          <div style={{display:"flex",gap:8}}>
            ...
            <input value={form.type} onChange={e=>setForm(p=>({...p,type:e.target.value}))} placeholder="Tipo/Categoria"
              style={{flex:1,padding:"9px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none"}}/>
          </div>
          <div style={{display:"flex",gap:8}}>
            <input value={form.qty} onChange={e=>setForm(p=>({...p,qty:e.target.value}))} placeholder="Qtd" type="number"
              style={{flex:1,padding:"9px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none"}}/>
            <input value={form.costPrice} onChange={e=>setForm(p=>({...p,costPrice:e.target.value}))} placeholder="Custo (R$)" type="number"
              style={{flex:1,padding:"9px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none"}}/>
            <input value={form.markup} onChange={e=>setForm(p=>({...p,markup:e.target.value}))} placeholder="Markup (%)" type="number"
              style={{flex:1,padding:"9px 12px",borderRadius:8,border:`1px solid ${B.gray600}`,background:B.gray900,color:B.white,fontSize:13,outline:"none"}}/>
          </div>
          <div style={{fontSize:12,color:B.gray400}}>
            Preço de venda calculado: <b style={{color:B.amber}}>{fmtBRL(salePrice)}</b>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:10}}>
        ...
        <div style={{display:"flex",gap:8,marginTop:18}}>
          <button onClick={save} style={{flex:1,padding:"10px 0",borderRadius:9,background:B.purple,border:"none",color:B.white,fontWeight:800,cursor:"pointer",fontSize:14}}>Salvar alterações</button>
        ...
        <button onClick={()=>onDelete(item.id)} style={{width:"100%",marginTop:10,padding:"9px 0",borderRadius:9,background:"transparent",border:`1px solid ${B.red}44`,color:B.red,cursor:"pointer",fontWeight:600,fontSize:13,display:"flex",alignItems:"center",justifyContent:"center",gap:6}}>
          <ITrash s={14} c={B.red}/>Remover produto do estoque
        </button>
      </div>
    </div>
  </div>);
...
const ROLE_CONFIG = {
  owner:      { label:"Gestor",            tabs:["mechanics","clients","stock","finance"], envVar:"VITE_OWNER_PASSWORD" },
  admin:      { label:"Administrativo",    tabs:["mechanics","clients","stock"],           envVar:"VITE_ADMIN_PASSWORD" },
